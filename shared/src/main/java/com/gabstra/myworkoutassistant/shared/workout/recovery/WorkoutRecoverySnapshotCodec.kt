...
/**
 * Decoded runtime snapshot for process-death recovery.
 * Contains the state machine sequence and current index; platform uses it with
 * [WorkoutViewModel.restoreRecoveryStateMachine][com.gabstra.myworkoutassistant.shared.viewmodels.WorkoutViewModel.restoreRecoveryStateMachine].
 */
data class DecodedRecoveryRuntimeSnapshot(
    val workoutId: UUID,
    val workoutHistoryId: UUID?,
    val currentIndex: Int,
...
/**
 * Encodes/decodes the workout state machine to/from JSON for process-death recovery.
 * Platform stores the JSON string (e.g. via SharedPreferences); this codec is shared so
 * mobile and Wear OS use the same format.
 */
object WorkoutRecoverySnapshotCodec {
