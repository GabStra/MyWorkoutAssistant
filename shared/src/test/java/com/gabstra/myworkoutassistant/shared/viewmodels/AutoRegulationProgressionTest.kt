...
class AutoRegulationProgressionTest {
    ...
    @Test
    fun applyAutoRegulationRIR_modifiedRepsOnFirstSet_doesNotPropagateRepsToFollowingSet() = runTest(testDispatcher) {
        val initialWeight = 80.0
        val programmedReps = 8
        val modifiedFirstSetReps = 6
        val rir = 2.0
        ...
        val workSet1DataWithModifiedReps = WeightSetData(
            actualReps = modifiedFirstSetReps,
            actualWeight = initialWeight,
            volume = initialWeight * modifiedFirstSetReps
        )
        val workSet2Data = WeightSetData(programmedReps, initialWeight, initialWeight * programmedReps)

        val workSet1State = WorkoutState.Set(
            exerciseId = exerciseId,
            set = WeightSet(workSet1Id, programmedReps, initialWeight, subCategory = SetSubCategory.WorkSet),
            setIndex = 0u,
            previousSetData = null,
            currentSetDataState = mutableStateOf(workSet1DataWithModifiedReps),
            hasNoHistory = true,
            startTime = null,
            skipped = false,
            lowerBoundMaxHRPercent = null,
        ...
        val workSet2State = WorkoutState.Set(
            exerciseId = exerciseId,
            set = WeightSet(workSet2Id, programmedReps, initialWeight, subCategory = SetSubCategory.WorkSet),
            setIndex = 2u,
            previousSetData = null,
            currentSetDataState = mutableStateOf(workSet2Data),
            hasNoHistory = true,
        ...
        // exercisesById already populated by initializeExercisesMaps(workout) above
        ...
        val secondSetData = setStates[1].currentSetData as? WeightSetData
        assertNotNull(secondSetData)
        assertEquals(
            "Reps must not propagate from first set to following set; second set should keep programmed reps",
            programmedReps,
            secondSetData!!.actualReps
        )
    }
    ...
    fun completeAutoRegulationSet_repsInRange_doesNotPropagateManuallyPickedCurrentWeight() = runTest(testDispatcher) {
        ...
        val followingSetProgrammedWeight = 92.5
        val expectedWeight = nearestAvailableWeight(followingSetProgrammedWeight)
        val workoutExercise = Exercise(
            id = exerciseId,
            enabled = true,
            name = "Bench",
        ...
        val workSet2State = WorkoutState.Set(
            exerciseId = exerciseId,
            set = WeightSet(workSet2Id, 8, followingSetProgrammedWeight, subCategory = SetSubCategory.WorkSet),
            setIndex = 2u,
            previousSetData = null,
            currentSetDataState = mutableStateOf(
                WeightSetData(8, followingSetProgrammedWeight, followingSetProgrammedWeight * 8)
            ),
            hasNoHistory = true,
            startTime = null,
            skipped = false,
        ...
        assertNotNull(secondSet)
        assertTrue(
            "Subsequent set weight should stay based on its own programmed load. expected=$expectedWeight actual=${secondSet!!.weight}",
            abs(secondSet.weight - expectedWeight) < 0.01
        )
    }

    @Test
    fun completeAutoRegulationSet_repsAboveRange_usesSelectedCurrentWeightForFollowingSetAdjustment() = runTest(testDispatcher) {
        val programmedWeight = 80.0
        val pickedCurrentWeight = 85.0
        val followingSetProgrammedWeight = 92.5
        val expectedAdjustedWeight = nearestAvailableWeight(pickedCurrentWeight * 1.025)
        val workoutExercise = Exercise(
            id = exerciseId,
            enabled = true,
            name = "Bench",
        ...
        val workSet1State = WorkoutState.Set(
            ...
            previousSetData = null,
            currentSetDataState = mutableStateOf(
                WeightSetData(14, pickedCurrentWeight, pickedCurrentWeight * 14)
            ),
            hasNoHistory = true,
            startTime = null,
            skipped = false,
        ...
        assertNotNull(secondSet)
        assertTrue(
            "Subsequent set weight should be based on the final selected current weight. expected=$expectedAdjustedWeight actual=${secondSet!!.weight}",
            abs(secondSet.weight - expectedAdjustedWeight) < 0.01
        )
    }
