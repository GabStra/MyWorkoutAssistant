...
if (-not $TimingOutputPath) {
    $TimingOutputPath = Join-Path $logsDir "cross_device_resume_discard_$timestamp.json"
}
...
try {
    ...
    Write-Host "Preparing phone app state with an incomplete workout for Wear recovery..." -ForegroundColor Cyan
    ...
    Write-Host "Running mobile discard verification instrumentation test..." -ForegroundColor Cyan
    ...
    Write-Host "Cross-device resume discard E2E completed successfully." -ForegroundColor Green
} finally {
