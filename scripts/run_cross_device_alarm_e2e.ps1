Param(
    [string]$WearTestClass = "PhoneToWearScheduledAlarmTriggerE2ETest",
    [string]$MobilePrepTestClass = "com.gabstra.myworkoutassistant.e2e.PhoneAlarmSyncPreparationTest",
    [string]$ExpectedWorkoutName = "Alarm Trigger Workout",
...
if (-not $TimingOutputPath) {
    $TimingOutputPath = Join-Path $logsDir "cross_device_alarm_timings_$timestamp.json"
}
...
try {
    ...
    Write-Host "Preparing phone app state for alarm sync..." -ForegroundColor Cyan
    ...
    Write-Host "Running Wear alarm verification E2E test..." -ForegroundColor Cyan
    ...
    & pwsh @wearArgs
    if ($LASTEXITCODE -ne 0) {
        throw "Wear alarm verification E2E test failed."
    }
    $wearPhase.Stop()
    $timings["wearAlarmVerificationSeconds"] = [math]::Round($wearPhase.Elapsed.TotalSeconds, 3)

    Write-Host "Cross-device alarm E2E completed successfully." -ForegroundColor Green
} finally {
    $runStopwatch.Stop()
