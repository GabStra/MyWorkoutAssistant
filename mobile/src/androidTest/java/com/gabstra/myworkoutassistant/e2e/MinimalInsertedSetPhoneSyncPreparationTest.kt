...
class MinimalInsertedSetPhoneSyncPreparationTest {
    private fun resolvedWorkerTimeoutMs(): Long = 60_000

    @Test
    fun preparePhoneForMinimalInsertedSetCrossDeviceSync() = runBlocking {
        ...
        WorkoutStoreRepository(context.filesDir)
            .saveWorkoutStore(MinimalCrossDeviceInsertedSetPhoneWorkoutStoreFixture.createWorkoutStore())
    ...
    private suspend fun waitForSyncWorkerSuccess(
        ...
        workManager.getWorkInfoByIdFlow(requestId).first { info ->
            if (System.currentTimeMillis() - start > timeoutMs) {
                error("Timed out waiting for minimal inserted-set mobile sync worker success.")
            }
            when (info?.state) {
                WorkInfo.State.SUCCEEDED -> true
                WorkInfo.State.FAILED -> error("Minimal inserted-set mobile sync worker failed during phone prep.")
                WorkInfo.State.CANCELLED -> error("Minimal inserted-set mobile sync worker was cancelled during phone prep.")
                else -> false
            }
        }
    }
