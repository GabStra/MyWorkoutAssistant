...
class MinimalRestPausePhoneSyncPreparationTest {
    private fun resolvedWorkerTimeoutMs(): Long = 60_000

    @Test
    fun preparePhoneForMinimalRestPauseCrossDeviceSync() = runBlocking {
        ...
        WorkoutStoreRepository(context.filesDir)
            .saveWorkoutStore(MinimalCrossDeviceRestPausePhoneWorkoutStoreFixture.createWorkoutStore())
    ...
    private suspend fun waitForSyncWorkerSuccess(
        ...
        workManager.getWorkInfoByIdFlow(requestId).first { info ->
            if (System.currentTimeMillis() - start > timeoutMs) {
                error("Timed out waiting for minimal rest-pause mobile sync worker success.")
            }
            when (info?.state) {
                WorkInfo.State.SUCCEEDED -> true
                WorkInfo.State.FAILED -> error("Minimal rest-pause mobile sync worker failed during phone prep.")
                WorkInfo.State.CANCELLED -> error("Minimal rest-pause mobile sync worker was cancelled during phone prep.")
                else -> false
            }
        }
    }
