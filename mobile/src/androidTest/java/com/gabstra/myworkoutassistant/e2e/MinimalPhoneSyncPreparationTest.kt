...
class MinimalPhoneSyncPreparationTest {
    ...
    @Test
    fun preparePhoneForMinimalCrossDeviceSync() = runBlocking {
        ...
        WorkoutStoreRepository(context.filesDir)
            .saveWorkoutStore(MinimalCrossDeviceSyncPhoneWorkoutStoreFixture.createWorkoutStore())
    ...
    private suspend fun waitForSyncWorkerSuccess(
        ...
        workManager.getWorkInfoByIdFlow(requestId).first { info ->
            if (System.currentTimeMillis() - start > timeoutMs) {
                error("Timed out waiting for minimal mobile sync worker success.")
            }
            when (info?.state) {
                WorkInfo.State.SUCCEEDED -> true
                WorkInfo.State.FAILED -> error("Minimal mobile sync worker failed during phone prep.")
                WorkInfo.State.CANCELLED -> error("Minimal mobile sync worker was cancelled during phone prep.")
                else -> false
            }
        }
    }
