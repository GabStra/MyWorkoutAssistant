            ...
            if (isRepsInEditMode || isWeightInEditMode) {
                ControlButtonsVertical(
                    ...
                    isResetEnabled = if (isRepsInEditMode) {
                        ...
                        currentSetData.additionalWeight != initialSetData.additionalWeight
                    },
                    onCloseClick = {
                        isRepsInEditMode = false
                        isWeightInEditMode = false
                    ...
                    onResetClick = {
                        val resetSetData = currentSetData.copy(
                            ...
                            additionalWeight = if (isWeightInEditMode) initialSetData.additionalWeight else currentSetData.additionalWeight,
                        )
                        currentSetData = resetSetData.copy(volume = resetSetData.calculateVolume())
                        if (isWeightInEditMode) {
                            viewModel.schedulePlateRecalculation(initialSetData.getWeight())
                        }
                        updateInteractionTime()
                        hapticsViewModel.doGentleVibration()
                    },
