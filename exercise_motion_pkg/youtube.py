...
DEFAULT_YOUTUBE_SEARCH_TIMEOUT_SECONDS = 60.0
YOUTUBE_SEARCH_MAX_ATTEMPTS = 4
YOUTUBE_SEARCH_RETRY_BASE_DELAY_SECONDS = 2.0
YOUTUBE_SEARCH_RETRY_MAX_DELAY_SECONDS = 16.0
YOUTUBE_SEARCH_RETRY_JITTER_RATIO = 0.25
YOUTUBE_SEARCH_CACHE_SCHEMA_VERSION = 1
YOUTUBE_SEARCH_CACHE_FRESH_SECONDS = 6 * 60 * 60
YOUTUBE_SEARCH_CACHE_STALE_FALLBACK_SECONDS = 7 * 24 * 60 * 60
...
RECONSTRUCTION_VIDEO_FORMAT = (
    "bestvideo[height<=480][ext=mp4][vcodec!=none]/"
    "bestvideo[height<=480][vcodec!=none]/"
    "bestvideo[height<=720][ext=mp4][vcodec!=none]/"
    "bestvideo[height<=720][vcodec!=none]/"
    "bestvideo[ext=mp4][vcodec!=none]/bestvideo[vcodec!=none]/best"
)

RECONSTRUCTION_PROGRESSIVE_VIDEO_FORMAT = (
    "best[height<=480][ext=mp4][vcodec!=none]/"
    "best[height<=480][vcodec!=none]/"
    "best[height<=720][ext=mp4][vcodec!=none]/"
    "best[ext=mp4][vcodec!=none]/best[vcodec!=none]/best"
)

HIGH_RES_RECONSTRUCTION_VIDEO_FORMAT = (
    "bestvideo[height<=720][ext=mp4][vcodec!=none]/"
    "bestvideo[height<=720][vcodec!=none]/"
    "bestvideo[ext=mp4][vcodec!=none]/bestvideo[vcodec!=none]/best"
)

HIGH_RES_RECONSTRUCTION_PROGRESSIVE_VIDEO_FORMAT = (
    "best[height<=720][ext=mp4][vcodec!=none]/"
            ...
            options = build_youtube_download_options(
                ...
                format_selector=(
                    (
                        ...
                        else RECONSTRUCTION_PROGRESSIVE_VIDEO_FORMAT
                    )
                    if use_progressive_format
                    else (
                        HIGH_RES_RECONSTRUCTION_VIDEO_FORMAT
                        if prefer_high_resolution
                        else RECONSTRUCTION_VIDEO_FORMAT
                    )
                ),
            )
...
def build_youtube_download_options(
    *,
    ...
    if preview:
        ...
        options = {
            "format": format_selector or RECONSTRUCTION_VIDEO_FORMAT,
            "outtmpl": outtmpl,
            "quiet": quiet,
            "noprogress": noprogress,
            "noplaylist": True,
        ...
        options = {
            "format": RECONSTRUCTION_PROGRESSIVE_VIDEO_FORMAT,
            "outtmpl": outtmpl,
            "quiet": quiet,
            "noprogress": noprogress,
            "noplaylist": True,
...
class YouTubeRankingSettings:
    ...
    youtube_search_timeout_seconds: float = DEFAULT_YOUTUBE_SEARCH_TIMEOUT_SECONDS
...
class YouTubeSearchError(RuntimeError):
    ...
    def __init__(self, query: str, message: str, *, transient: bool = False) -> None:
        self.query = query
...
YOUTUBE_QUERY_EQUIPMENT_SYNONYM_GROUPS: tuple[tuple[str, tuple[str, ...]], ...] = (
    ("spin bike", ("Indoor Cycling", "Stationary Bike", "Spinning Bike")),
)
...
def extract_workout_plan_exercises(
    ...
    *,
    ...
    def motion_context(node: dict[str, Any]) -> dict[str, Any]:
        ...
        accessory_ids = [
            str(value).strip()
            for value in node.get("requiredAccessoryEquipmentIds") or []
            if str(value).strip()
        ]

        def equipment_summary(record: dict[str, Any] | None) -> dict[str, Any] | None:
        ...
        derived_context = {
            ...
            "secondaryMuscleGroups": list(node.get("secondaryMuscleGroups") or []),
            "primaryEquipment": equipment_summary(primary),
            "requiredAccessories": [
                summary
    ...
    def is_rest(node: dict[str, Any], *, is_library_definition: bool = False) -> bool:
        component_type = str(
            node.get("type")
        ...
        return (
            ...
            or (not is_library_definition and exercise_type in NON_MOTION_EXERCISE_TYPES)
        )
    ...
    def visit(node: Any, *, is_library_definition: bool = False) -> None:
        if isinstance(node, list):
            for child in node:
                visit(child, is_library_definition=is_library_definition)
            return
        if not isinstance(node, dict):
            return
        if is_disabled(node) or is_rest(node, is_library_definition=is_library_definition):
            return
    ...
    elif isinstance(payload, dict) and isinstance(payload.get("exerciseDefinitions"), list):
        visit(payload["exerciseDefinitions"], is_library_definition=True)
    else:
        visit(payload)

    entries: list[ExerciseEntry] = []
    seen: set[str] = set()
    for source_id, source_name, equipment_qualified_name, is_library_definition, context in raw_entries:
        name = equipment_qualified_name
        normalized = normalize_exercise_name(name)
        dedupe_key = (
            f"id:{str(source_id).strip().lower()}"
            if is_library_definition and source_id
...
def build_youtube_queries(exercise_name: str) -> list[str]:
    ...
    queries = [
        ...
        f"{base_term} three quarter view exercise{exclusions}",
...
def build_youtube_queries_with_contract_aliases(
    ...
    strict_transition_review = exercise_motion_contract_requires_strict_source_review(contract)
    if strict_transition_review:
        queries = [query for query in queries if "shorts" not in query.casefold()]
    ...
    contract_queries = [f"{base_term} {suffix}" for suffix in mode_suffixes]
    if strict_transition_review:
        strict_sequence_suffix = (
            "complete movement start to finish ending in the distinct end position"
            if completion_mode == "distinct_end_state"
            else "complete transition sequence start to finish and return to start"
        )
        contract_queries.insert(0, f"{base_term} {strict_sequence_suffix}")
        queries = [*contract_queries, *queries]
    else:
        queries.extend(contract_queries)
    for alias in aliases:
        ...
        queries.extend(
            [
                ...
                f"{alias_term} three quarter view exercise",
    ...
    for alias in aliases:
        ...
        queries.extend(
            [
                ...
                f"{alias_term} three quarter angle full rep",
...
def generic_youtube_query_aliases(exercise_name: str) -> list[str]:
    ...
    aliases.extend(equipment_preserving_youtube_query_aliases(exercise_name))
    return aliases


def equipment_preserving_youtube_query_aliases(exercise_name: str) -> list[str]:
    normalized = normalize_exercise_name(exercise_name)
    aliases: list[str] = []
    for equipment_prefix, equipment_aliases in YOUTUBE_QUERY_EQUIPMENT_SYNONYM_GROUPS:
        normalized_prefix = normalize_exercise_name(equipment_prefix)
        if not normalized.startswith(f"{normalized_prefix} "):
            continue
        action = normalized[len(normalized_prefix) + 1 :].strip()
        if not action:
            continue
        aliases.extend(f"{equipment_alias} {action.title()}" for equipment_alias in equipment_aliases)
    return aliases


def planned_query_preserves_target_action(exercise_name: str, query: str) -> bool:
    target_tokens = normalize_exercise_name(exercise_name).split()
    query_tokens = set(normalize_exercise_name(query).split())
    removable_prefix_tokens = {
        token
        for prefix in (*YOUTUBE_QUERY_EQUIPMENT_PREFIXES, *YOUTUBE_QUERY_LOAD_PREFIXES)
        for token in normalize_exercise_name(prefix).split()
    }
    for equipment_prefix, _aliases in YOUTUBE_QUERY_EQUIPMENT_SYNONYM_GROUPS:
        removable_prefix_tokens.update(normalize_exercise_name(equipment_prefix).split())
    action_tokens = [token for token in target_tokens if token not in removable_prefix_tokens]
    return not action_tokens or any(token in query_tokens for token in action_tokens)
...
class LlamaCppYouTubeQueryPlanner:
    ...
    def close(self, *, force_stop_server: bool = False) -> None:
        if self._owned_ranker is not None:
            self._owned_ranker.close(force_stop_server=force_stop_server)
...
def build_youtube_query_planner_prompt(
    *,
    ...
    return (
        ...
        "Before composing the response, identify the equipment phrase, the action phrase, and any distinctive variant "
        "term in the target. Replace a phrase only when you know an established domain synonym for it. Preserve an "
        "unfamiliar or specialized variant term across queries while varying known equipment or coaching-context terms. "
        "Never invent a title alias by converting a movement name into a literal anatomical description such as hip lift, "
        "arm movement, leg push, or standing pedal. It is valid to change only the equipment or domain wording when the "
        "action term is the distinctive searchable name.\n"
...
def search_youtube(
    ...
    *,
    timeout_seconds: float = DEFAULT_YOUTUBE_SEARCH_TIMEOUT_SECONDS,
    cookies_path: Path | None = None,
    cache_dir: Path | None = None,
    ...
    if cookies_path is not None:
        ...
        if not resolved_cookies_path.exists():
            raise FileNotFoundError(f"YouTube cookies file not found: {resolved_cookies_path}")
    fresh_cached_info = load_cached_youtube_search_info(
        cache_dir,
        query=query,
    ...
    with tempfile.TemporaryDirectory(prefix="exercise-motion-youtube-search-") as temporary_dir:
        command = [
            sys.executable,
            "-m",
            "yt_dlp",
        ...
        if resolved_cookies_path is not None:
            isolated_cookies_path = Path(temporary_dir) / "cookies.txt"
            shutil.copy2(resolved_cookies_path, isolated_cookies_path)
            command += ["--cookies", str(isolated_cookies_path)]
        command.append(f"ytsearch{max(1, results_per_query)}:{query}")
        for attempt in range(1, YOUTUBE_SEARCH_MAX_ATTEMPTS + 1):
            try:
                with InterProcessFileLock(
                    Path(tempfile.gettempdir()) / "myworkoutassistant-youtube-search.lock",
                    stage="youtube_search",
                    timeout_seconds=YOUTUBE_SEARCH_LOCK_TIMEOUT_SECONDS,
                    stale_after_seconds=YOUTUBE_SEARCH_LOCK_STALE_SECONDS,
                ):
                    completed = subprocess.run(
                        command,
                        check=False,
                ...
                raise YouTubeSearchError(
                    query,
                    "yt-dlp timed out while searching YouTube.",
                    transient=True,
                ) from exc
            if completed.returncode == 0:
                break
...
def youtube_search_retry_delay_seconds(attempt: int) -> float:
    exponential_delay = min(
        YOUTUBE_SEARCH_RETRY_MAX_DELAY_SECONDS,
        YOUTUBE_SEARCH_RETRY_BASE_DELAY_SECONDS * (2 ** max(0, attempt - 1)),
    )
    jitter = random.uniform(0.0, exponential_delay * YOUTUBE_SEARCH_RETRY_JITTER_RATIO)
    return exponential_delay + jitter


def youtube_search_cache_path(
    cache_dir: Path | None,
    *,
    query: str,
    results_per_query: int,
    ...
    cache_key = json.dumps(
        {
            ...
            "resultsPerQuery": max(1, int(results_per_query)),
        },
        ensure_ascii=False,
        sort_keys=True,
    )
    digest = hashlib.sha256(cache_key.encode("utf-8")).hexdigest()[:24]
    return cache_dir.expanduser().resolve() / "search-results" / f"{digest}.json"


def load_cached_youtube_search_info(
    cache_dir: Path | None,
    *,
    query: str,
    results_per_query: int,
    ...
    cache_path = youtube_search_cache_path(
        cache_dir,
        query=query,
        results_per_query=results_per_query,
    )
    ...
    if (
        ...
        or age_seconds > max(0.0, max_age_seconds)
...
def write_youtube_search_cache(
    ...
    *,
    ...
    if cache_path is None:
        return
    payload = {
        "schemaVersion": YOUTUBE_SEARCH_CACHE_SCHEMA_VERSION,
        "cachedAtEpochSeconds": time.time(),
        "query": normalize_search_query(query),
        "resultsPerQuery": max(1, int(results_per_query)),
        "info": info,
    }
...
def youtube_search_error_is_transient(error: str) -> bool:
    ...
    *,
    ...
    for item in valid_chunks:
        ...
        requested_duration = min(
            duration_seconds,
            max(end - start, float(min_window_seconds or 0.0)),
        )
        if requested_duration > end - start + 1e-6:
            center = (start + end) * 0.5
            start = max(0.0, min(duration_seconds - requested_duration, center - requested_duration * 0.5))
...
SEMANTIC_PRESENTATION_CONTEXT_TERMS = {
    "beginner",
    "beginners",
    "class",
    "demo",
...
def rank_youtube_review_pool(
    ...
    indexed_candidates = apply_source_outcome_priors(candidates, settings)
    return sorted(
        select_review_candidate_pool(indexed_candidates, settings),
        key=lambda candidate: (
            candidate_source_outcome_prior_score(candidate),
...
def candidate_source_outcome_prior_score(candidate: YouTubeCandidate) -> float:
    payload = candidate.vision_payload if isinstance(candidate.vision_payload, dict) else {}
    prior = payload.get("sourceOutcomePrior") if isinstance(payload, dict) else None
    score = coerce_float(prior.get("score")) if isinstance(prior, dict) else None
    return max(-1.0, min(1.0, score)) if score is not None else 0.0


def apply_source_outcome_priors(
    candidates: Iterable[YouTubeCandidate],
    settings: YouTubeRankingSettings,
) -> list[YouTubeCandidate]:
    candidates = list(candidates)
    if settings.source_outcome_index is None:
        return candidates
    index = load_source_outcome_index(settings.source_outcome_index)
    annotated: list[YouTubeCandidate] = []
    for candidate in candidates:
        prior = source_outcome_prior(
            index,
            video_id=candidate.video_id,
            url=candidate.url,
            channel=candidate.channel,
        ...
        payload["sourceOutcomePrior"] = prior
        reasons = list(candidate.score_reasons)
        if int(prior["sourceAttempts"]) > 0:
            reasons.append("source_outcome_video_prior")
        elif int(prior["channelAttempts"]) >= 3:
            reasons.append("source_outcome_channel_prior")
        annotated.append(
            replace_candidate(
                candidate,
...
VISION_HARD_GATE_REASONS = {
    ...
    "contract_start_state_match",
    "contract_end_state_match",
    "all_required_phases_visible",
}
...
def youtube_candidate_source_gate_reasons(candidate: YouTubeCandidate) -> list[str]:
    payload = candidate.vision_payload if isinstance(candidate.vision_payload, dict) else None
    if not isinstance(payload, dict):
        return []
    from exercise_motion_pkg.bake_and_rank import RankedCandidate, evaluate_source_candidate_gate

    gate = evaluate_source_candidate_gate(
        RankedCandidate(
            exercise_index=0,
            candidate_rank=0,
...
def rank_candidates_with_semantic_gate(
    *,
    ...
    if not ranked:
        return []
    review_eligible = prioritize_semantic_review_query_coverage([
        candidate
        for candidate in ranked
...
def apply_semantic_gate_score(
    ...
    *,
    ...
    blocking_model_variants = [
        variant
        for variant in model_unrequested_variants
        if normalize_exercise_name(variant) not in SEMANTIC_PRESENTATION_CONTEXT_TERMS
    ]
    exact_identity_match = (
        normalize_exercise_name(str(semantic_payload.get("matchedExercise") or ""))
        == normalize_exercise_name(exercise.name)
    )
    exact_identity_visual_fallback = bool(
        model_unrequested_variants
        and not blocking_model_variants
        and semantic_payload.get("wrongEquipment") is not True
        and exact_identity_match
    )
    omitted_limb_count_visual_fallback = semantic_omitted_limb_count_visual_fallback_applies(
        exercise.name,
        candidate.title,
        semantic_payload,
        blocking_model_variants,
    )
    omitted_equipment_visual_fallback = semantic_omitted_equipment_visual_fallback_applies(
        exercise.name,
        candidate.title,
        semantic_payload,
        blocking_model_variants,
    ...
    if blocking_model_variants:
        clamped_score = min(clamped_score, 0.20)
        semantic_reasons = dedupe_reasons([*semantic_reasons, *model_variant_reasons])
    elif exact_identity_visual_fallback or omitted_limb_count_visual_fallback or omitted_equipment_visual_fallback:
        clamped_score = max(clamped_score, settings.semantic_gate_min_score)
        if semantic_payload.get("wrongExercise") is True:
            semantic_payload["wrongExercise"] = False
            semantic_payload["identityContradictionRepaired"] = True
        semantic_reasons = dedupe_reasons(
            [
                *semantic_reasons,
                (
                    "semantic_omitted_limb_count_visual_fallback"
                    if omitted_limb_count_visual_fallback
                    else (
                        "semantic_omitted_equipment_visual_fallback"
                        if omitted_equipment_visual_fallback
                        else "semantic_identity_match_visual_fallback"
                    )
                ),
            ]
        )
    ...
    final_score = 0.0 if blocking_model_variants else clamped_score
...
def semantic_variant_term_is_movement_changing(term: str) -> bool:
    normalized = normalize_exercise_name(term)
    if not normalized or normalized in {"a", "an", "the"}:
        return False
    # A stated load does not define a different movement when equipment identity
    # is checked separately. Models commonly mislabel titles such as "25 lb
    # dumbbell" as exercise variants.
    without_load = re.sub(
        r"\b\d+(?:\.\d+)?\s*(?:kg|kgs|kilograms?|lb|lbs|pounds?|#)\b",
        "",
        normalized,
    ).strip()
...
def semantic_omitted_limb_count_visual_fallback_applies(
    ...
    blocking_model_variants: list[str],
) -> bool:
    """Let vision resolve an omitted unilateral label when base identity is exact."""
    if blocking_model_variants or semantic_payload.get("wrongEquipment") is True:
        return False
    target = normalize_exercise_name(exercise_name)
    candidate = normalize_exercise_name(candidate_title)
    omitted_qualifiers = [
        qualifier
        for qualifier in ("single arm", "single leg")
        if qualifier in target and qualifier not in candidate
    ]
    if not omitted_qualifiers:
        return False
    target_tokens = set(target.split())
    for qualifier in omitted_qualifiers:
        target_tokens.difference_update(qualifier.split())
    candidate_tokens = set(candidate.split())
    # Ignore presentation words, but require every remaining target identity
    # token (including equipment) to be stated by the candidate.
    candidate_tokens.difference_update({"a", "an", "the"})
    return bool(target_tokens) and target_tokens.issubset(candidate_tokens)


def semantic_omitted_equipment_visual_fallback_applies(
    exercise_name: str,
    candidate_title: str,
    semantic_payload: dict[str, Any],
    blocking_model_variants: list[str],
) -> bool:
    """Defer an unstated equipment prefix to vision when the base movement matches."""
    if blocking_model_variants or semantic_payload.get("wrongEquipment") is True:
        return False
    target = normalize_exercise_name(exercise_name)
    candidate_tokens = set(normalize_exercise_name(candidate_title).split())
    for equipment_prefix in (*YOUTUBE_QUERY_EQUIPMENT_PREFIXES, *YOUTUBE_QUERY_LOAD_PREFIXES):
        normalized_prefix = normalize_exercise_name(equipment_prefix)
        if not target.startswith(f"{normalized_prefix} "):
            continue
        base_movement_tokens = set(target[len(normalized_prefix) + 1 :].split())
        if base_movement_tokens and base_movement_tokens.issubset(candidate_tokens):
            return True
    return False
...
def build_exercise_motion_contract_prompt(exercise: ExerciseEntry) -> str:
    ...
    return (
        ...
        "A repeated exercise can still have a one-way target action: when the exercise-defining phase is an eccentric "
        "lowering, release, ballistic action, dismount, or other action normally followed by assistance or a non-target "
        "reset, choose distinct_end_state. Do not make that assistance or reset a required return phase merely because "
        "the athlete may repeat the exercise afterward. "
        "Decide this natural boundary before writing any phases: if athletes commonly catch themselves with the hands, "
        "step down, drop an implement, receive assistance, or otherwise reset after the target action, that reset is not "
        "proof of a return_to_start repetition. "
        ...
        "such as stable exercise posture, move away, turning point, perform the action, or return through the action.\n"
        ...
        "Describe the target-defining body motion with primaryMovingRegions, referenceRegions, primaryAxis, and motionPattern. "
        "Use the smallest region set that distinguishes the exercise: for example, a shrug is shoulder travel relative to the torso, "
        "not generic upper-limb motion. referenceRegions is the body region whose relationship to the moving region makes the action "
        ...
        "for return_to_start.\n"
        "startPoseConstraints and endPoseConstraints must each be objects with exactly supportMode, handHeight, "
        ...
        "upright merely because it is straight or aligned with a bench. Never describe a kneeling posture as standing. "
        "When shoulders, hips, and knees remain aligned while the body lowers, describe rotation about the knees rather "
        "than a hip hinge.\n"
        ...
        "referenceRegions uses the same region values. primaryAxis must be vertical, horizontal, depth, or any. "
        "motionPattern must be joint_travel, body_toward_anchor, body_away_from_anchor, limb_toward_body, "
        "limb_away_from_body, distal_limb_vertical, joint_flex_extend, or other. Do not use both primaryAxis any, "
        "motionPattern other, and an empty referenceRegions list for a moving exercise; that combination is not testable enough.\n"
...
def generate_exercise_motion_contract_with_ranker(
    *,
    ...
    for attempt_mode, disable_reasoning, use_repair_draft in attempt_specs:
        ...
        if use_repair_draft and repair_draft is not None:
            ...
            prompt += (
                ...
                "completionMode, requiresReturnToStart, end state, and required phases from normal exercise knowledge. "
                "Do not preserve a return phase when it is normally assistance or reset.\n"
                f"Validation issues: {json.dumps(repair_issues, ensure_ascii=False)}\n"
                "Previous draft: "
                f"{json.dumps(exercise_motion_contract_for_prompt(repair_draft), ensure_ascii=False)}\n"
                "Return one corrected minified JSON object using the original required keys."
            )
        caption_kwargs: dict[str, Any] = {
            "frame_paths": [],
            "prompt": prompt,
    ...
    return {
        ...
        "rejectedDrafts": rejected_drafts,
...
def shared_ranker_matches_settings(ranker: Any | None, settings: YouTubeRankingSettings) -> bool:
    ...
    if not isinstance(ranker_settings, YouTubeRankingSettings):
        return True
    if ranker_settings.llama_cpp_model != settings.llama_cpp_model:
        return False
    # A server with the vision projector loaded can also handle text-only
    # requests. Requiring an exact mmproj match made the text planner and
    # exercise-contract paths start a second server while the shared vision
    # server already owned the process-wide GPU lock.
    return (
        settings.llama_cpp_mmproj is None
        or ranker_settings.llama_cpp_mmproj == settings.llama_cpp_mmproj
    )
...
def normalize_exercise_motion_contract(
    ...
    *,
    ...
    contract["contractPolicyVersion"] = EXERCISE_MOTION_CONTRACT_POLICY_VERSION
    return contract
...
def normalize_implement_support_mode(value: Any) -> str | None:
    text = re.sub(r"[^a-z0-9]+", "_", str(value or "").strip().casefold()).strip("_")
    aliases = {
        "hands": "hands_only",
        "hand_supported": "hands_only",
        "rigid_two_hand": "hands_only",
        "body": "body_supported",
        "rests_on_body": "body_supported",
        "body_and_hands": "mixed",
        "hands_and_body": "mixed",
        "machine": "external",
        "rack": "external",
        "not_applicable": "none",
    }
    text = aliases.get(text, text)
    return text if text in {"hands_only", "body_supported", "mixed", "external", "none"} else None
...
EXERCISE_POSE_CONSTRAINT_VALUES = {
...
def normalize_exercise_pose_constraints(value: Any) -> dict[str, str] | None:
    if not isinstance(value, dict):
        return None
    normalized: dict[str, str] = {}
    for key, allowed in EXERCISE_POSE_CONSTRAINT_VALUES.items():
        raw = re.sub(r"[^a-z0-9]+", "_", str(value.get(key) or "").strip().casefold()).strip("_")
        aliases = {
            "shoulder_width_stance": "shoulder_width",
            "shoulder_or_chest": "shoulder_chest",
            "chest": "shoulder_chest",
            "hips": "hip",
            "below_hip": "below_hips",
            "deep": "deep_flexion",
            "bent": "flexed",
            "straight": "extended",
        }
        normalized_value = aliases.get(raw, raw)
        normalized[key] = normalized_value if normalized_value in allowed else "any"
    if (
        normalized.get("supportMode") == "lying"
...
def normalized_exercise_motion_contract_fields(payload: dict[str, Any]) -> dict[str, Any]:
    fields: dict[str, Any] = {}
    motion_context = payload.get("motionContext")
    if isinstance(motion_context, dict) and motion_context:
        # Runtime library context (especially the concrete equipment type) is
        # deterministic input, not model-authored guidance. Preserve it while
        # normalizing so downstream reconstruction gates can enforce constraints
    ...
    implement_support_mode = normalize_implement_support_mode(
        first_contract_value(payload, "implementSupportMode", "implement_support_mode")
    )
    ...
    requires_return = parse_contract_bool(
        first_contract_value(payload, "requiresReturnToStart", "requires_return_to_start")
    )
    completion_mode = explicit_completion_mode
    if completion_mode is None:
        ...
        # The completion mode describes the observable boundary of the movement
        # and is therefore authoritative when a model emits contradictory flags.
        # Only return_to_start requires the final posture to match the start.
        requires_return = completion_mode == "return_to_start"
    fields["requiresReturnToStart"] = requires_return
    fields["completionMode"] = completion_mode

    for output_key, aliases in {
        "startPoseConstraints": ("startPoseConstraints", "start_pose_constraints"),
        "endPoseConstraints": ("endPoseConstraints", "end_pose_constraints"),
    }.items():
    ...
    if requires_return is True and required_phases and not any(
        ...
        # The final observable phase already ends at validStartState; label that
        # existing phase explicitly instead of inventing an additional action.
        required_phases[-1] = f"Return to start: {required_phases[-1]}"[:180].rstrip()
    if required_phases:
        fields["requiredPhases"] = required_phases
    ...
    if observable_spec is not None:
        observable_spec = dict(observable_spec)
        observable_spec["requiresReturnToStart"] = requires_return
        observable_spec["oneWayPartialIsInvalid"] = requires_return
        observable_spec["mustShowFullCycle"] = requires_return
...
def cache_exercise_motion_contract(
    exercise: ExerciseEntry,
    settings: YouTubeRankingSettings,
...
def prefetch_exercise_motion_contracts(
    *,
    ...
    ranker_factory: Callable[[YouTubeRankingSettings], Any] | None = None,
) -> dict[str, Any]:
    """Fill the reusable motion-contract cache with one persistent llama.cpp server."""
    started = time.monotonic()
    selected_ids = {value.strip() for value in (exercise_ids or set()) if value.strip()}
    exercises = [
        exercise
        for exercise in load_workout_plan_exercises(
            workout_plan_json,
            include_disabled=settings.include_disabled,
            equipment_path=equipment_json,
        )
        if not selected_ids or exercise.exercise_id in selected_ids
    ]
    results_by_id: dict[str, dict[str, Any]] = {}
    missing: list[ExerciseEntry] = []
    completed = 0
    total = len(exercises)
    for exercise in exercises:
        cached = load_cached_exercise_motion_contract(exercise, settings)
        if cached is None:
        ...
        result = {
            ...
            "exerciseName": exercise.name,
            "status": "reused",
            "cachePath": cached.get("cachePath"),
            "generationElapsedSeconds": cached.get("generationElapsedSeconds"),
        }
        results_by_id[exercise.exercise_id] = result
        completed += 1
        if progress_callback is not None:
            progress_callback(completed, total, result)

    if missing:
        contract_settings = dataclass_replace(
            exercise_contract_llama_cpp_settings(settings),
            keep_llama_cpp_server=False,
        )
        active_workers = max(
            1,
            min(
                int(workers or contract_settings.llama_cpp_parallel or 1),
                len(missing),
            ),
        )
        factory = ranker_factory or LlamaCppVisionRanker
        ranker: Any | None = None
        try:
            ranker = factory(contract_settings)

            def generate(exercise: ExerciseEntry) -> dict[str, Any]:
                contract = generate_exercise_motion_contract_with_ranker(
                    exercise=exercise,
                    settings=contract_settings,
                    ranker=ranker,
                )
                cache_path = cache_exercise_motion_contract(exercise, settings, contract)
                return {
                    "exerciseId": exercise.exercise_id,
                    "exerciseName": exercise.name,
                    ...
                    "rejectedDrafts": contract.get("rejectedDrafts") if cache_path is None else None,
                }

            with ThreadPoolExecutor(max_workers=active_workers) as executor:
                futures = {
                    executor.submit(generate, exercise): exercise
                ...
                for future in as_completed(futures):
                    ...
                    try:
                        ...
                        result = {
                            ...
                            "cachePath": None,
                            "error": str(exc),
                            "vlmError": vlm_error_payload(exc),
                        }
                    results_by_id[exercise.exercise_id] = result
                    completed += 1
                    if progress_callback is not None:
            ...
            for exercise in missing:
                if exercise.exercise_id in results_by_id:
                    continue
                result = {
                    "exerciseId": exercise.exercise_id,
                    "exerciseName": exercise.name,
    ...
    ordered_results = [
        results_by_id[exercise.exercise_id]
        for exercise in exercises
    ]
    counts = {
        status: sum(1 for result in ordered_results if result.get("status") == status)
        for status in ("generated", "reused", "failed")
    }
    report = {
        "schemaVersion": 1,
        "generatedAt": datetime.now(timezone.utc).isoformat(),
        "sourceWorkoutPlanPath": str(workout_plan_json),
        ...
        "workerCount": max(1, int(workers or settings.llama_cpp_parallel or 1)),
        "totalElapsedSeconds": round_elapsed(time.monotonic() - started),
        "counts": counts,
        "exercises": ordered_results,
    }
    out_json.parent.mkdir(parents=True, exist_ok=True)
    temporary_path = out_json.with_name(f"{out_json.name}.{os.getpid()}.tmp")
...
def cleaned_contract_string(value: Any, limit: int) -> str:
    text = readable_contract_value(value).strip()
    return truncate_text(text, limit) or ""


def readable_contract_value(value: Any) -> str:
    if isinstance(value, dict):
        parts = []
        for key, nested_value in value.items():
            nested_text = readable_contract_value(nested_value).strip()
            if not nested_text:
                continue
    ...
    if isinstance(value, (list, tuple)):
        return "; ".join(
            text for item in value if (text := readable_contract_value(item).strip())
        )
...
def normalize_required_phase_labels(value: Any) -> list[str]:
    """Recover concise phase labels from common small-model schema leakage."""
    normalized: list[str] = []
    for raw_phase in cleaned_contract_string_list(value, limit=8, item_limit=360):
        phase = raw_phase.strip()
        description_match = re.search(
            r"(?:^|\b)description\s*:\s*(.+)$", phase, flags=re.IGNORECASE
        )
        if description_match:
            phase = description_match.group(1).strip()
        phase = re.sub(
            r"^(?:phase\s*)?(?:name|label)\s*:\s*", "", phase, flags=re.IGNORECASE
        ).strip()
        phase = phase.rstrip(".\u2026 ")
        if not phase or "..." in phase or "\u2026" in phase:
            continue
        if phase.casefold() not in {item.casefold() for item in normalized}:
            normalized.append(phase[:180].rstrip())
    return normalized


GENERIC_EXERCISE_MOTION_CONTRACT_PHRASES = (
    "a stable exercise posture immediately before one complete repetition",
...
ANATOMICALLY_INVALID_CONTRACT_PHRASES = (
    "standing on hands",
    "standing on knees",
)
...
def exercise_motion_contract_quality_issues(
    ...
    topology_text = json.dumps(topology, ensure_ascii=False) if isinstance(topology, dict) else ""
    contract_text = " ".join(
        (
            str(contract.get("advisoryText") or ""),
            str(contract.get("validStartState") or ""),
            str(contract.get("validEndState") or ""),
            topology_text,
        )
    ).casefold()
    topology_start_state = topology.get("startState") if isinstance(topology, dict) else None
    topology_end_state = topology.get("endState") if isinstance(topology, dict) else None
    posture_state_texts = [
        str(contract.get("validStartState") or "").casefold(),
        str(contract.get("validEndState") or "").casefold(),
        str(topology_start_state.get("label") or "").casefold()
        if isinstance(topology_start_state, dict)
        else "",
        str(topology_end_state.get("label") or "").casefold()
        if isinstance(topology_end_state, dict)
        else "",
    ]
    ...
    if all(
        re.search(pattern, posture_text)
        for pattern in (
            r"support\s*mode\s*:",
            r"hand\s*height\s*:",
            r"torso\s*orientation\s*:",
            r"knee\s*state\s*:",
        )
    ):
        issues.append("valid posture state repeats schema field labels instead of describing anatomy")
    if any(
        "supported by hands on floor" in state_text and "torso upright" in state_text
        for state_text in posture_state_texts
    ):
        issues.append("floor hand support contradicts an upright torso posture")
    if any(
        re.search(r"\bstanding\b", state_text)
        and re.search(r"\bkneel(?:ing|s|ed)?\b", state_text)
        for state_text in posture_state_texts
    ):
        issues.append("posture is described as both standing and kneeling")
    if "hinge at the hips" in contract_text and re.search(
        r"straight line.{0,80}(?:shoulders?.{0,40}(?:hips?.{0,40})?knees?|knees?.{0,40}(?:hips?.{0,40})?shoulders?)",
        contract_text,
    ):
        issues.append("hip hinge contradicts a straight shoulder-to-knee body line")
    completion_mode = normalize_exercise_completion_mode(contract.get("completionMode"))
    phase_labels = cleaned_contract_string_list(
        contract.get("requiredPhases"),
        limit=8,
        item_limit=240,
    )
    phase_text = " ".join(phase_labels).casefold()
    if any(
        re.search(r"(?:^|\b)(?:phase\s*name|description)\s*:", phase.casefold())
        or "..." in phase
        or "…" in phase
        for phase in phase_labels
    ):
        issues.append("required phase contains schema labels or truncated text")
    if completion_mode == "return_to_start" and not re.search(
        r"\b(?:return|returns|returned|back)\b",
        phase_text,
    ):
        issues.append("return_to_start contract has no explicit return-to-start phase")
    if completion_mode == "distinct_end_state":
        start_state = normalize_exercise_name(str(contract.get("validStartState") or ""))
        end_state = normalize_exercise_name(str(contract.get("validEndState") or ""))
        if start_state and start_state == end_state:
            issues.append("distinct_end_state contract has identical start and end states")
        if re.search(r"\b(return|returns|returned|back)\b.{0,60}\b(start|starting|initial)\b", phase_text):
            issues.append("distinct_end_state contract includes a return-to-start phase")
    return issues


def exercise_motion_contract_has_specific_topology(contract: dict[str, Any] | None) -> bool:
    if not isinstance(contract, dict):
        return False
    if int(contract.get("contractPolicyVersion") or 0) < EXERCISE_MOTION_CONTRACT_POLICY_VERSION:
        return False
    if contract.get("contractSimplification") == "generic_observable_return_cycle":
        return False
    if exercise_motion_contract_quality_issues(contract):
        return False
    for key in ("startPoseConstraints", "endPoseConstraints"):
        constraints = contract.get(key)
        if not isinstance(constraints, dict):
            return False
        if set(constraints) != set(EXERCISE_POSE_CONSTRAINT_VALUES):
            return False
        if any(
            str(constraints.get(field) or "") not in allowed
            for field, allowed in EXERCISE_POSE_CONSTRAINT_VALUES.items()
        ):
            return False
    topology = contract.get("movementTopology")
    ...
    if not isinstance(start_state, dict) or not isinstance(end_state, dict) or not isinstance(phases, list):
        return False
    labels = [
        str(start_state.get("label") or "").strip(),
        *[
            str(phase.get("label") or "").strip()
            for phase in phases
            if isinstance(phase, dict)
        ],
        str(end_state.get("label") or "").strip(),
    ]
    if len(labels) < 3 or any(not label for label in labels):
        return False
    normalized_labels = "\n".join(labels).casefold()
    if any(
        phrase in normalized_labels
        for phrase in GENERIC_EXERCISE_MOTION_CONTRACT_PHRASES
    ):
        return False
    movement_type = str(contract.get("movementType") or "").strip()
    if movement_type == "hold":
        return True
    observable_spec = normalize_observable_motion_spec(contract.get("observableMotionSpec"))
    if observable_spec is None or not observable_spec.get("primaryMovingRegions"):
        return False
    return not (
        observable_spec.get("primaryAxis") == "any"
        and observable_spec.get("motionPattern") == "other"
        and not observable_spec.get("referenceRegions")
    )
...
def build_candidate_semantic_gate_prompt(exercise: ExerciseEntry, candidate: YouTubeCandidate) -> str:
    ...
    return (
        ...
        "position, or support, keep wrongExercise=false and wrongEquipment=false so the visual gate can confirm the omitted detail. "
        "Set wrongEquipment=true only for explicitly conflicting equipment, never merely because the target equipment noun is absent. "
    ...
    *,
    ...
    if settings.semantic_gate_enabled:
        ...
        try:
            ...
            if isinstance(semantic_gate, LlamaCppSemanticGate):
                semantic_gate.close(force_stop_server=True)
            semantic_elapsed = time.monotonic() - semantic_gate_started

    if settings.pose_prefilter_enabled:
        if isinstance(vision_ranker, LlamaCppVisionRanker):
            raise RuntimeError(
                "YOLO pose filtering cannot run while a shared llama.cpp ranker owns the GPU. "
                "Prepare pose-filtered candidates before passing a shared vision ranker."
            )
    ...
    if settings.rank_with_vision:
        ...
        if not vision_candidates:
            reviewed = demote_candidates_failing_source_gate(
                sort_youtube_reviewed_candidates(reviewed, settings)
            )
            return CandidateReviewPassResult(
                ...
                vision_elapsed_seconds=time.monotonic() - vision_started,
            )
        ...
        try:
            ...
            if owned_vision_ranker is not None:
                owned_vision_ranker.close(force_stop_server=True)
        ...
        reviewed = sort_youtube_reviewed_candidates(reviewed, settings)
    reviewed = demote_candidates_failing_source_gate(reviewed)
...
def candidate_review_frame_paths(candidate: YouTubeCandidate) -> list[Path]:
    ...
    if not isinstance(reviewed_chunks, list):
        return []
    ordered_chunks = sorted(
        (chunk for chunk in reviewed_chunks if isinstance(chunk, dict)),
        key=lambda chunk: float(chunk.get("score") or 0.0),
        reverse=True,
    )
    for chunk in ordered_chunks:
        debug = chunk.get("debugArtifacts")
        debug_path = Path(str(debug.get("debugPath") or "")) if isinstance(debug, dict) else None
        ...
        paths = [
            Path(str(value))
            for value in debug_payload.get("framePaths", [])
...
def build_source_observed_boundary_prompt(exercise: ExerciseEntry, candidate: YouTubeCandidate) -> str:
    return (
        "Classify the natural visible boundary of the exact target exercise from these ordered source frames. "
        "Ignore any previous movement contract. Use exercise knowledge only to interpret the visible human motion; "
        "do not name equipment or describe anatomy. Distinguish the target action from assistance or reset after it.\n"
        f"Target exercise: {exercise.name}.\n"
        f"Video title: {candidate.title}.\n"
        "naturalBoundary must be exactly one_way_then_hand_assisted_reset, unassisted_return_to_start, or unclear. "
        "Return JSON only with exactly these keys: exactExerciseVisible (boolean), targetActionContinuous (boolean), "
        "normalSpeed (boolean), cleanTargetIntervalVisible (boolean), naturalBoundary (string), "
        "handAssistedResetVisible (boolean)."
    )


def source_observed_completion_boundary(
    *,
    exercise: ExerciseEntry,
    candidate: YouTubeCandidate,
    settings: YouTubeRankingSettings,
    vision_ranker: "LlamaCppVisionRanker",
) -> dict[str, Any] | None:
    from exercise_motion_pkg.segment_detection import extract_json_object

    frame_paths = candidate_review_frame_paths(candidate)
    if not frame_paths:
        return None
    raw = vision_ranker.client.caption_images(
        frame_paths=frame_paths,
        prompt=build_source_observed_boundary_prompt(exercise, candidate),
        max_tokens=512,
    ...
    if not isinstance(payload, dict) or any(
        ...
        for key in (
            "exactExerciseVisible",
            "targetActionContinuous",
            "normalSpeed",
            "cleanTargetIntervalVisible",
        )
    ):
        return None
    if (
        str(payload.get("naturalBoundary") or "").strip() != "one_way_then_hand_assisted_reset"
        or payload.get("handAssistedResetVisible") is not True
    ):
        return None
    return {
        "naturalBoundary": "one_way_then_hand_assisted_reset",
        "handAssistedResetVisible": True,
        "resetOrAssistanceAfterTarget": "post-target hand-assisted reset",
...
def run_youtube_candidate_review_batches(
    *,
    ...
    for batch_index, start in enumerate(range(0, len(bounded_ranked), batch_size), start=1):
        ...
        # The configured value is a portfolio target, not merely a ceiling.
        # A high-readiness source can lead the portfolio, but it cannot stand
        # in for the distinct fallback source required when reconstruction or
        # final validation rejects that leader.
        target_suitable_count = configured_target_suitable_count
        batch_payload = {
    ...
    ranked_reviewed = demote_candidates_failing_source_gate(
        demote_candidates_missing_required_review(
...
def youtube_candidate_search_should_expand(
    *,
    ...
    """Widen search while the reviewed source portfolio is below its target."""
    return (
        suitable_count < settings.resolved_candidate_review_target_suitable_count()
    ...
    *,
    ...
    for query in queries:
        ...
        try:
            ...
            for result_rank, candidate in enumerate(search_results, start=1):
                ...
                if youtube_candidate_is_excluded(candidate, excluded_keys):
                    ...
                    continue
                key = candidate.key()
                existing_candidate = by_key.get(key)
                existing_payload = (
                    dict(existing_candidate.vision_payload or {})
                    if existing_candidate is not None
                    else dict(candidate.vision_payload or {})
                )
                discovery = dict(existing_payload.get("searchDiscovery") or {})
                matched_queries = dedupe_reasons(
                    [*list(discovery.get("queries") or []), query]
                )
                phases = dedupe_reasons(
                    [*list(discovery.get("phases") or []), phase]
                )
                previous_rank = as_optional_int(discovery.get("bestResultRank"))
                discovery.update(
                    {
                        ...
                        "bestResultRank": (
                            ...
                            else min(previous_rank, result_rank)
                        ),
                    }
                )
                existing_payload["searchDiscovery"] = discovery
                if existing_candidate is None:
                    by_key[key] = replace_candidate(
                        candidate,
                        vision_payload=existing_payload,
                    )
                    new_for_query += 1
                else:
                    by_key[key] = replace_candidate(
            ...
            elapsed_total += time.monotonic() - search_started
            if not allow_transient_search_errors or not exc.transient:
                raise
...
def prioritize_semantic_review_query_coverage(
    ranked: list[YouTubeCandidate],
) -> list[YouTubeCandidate]:
    """Interleave global text rank with the best result from each search route."""
    if len(ranked) < 2:
        return list(ranked)
    global_index = {candidate.key(): index for index, candidate in enumerate(ranked)}
    best_by_query: dict[str, tuple[int, int, YouTubeCandidate]] = {}
    for candidate in ranked:
        payload = candidate.vision_payload if isinstance(candidate.vision_payload, dict) else {}
        discovery = payload.get("searchDiscovery")
        if not isinstance(discovery, dict):
            continue
        result_rank = as_optional_int(discovery.get("bestResultRank"))
        if result_rank is None:
            continue
        for query in discovery.get("queries") or []:
            query_key = normalize_search_query(str(query)).casefold()
            if not query_key:
                continue
            candidate_key = (result_rank, global_index[candidate.key()], candidate)
            current = best_by_query.get(query_key)
            if current is None or candidate_key[:2] < current[:2]:
                best_by_query[query_key] = candidate_key
    route_heads: list[YouTubeCandidate] = []
    route_head_keys: set[str] = set()
    for _rank, _global_index, candidate in best_by_query.values():
        if candidate.key() in route_head_keys:
            continue
        route_head_keys.add(candidate.key())
        route_heads.append(candidate)
    prioritized: list[YouTubeCandidate] = []
    seen: set[str] = set()
    for source in (route_heads, ranked):
...
def raise_if_all_youtube_searches_failed(
    search_result: YouTubeSearchPassResult,
    *,
    previously_successful_queries: Iterable[str] = (),
) -> None:
    if (
        search_result.by_key
        or search_result.search_attempts
        or any(str(query).strip() for query in previously_successful_queries)
        or not search_result.search_errors
    ):
        return
    first_error = search_result.search_errors[0]
    raise YouTubeSearchError(
        str(first_error.get("query") or ""),
        str(first_error.get("error") or "All YouTube searches failed."),
        transient=bool(first_error.get("transient")),
    )


def prefetch_youtube_candidate_previews(
    *,
    workout_plan_json: Path,
    ...
    preview_prefetcher: Callable[[list[YouTubeCandidate], YouTubeRankingSettings], None] = prefetch_youtube_previews_parallel,
) -> dict[str, Any]:
    """Prepare network-bound YouTube inputs without starting any GPU model."""
    ...
    for exercise in exercises:
        cached_motion_contract = load_cached_exercise_motion_contract(exercise, settings)
        queries = (
        ...
        prepared = [
            ...
            for candidate in search_result.by_key.values()
        ]
        ranked = rank_youtube_review_pool(exercise, prepared, settings)
        prefetch_limit = max(
        ...
        exercise_payloads.append(
            {
                ...
                "slug": exercise.slug,
                # Discovery skips only queries that prefetch actually completed.
                # Timed-out queries remain eligible for the normal discovery retry.
                "queries": [
                    str(attempt["query"])
                ...
                # Keep the complete searched inventory for discovery. Only the
                # bounded leading subset is downloaded here; marking a query as
                # prefetched must not discard its un-downloaded alternatives.
    ...
    manifest = {
        ...
        "sourcePlanSha256": hashlib.sha256(workout_plan_json.read_bytes()).hexdigest(),
        "equipmentJsonPath": str(equipment_json) if equipment_json is not None else None,
        "equipmentSha256": (
            hashlib.sha256(equipment_json.read_bytes()).hexdigest()
...
def load_youtube_candidate_prefetch(
    path: Path | None,
    *,
    source_plan_path: Path,
) -> dict[str, tuple[list[str], dict[str, YouTubeCandidate]]]:
    if path is None or not path.exists():
        return {}
    payload = json.loads(path.read_text(encoding="utf-8-sig"))
    expected_hash = hashlib.sha256(source_plan_path.read_bytes()).hexdigest()
    if payload.get("kind") != "youtube_candidate_prefetch" or payload.get("sourcePlanSha256") != expected_hash:
        return {}
    result: dict[str, tuple[list[str], dict[str, YouTubeCandidate]]] = {}
    for exercise in payload.get("exercises") or []:
        if not isinstance(exercise, dict):
            continue
        exercise_id = str(exercise.get("exerciseId") or "").strip()
        if not exercise_id:
            continue
        candidates: dict[str, YouTubeCandidate] = {}
        for item in exercise.get("candidates") or []:
            ...
            candidate = YouTubeCandidate(
                ...
                vision_score=coerce_float(item.get("visionScore")),
                final_score=coerce_float(item.get("finalScore")) or 0.0,
                status=str(item.get("status") or "candidate"),
                score_reasons=[str(reason) for reason in item.get("scoreReasons") or []],
                vision_payload=(
                    dict(item["visionPayload"])
                    if isinstance(item.get("visionPayload"), dict)
                    else None
                ),
            )
            candidates[candidate.key()] = candidate
        result[exercise_id] = (
            [str(query) for query in exercise.get("queries") or [] if str(query).strip()],
            candidates,
        )
    return result
...
def discover_and_rank_youtube_candidates(
    *,
    ...
    out_json: Path,
    settings: YouTubeRankingSettings,
    prefetched_candidates_json: Path | None = None,
    search_fn: SearchFn | None = None,
    ...
    prefetched_by_exercise_id = load_youtube_candidate_prefetch(
        prefetched_candidates_json,
        source_plan_path=workout_plan_json,
    )
    vision_enabled = settings.rank_with_vision
    owns_vision_ranker = False
    if (
        settings.pose_prefilter_enabled
        and vision_ranker is None
        and settings.llama_cpp_auto_start_server
        and settings.llama_cpp_base_url is not None
    ):
        # A previous discovery process may have kept its local server alive
        # after releasing the file lock. Clear that stale GPU owner once at
        # startup; every ranker created below is force-stopped at its phase
        # boundary before YOLO can run.
        stop_llama_cpp_servers_for_base_url(
            settings.llama_cpp_base_url,
            expected_command=settings.llama_cpp_server_command,
            expected_model=settings.llama_cpp_model,
        )
    ...
    try:
        for source_exercise in exercises:
            ...
            owns_exercise_name_rewrite_ranker = False
            if (
                exercise_name_rewriter is None
                and settings.exercise_name_rewrite_enabled
                and settings.llama_cpp_base_url is not None
            ):
                rewrite_settings = text_llama_cpp_settings(settings)
                if (
                    isinstance(vision_ranker, LlamaCppVisionRanker)
                    and shared_ranker_matches_settings(vision_ranker, rewrite_settings)
                ):
                    exercise_name_rewrite_ranker = vision_ranker
                else:
                    exercise_name_rewrite_ranker = LlamaCppVisionRanker(rewrite_settings)
                    owns_exercise_name_rewrite_ranker = True
            if exercise_name_rewriter is not None:
                try:
                    ...
                    if owns_exercise_name_rewrite_ranker and exercise_name_rewrite_ranker is not None:
                        exercise_name_rewrite_ranker.close(force_stop_server=True)
            else:
                try:
            ...
            if settings.exercise_motion_contract_enabled and vision_enabled:
                ...
                if exercise_motion_contract_provider is not None:
                    ...
                    if exercise_motion_contract is None and settings.llama_cpp_base_url is not None:
                        ...
                        if (
                            isinstance(vision_ranker, LlamaCppVisionRanker)
                            and shared_ranker_matches_settings(vision_ranker, contract_settings)
                        ):
                            contract_ranker = vision_ranker
                        else:
                            contract_ranker = LlamaCppVisionRanker(contract_settings)
                            owns_contract_ranker = True
                        try:
                            try:
                            ...
                            except CriticalVlmInteractionError as exc:
                                # The movement contract improves discovery, but it is not
                                # required to search for and visually validate a source.
                                # A text-only contract timeout must not abort the whole
                                # exercise before any source candidate is considered.
                                exercise_motion_contract = {
                                    "schemaVersion": 1,
                                    "enabled": True,
                                    "status": "failed",
                            ...
                            if owns_contract_ranker:
                                contract_ranker.close(force_stop_server=True)
                        cache_path = cache_exercise_motion_contract(
                            exercise,
                            settings,
                            exercise_motion_contract,
                        ...
                        if cache_path is not None:
                            exercise_motion_contract["cacheStatus"] = "generated"
                            exercise_motion_contract["cachePath"] = str(cache_path)
                else:
                    exercise_motion_contract = {
                        "schemaVersion": 1,
            ...
            if query_planner is not None and not settings.single_exercise_name_query:
                try:
                    ...
                    added_queries = [
                        ...
                        and planned_query_preserves_target_action(exercise.name, query)
                    ]
                    ...
                    if owns_query_planner and isinstance(query_planner, LlamaCppYouTubeQueryPlanner):
                        query_planner.close(force_stop_server=True)
            append_youtube_discovery_progress(
                progress_path,
            ...
            prefetched_queries, prefetched_candidates = prefetched_by_exercise_id.get(
            ...
            raise_if_all_youtube_searches_failed(
                search_result,
                previously_successful_queries=prefetched_queries,
            )
            ...
            by_key.update(debug_candidates_by_key)
            ...
            if (
                ...
                append_youtube_discovery_progress(
                    ...
                    started_at=run_started,
                    exercise=exercise,
                    reason="insufficient_suitable_candidates_after_initial_review",
                    initialSuitableCandidateCount=initial_suitable_count,
                    reviewCandidateCount=len(review_pool_ranked),
                )
            ...
            if (review_budget is None or not review_budget.exhausted) and youtube_candidate_search_should_expand(
                ...
                search_expansion_results_per_query = min(
                    YOUTUBE_FAILURE_SEARCH_EXPANSION_MAX_RESULTS_PER_QUERY,
                    settings.results_per_query + max(1, settings.max_candidates),
                )
                search_expansion_query_list = merge_youtube_queries(
                    [*queries[:1], *search_expansion_queries],
                    limit=1,
                )
                ...
                pre_expansion_candidate_keys = set(by_key)
                ...
                if expanded_search_result.new_candidate_count > 0:
                    ...
                    prepared_ranked = [
                        ...
                        if key not in pre_expansion_candidate_keys
                    ]
                    ...
                    candidate_expansion_payload.update(
                        {
                            ...
                            or "insufficient_suitable_candidates_after_initial_review",
                            "searchExpansionTriggered": True,
                            "searchExpansionReason": "insufficient_suitable_candidates_after_expanded_review",
        ...
        if owns_query_planner and isinstance(query_planner, (DeepSeekYouTubeQueryPlanner, LlamaCppYouTubeQueryPlanner)):
            if isinstance(query_planner, LlamaCppYouTubeQueryPlanner):
                query_planner.close(force_stop_server=True)
            else:
                query_planner.close()
        if owns_semantic_gate and semantic_gate_ranker is not None:
            semantic_gate_ranker.close(force_stop_server=True)
        if owns_vision_ranker and isinstance(vision_ranker, LlamaCppVisionRanker):
            vision_ranker.close(force_stop_server=True)
...
def rank_candidates_with_prepared_vision_reviews(
    *,
    ...
    if settings.vision_llm_workers > 1:
        ...
        # The suitable-result target controls when to stop scheduling new work;
        # it must not reduce candidate-level concurrency.  In the common
        # first-pass target=1 case that coupling left five of six llama.cpp
        # slots idle and serialized every rejected candidate.
        parallel_batch_size = max(1, settings.vision_llm_workers)
            ...
            try:
                ...
                while index < batch_end:
                    ...
                    if candidate.key() not in prepared_by_key:
                        ...
                        # The wave already reached its suitable-source target and
                        # cooperatively stopped this candidate. Preserve it as
                        # unreviewed so a deeper retry may still use it.
                        reviewed = candidate
                    else:
                    ...
                    index += 1
                    if candidate_passes_vision_hard_gates(reviewed, settings):
                        passing_candidates_in_wave += 1
                passing_candidate_count += passing_candidates_in_wave
                if passing_candidate_count >= settings.resolved_candidate_review_target_suitable_count():
                    reranked.extend(ranked[index:])
                    return reranked
            finally:
...
class PreparedVisionReviewCancelled(RuntimeError):
    """An in-flight candidate stopped after another candidate satisfied the wave."""
...
def score_prepared_vision_reviews_parallel(
    *,
    ...
    suitable_result_count = 0
    critical_failures: list[tuple[PreparedVisionReview, BaseException]] = []

    def rank_prepared(prepared: PreparedVisionReview) -> VisionRankResult:
        if callable_accepts_keyword(vision_ranker.rank_prepared, "stop_event"):
            return vision_ranker.rank_prepared(
                prepared,
                settings,
                stop_event=stop_event,
            )
        return vision_ranker.rank_prepared(prepared, settings)

    with ThreadPoolExecutor(max_workers=workers) as executor:
        futures = {
            executor.submit(rank_prepared, prepared): prepared
        ...
        for future in as_completed(futures):
            ...
            try:
                result = future.result()
                results[prepared.candidate.key()] = result
                vision_score, vision_reasons, vision_payload = normalize_vision_result(result)
                reviewed = apply_vision_score(
                    prepared.candidate,
                    vision_score,
                    vision_reasons,
                    vision_payload,
                    settings=settings,
                    exercise_motion_contract=prepared.exercise_motion_contract,
                )
                if candidate_passes_vision_hard_gates(reviewed, settings):
                    suitable_result_count += 1
                    if suitable_result_count >= target_suitable_count and not stop_event.is_set():
                        stop_event.set()
                        for pending_future in futures:
                            if pending_future is not future:
                                pending_future.cancel()
            except PreparedVisionReviewCancelled:
                continue
            except Exception as exc:
                if is_critical_vlm_interaction_error(exc):
                    ...
                    critical_failures.append((prepared, exc))
                    continue
                results[prepared.candidate.key()] = (0.0, ["vision_review_failed"])
    if not results and critical_failures:
        # Each request already performs one bounded server recovery and retries
        # itself. If the whole wave still failed, surface the infrastructure
        # failure instead of misclassifying every source as unsuitable.
        raise critical_failures[0][1]
    return results
...
def exercise_motion_contract_requires_strict_source_review(
    contract: dict[str, Any] | None,
) -> bool:
    """Prevent identity-only fallback for movements defined as ordered transitions."""
    if not isinstance(contract, dict):
        return False
    completion_mode = str(contract.get("completionMode") or "").strip().lower()
    if completion_mode == "distinct_end_state":
        return True
    movement_type = str(contract.get("movementType") or "").strip().lower()
    if movement_type == "transition_sequence":
        return True
    rejected_drafts = contract.get("rejectedDrafts")
    if not isinstance(rejected_drafts, list):
        return False
    return any(
        isinstance(draft, dict)
        and isinstance(draft.get("contract"), dict)
        and (
            str(draft["contract"].get("movementType") or "").strip().lower()
            == "transition_sequence"
            or str(draft["contract"].get("completionMode") or "").strip().lower()
            == "distinct_end_state"
        )
    ...
    try:
        ...
        review_windows = prepend_preferred_review_windows(
            pose_prefilter_review_windows_for_candidate(
                candidate,
                duration_seconds=duration,
                min_window_seconds=chunk_estimate.rep_duration_max_sec,
            ),
            review_windows,
        )
        ...
        return PreparedVisionReview(
            ...
            # Prompt construction accepts only a usable contract, but scoring
            # still needs failed-contract diagnostics (for example a rejected
            # transition-sequence draft) to decide whether an identity-only
            # source fallback is safe.
            exercise_motion_contract=exercise_motion_contract,
            video_path=video_path,
...
def single_strong_chunk_is_enough(
    *,
    ...
    if best_chunk_score is None or best_chunk_score < 0.80:
        return False
    # Source selection needs one complete extraction interval. Other sampled
    # windows being setup, instruction, or partial movement do not contradict
    # that interval; the downstream source-cut gate independently validates
    # its boundaries before WHAM.
    return True
...
def score_candidate_vision_payload(payload: dict[str, Any]) -> tuple[float, list[str]]:
    ...
    movement_phase_gates_pass = (
        ...
        and explicit_gate_values.get("contract_start_state_match") is not False
        and explicit_gate_values.get("contract_end_state_match") is not False
        and explicit_gate_values.get("all_required_phases_visible") is not False
    )
...
def apply_explicit_gate_caps(score: float, explicit_gate_values: dict[str, bool | None]) -> float:
    caps = {
        ...
        "contract_start_state_match": 0.34,
        "contract_end_state_match": 0.34,
        "all_required_phases_visible": 0.34,
...
def build_candidate_vision_prompt(
    ...
    completion_mode = normalize_exercise_completion_mode(
        exercise_motion_contract.get("completionMode")
        if isinstance(exercise_motion_contract, dict)
        else None
    )
    motion_context = (
    ...
    required_accessories = (
        motion_context.get("requiredAccessories")
        if isinstance(motion_context, dict)
        else None
    )
    accessory_names = [
        str(accessory.get("name") or accessory.get("type") or "").strip()
        for accessory in required_accessories or []
        if isinstance(accessory, dict)
        and str(accessory.get("name") or accessory.get("type") or "").strip()
    ]
    required_support_note = (
        "Required support/accessories: "
        + ", ".join(accessory_names)
        + ". Each must visibly perform its exercise-defining support role; background presence is insufficient. "
        "If any required support is absent, replaced, or used in a different role, set target_identity_match false "
        "and include wrong_variant.\n"
        if accessory_names
        else ""
    )
    one_way_boundary_note = (
        "This contract ends at a distinct end state. Frames showing assistance or reset after that natural end are "
        "outside the target interval, not proof that the target action is partial. If one clean uninterrupted target "
        "action from its start through its distinct end is visible, judge that interval as complete and usable; the "
        "downstream segment detector will remove the post-end reset. Do not mark setup_or_talking solely because that "
        "post-end reset context is also sampled.\n"
        if completion_mode == "distinct_end_state"
        else ""
    )
    return (
        ...
        f"Video title: {candidate.title}.\n"
        f"{contract_section}"
        f"{required_support_note}"
        f"{one_way_boundary_note}"
        ...
        '"contract_start_state_match": boolean, '
        '"contract_end_state_match": boolean, '
        '"all_required_phases_visible": boolean, '
