...
def extract_smpl_mesh_payload_for_preview(payload: dict[str, object]) -> dict[str, object]:
    return {
        "bodyModel": payload.get("bodyModel"),
        "faces": payload.get("faces", []),
        "frames": payload.get("frames", []),
        "loop": payload.get("loop"),
        "bakedPreviewConfiguration": payload.get("bakedPreviewConfiguration"),
    }


def build_wham_smpl_runtime_mesh_payload(
    *,
    sequence: WhamSmplMeshSequence,
    raw_clip: MotionClip,
    cleaned_clip: MotionClip,
    ...
    for preview_frame_index, preview_frame in enumerate(preview_clip.frames):
        raw_frame_index = trim_start + preview_frame_index
        if raw_frame_index < 0 or raw_frame_index >= sequence.frame_count or raw_frame_index >= raw_clip.frame_count:
            continue
        motion_delta = _root_delta_for_frame(
            raw_clip=raw_clip,
            target_clip=preview_clip,
            raw_frame_index=raw_frame_index,
            target_frame_index=preview_frame_index,
            raw_root_joint=raw_root_joint,
            target_root_joint=preview_root_joint,
        )
        frames.append(
            {
                "frameIndex": len(frames),
                "sourceFrameIndex": preview_frame_index,
                "rawFrameIndex": raw_frame_index,
                "timeSec": preview_frame.time_sec,
                "motionDeltaApplied": _point_to_list(motion_delta),
                "vertices": [
                    [
                        float(vertex[0]) + motion_delta[0],
                        float(vertex[1]) + motion_delta[1],
                        float(vertex[2]) + motion_delta[2],
                    ]
                    for vertex in sequence.vertices[raw_frame_index]
                ],
            }
