...
class WearResumeHistoryPreservationE2ETest : WearBaseE2ETest() {
    private lateinit var workoutDriver: WearWorkoutDriver

    override fun prepareAppStateBeforeLaunch() {
        ResumeAWorkoutHistoryFixture.setup(context)
    }
    ...
    @Test
    fun resumeThenComplete_preservesPreResumeExerciseHistory() {
        ...
        WearWorkoutCompletionHelper.completeWorkoutViaStateMutations(
            ...
            context = context,
            maxSteps = 80,
            completionErrorMessage = "Workout did not complete within step limit while validating resume history preservation."
        )
    ...
    private fun assertCompletedHistoryStillContainsSeededExerciseRows() = runBlocking {
        ...
        val countsByExercise = completedSets
        ...
        ResumeAWorkoutHistoryFixture.SEEDED_EXERCISE_MIN_COUNTS.forEach { (exerciseId, expectedMinCount) ->
            ...
            require(actualCount >= expectedMinCount) {
                "Completed history lost pre-resume rows for exercise=$exerciseId. expectedAtLeast=$expectedMinCount actual=$actualCount historyId=${completedHistory.id}"
            }
        }
    }
}
