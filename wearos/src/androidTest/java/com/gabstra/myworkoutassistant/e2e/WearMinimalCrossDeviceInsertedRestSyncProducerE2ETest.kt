...
import com.gabstra.myworkoutassistant.e2e.fixtures.MinimalCrossDeviceInsertedRestWorkoutStoreFixture
...
class WearMinimalCrossDeviceInsertedRestSyncProducerE2ETest : WearBaseE2ETest() {
    ...
    override fun prepareAppStateBeforeLaunch() {
        ...
        MinimalCrossDeviceInsertedRestWorkoutStoreFixture.setupWorkoutStore(context)
    }
    ...
    @Test
    fun completeWorkout_withInsertedMiddleRest_syncsHistoryToPhone() {
        startWorkout(MinimalCrossDeviceInsertedRestWorkoutStoreFixture.WORKOUT_NAME)
        require(WearWorkoutStateMutationHelper.addRestAfterCurrent(device, context, timeoutMs = 15_000)) {
            "Failed to insert a new rest after the current set."
        }
    ...
    private fun progressUntilWorkoutCompletion(timeoutMs: Long = 60_000) {
        ...
        error("Workout did not reach completion while progressing inserted-rest scenario. Last state=$lastStateDescription")
    }
