...
class WearMinimalCrossDeviceInsertedSetSyncProducerE2ETest : WearBaseE2ETest() {
    ...
    override fun prepareAppStateBeforeLaunch() {
        ...
        MinimalCrossDeviceInsertedSetWorkoutStoreFixture.setupWorkoutStore(context)
    }
    ...
    @Test
    fun completeWorkout_withInsertedMiddleSet_syncsHistoryToPhone() {
        startWorkout(MinimalCrossDeviceInsertedSetWorkoutStoreFixture.WORKOUT_NAME)
        require(WearWorkoutStateMutationHelper.addSetAfterCurrent(device, context, timeoutMs = 15_000)) {
            "Failed to insert a new standard set after the current set."
        }
    ...
    private fun progressUntilWorkoutCompletion(timeoutMs: Long = 60_000) {
        ...
        error("Workout did not reach completion while progressing inserted-set scenario. Last state=$lastStateDescription")
    }
