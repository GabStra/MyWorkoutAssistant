...
def make_torso_dominant_slow_same_phase_leg_clip() -> MotionClip:
    frames: list[MotionFrame] = []
    for index in range(96):
        phase = index / 95.0
        wave = math.sin(math.pi * phase)
        pelvis_y = 1.0 - 0.05 * wave
        torso_z = 0.58 * wave
        leg_y = 0.36 * wave
        right_leg_y = 0.31 * wave
        joints = {
            "pelvis": (0.0, pelvis_y, 0.0),
            "spine1": (0.0, pelvis_y + 0.22, torso_z * 0.18),
            "spine2": (0.0, pelvis_y + 0.42, torso_z * 0.38),
            "spine3": (0.0, pelvis_y + 0.62, torso_z * 0.64),
            "neck": (0.0, pelvis_y + 0.78, torso_z * 0.86),
            "head": (0.0, pelvis_y + 0.92, torso_z),
            "left_collar": (-0.08, pelvis_y + 0.72, torso_z * 0.72),
            "right_collar": (0.08, pelvis_y + 0.72, torso_z * 0.72),
            "left_shoulder": (-0.24, pelvis_y + 0.70, torso_z * 0.72),
            "right_shoulder": (0.24, pelvis_y + 0.70, torso_z * 0.72),
            "left_elbow": (-0.34, pelvis_y + 0.48, torso_z * 0.64),
            "right_elbow": (0.34, pelvis_y + 0.48, torso_z * 0.64),
            "left_wrist": (-0.38, pelvis_y + 0.25, torso_z * 0.54),
            "right_wrist": (0.38, pelvis_y + 0.25, torso_z * 0.54),
            "left_hand": (-0.40, pelvis_y + 0.18, torso_z * 0.50),
            "right_hand": (0.40, pelvis_y + 0.18, torso_z * 0.50),
            "left_hip": (-0.13, pelvis_y - 0.08, 0.0),
            "right_hip": (0.13, pelvis_y - 0.08, 0.0),
            "left_knee": (-0.17, pelvis_y - 0.52 + leg_y, 0.08),
            "right_knee": (0.17, pelvis_y - 0.52 + right_leg_y, 0.08),
            "left_ankle": (-0.19, pelvis_y - 0.96 + leg_y * 0.90, 0.14),
            "right_ankle": (0.19, pelvis_y - 0.96 + right_leg_y * 0.88, 0.14),
            "left_foot": (-0.20, pelvis_y - 1.04 + leg_y * 0.82, 0.28),
            "right_foot": (0.20, pelvis_y - 1.04 + right_leg_y * 0.80, 0.28),
        }
        frames.append(MotionFrame(time_sec=index / 30.0, joints=joints))
    return MotionClip(fps=30.0, joint_names=list(frames[0].joints), frames=frames)
...
def test_torso_dominant_refinement_softens_symmetric_slow_legs_without_freezing_them() -> None:
    clip = make_torso_dominant_slow_same_phase_leg_clip()
    ...
    refined = refine_motion_clip_structurally(clip)
    metadata = refined.metadata["structuralRefinement"]
    suppressed = set(metadata["lowMagnitudeSuppression"]["suppressedJoints"])
    leg_joints = {
        "left_knee",
        "right_knee",
        "left_ankle",
    ...
    assert metadata["strategy"] == "torso_dominant_conservative_cleanup"
    assert "torso" in metadata["dominantProfile"]["dominantGroups"]
    assert "legs" in metadata["dominantProfile"]["rangeDominantGroups"]
    assert metadata["bilateralModes"]["legs"]["mode"] == "same_phase_symmetric"
    assert metadata["softBilateralSymmetry"]["applied"] is True
    assert metadata["softBilateralSymmetry"]["groups"] == ["legs"]
    assert suppressed.isdisjoint(leg_joints)
    assert max(refined_left_ankle) - min(refined_left_ankle) > 0.85 * (
        max(source_left_ankle) - min(source_left_ankle)
    )
    assert (
        metadata["softBilateralSymmetry"]["maxDisplacement"]
        <= structural_refinement_module.SOFT_LEG_SYMMETRY_MAX_CORRECTION_METERS + 1e-9
    )
...
def test_write_preview_html_embeds_motion_payload(tmp_path: Path) -> None:
    ...
    assert "Output section" in text
    assert "<input id=\"sectionStartSeconds\" type=\"number\"" in text
    assert "<input id=\"sectionEndSeconds\" type=\"number\"" in text
    assert "<button id=\"setSectionStartFromFrame\" type=\"button\">Set start</button>" in text
    assert "<button id=\"setSectionEndFromFrame\" type=\"button\">Set end</button>" in text
    assert "<button id=\"applySectionRange\" type=\"button\">Preview section</button>" in text
    assert "function applySectionRangeFromInputs()" in text
    assert "function setSectionBoundaryFromCurrentFrame(boundary)" in text
    ...
    assert "function getFrameTranslation(frame, currentFixedRoot = fixedRoot)" in text
...
def test_compute_preview_auto_alignment_preserves_tilt_without_ground_authority() -> None:
    ...
    source_support_y_values = (
        frames[0].joints["left_foot"][1],
        frames[0].joints["right_foot"][1],
        frames[0].joints["left_ankle"][1],
        frames[0].joints["right_ankle"][1],
    )
    assert support_y_values == pytest.approx(source_support_y_values)
    assert all(abs(axis[1]) > 0.999 for axis, _ in rotations)
...
def test_baked_sagittal_alignment_uses_body_axis_not_movement_direction() -> None:
    frames: list[dict[str, object]] = []
    for index in range(8):
        progress = index / 7.0
        knee_z = -0.24 + progress * 0.48
        frames.append(
            {
                "joints": {
                    "pelvis": [0.0, 1.0, 0.0],
                    ...
                    "left_knee": [-0.16, 0.55, knee_z],
                    "right_knee": [0.16, 0.55, knee_z],
                    "left_ankle": [-0.14, 0.0, 0.02],
                    "right_ankle": [0.14, 0.0, 0.02],
                    "left_foot": [-0.16, 0.0, 0.10],
                    "right_foot": [0.16, 0.0, 0.10],
                }
            }
        )
    ...
    lateral_delta = (
        float(right_shoulder[0]) - float(left_shoulder[0]),  # type: ignore[index]
...
def test_baked_sagittal_alignment_resolves_opposite_body_axis_to_negative_z() -> None:
    frames = [
        {
            "joints": {
                "left_shoulder": [0.3, 1.7, 0.0],
                "right_shoulder": [-0.3, 1.7, 0.0],
                "left_hip": [0.16, 0.9, 0.0],
                "right_hip": [-0.16, 0.9, 0.0],
            }
        }
    ]
...
def test_compute_preview_auto_alignment_levels_inverted_spine_before_yaw() -> None:
    ...
    assert rotations
    assert all(abs(math.degrees(angle)) < 90.0 for _axis, angle in rotations)
...
def test_compute_preview_auto_alignment_does_not_pitch_for_horizontal_arm_motion() -> None:
    frames = []
    for index in range(8):
        progress = index / 7.0
...
def test_compute_preview_auto_alignment_keeps_arm_motion_pitch_during_root_drift() -> None:
    frames = []
    ...
    assert horizontal_length > 0.0
    source_horizontal_delta = (
        last_hand_relative[0] - first_hand_relative[0],
        last_hand_relative[2] - first_hand_relative[2],
    )
    assert horizontal_length == pytest.approx(math.hypot(*source_horizontal_delta))
    assert all(abs(axis[1]) > 0.999 for axis, _ in rotations)


def test_compute_preview_auto_alignment_prioritizes_dominant_vertical_body_travel() -> None:
    tilt = math.radians(25.0)
    frames = []
    for index in range(8):
        vertical_offset = index / 7.0 * 0.45
        pelvis = (0.0, 1.0 + vertical_offset, 0.0)
        spine = (math.sin(tilt) * 0.85, math.cos(tilt) * 0.85, 0.0)
        frames.append(
            MotionFrame(
                time_sec=index / 30.0,
                joints={
                    ...
                    "left_shoulder": (pelvis[0] + spine[0] - 0.28, pelvis[1] + spine[1] - 0.12, 0.0),
                    "right_shoulder": (pelvis[0] + spine[0] + 0.28, pelvis[1] + spine[1] - 0.12, 0.0),
                    "left_foot": (-0.16, vertical_offset, 0.08),
                    "right_foot": (0.16, vertical_offset, 0.08),
                },
            )
        )

    rotations = _compute_preview_auto_alignment(frames)
    aligned_start = _apply_rotations_to_point(frames[0].joints["pelvis"], rotations)
    aligned_end = _apply_rotations_to_point(frames[-1].joints["pelvis"], rotations)
    travel = tuple(aligned_end[index] - aligned_start[index] for index in range(3))

    assert abs(travel[1]) > 0.0
    assert math.hypot(travel[0], travel[2]) < abs(travel[1]) * 0.05
...
def test_resolve_ffmpeg_path_falls_back_to_imageio_binary(
    tmp_path: Path,
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    ffmpeg_binary = tmp_path / "ffmpeg.exe"
    ffmpeg_binary.write_bytes(b"ffmpeg")
    for env_name in ("MWA_FFMPEG_PATH", "FFMPEG_BINARY", "IMAGEIO_FFMPEG_EXE"):
        monkeypatch.delenv(env_name, raising=False)
    monkeypatch.setattr(ffmpeg_utils_module.shutil, "which", lambda _name: None)
    monkeypatch.setitem(
        sys.modules,
        "imageio_ffmpeg",
        SimpleNamespace(get_ffmpeg_exe=lambda: str(ffmpeg_binary)),
    )

    assert ffmpeg_utils_module.resolve_ffmpeg_path() == str(ffmpeg_binary)
    assert ffmpeg_utils_module.ffmpeg_location_for_ytdlp() == str(ffmpeg_binary)


def test_resolve_ffmpeg_path_accepts_environment_directory(
    tmp_path: Path,
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    suffix = ".exe" if ffmpeg_utils_module.os.name == "nt" else ""
    install_dir = tmp_path / "ffmpeg-install"
    ffmpeg_binary = install_dir / "bin" / f"ffmpeg{suffix}"
    ffmpeg_binary.parent.mkdir(parents=True)
    ffmpeg_binary.write_bytes(b"ffmpeg")

    monkeypatch.setenv("MWA_FFMPEG_PATH", str(install_dir))
    monkeypatch.setattr(ffmpeg_utils_module.shutil, "which", lambda _name: None)
...
def test_bake_source_download_uses_preview_cache_read_through(
    tmp_path: Path,
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    url = "https://www.youtube.com/watch?v=test"
    cache_stem = youtube_module.youtube_preview_cache_stem(url)
    preview_cache = tmp_path / "youtube-preview-cache"
    source_cache = tmp_path / "youtube-source-cache"
    preview_cache.mkdir()
    cached = preview_cache / f"{cache_stem}.mp4"
    cached.write_bytes(b"cached-video")

    def fail_download_youtube(url: str, output_dir: Path, cookies_path: Path | None = None) -> Path:
    ...
    ranked_candidate = RankedCandidate(
        ...
        exercise_slug="weighted-pull-ups",
        candidate={
    ...
    output = bake_and_rank_module.copy_or_download_candidate_source(
        ranked_candidate,
        tmp_path / "candidate" / "source",
        youtube_source_cache_dir=source_cache,
        youtube_preview_cache_dir=preview_cache,
    )

    assert output.name == "source.mp4"
    assert output.read_bytes() == b"cached-video"
    assert (source_cache / f"{cache_stem}.mp4").read_bytes() == b"cached-video"
...
def test_build_youtube_download_options_prefers_low_resolution_video_only(
    ...
    monkeypatch.setattr("exercise_motion_pkg.youtube.ffmpeg_location_for_ytdlp", lambda: "ffmpeg")
    ...
    assert options["ffmpeg_location"] == "ffmpeg"
...
def test_sanitize_downloaded_video_strips_audio_when_copying(
    ...
    monkeypatch.setattr("exercise_motion_pkg.youtube.resolve_ffmpeg_path", lambda: "ffmpeg")
...
def test_run_wham_with_warm_worker_submits_job_and_reads_result(
    tmp_path: Path,
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    import contextlib
    ...
    input_dir = mount_root / "candidate" / "input"
    output_root = mount_root / "candidate" / "raw" / "wham"
    logs_dir = mount_root / "candidate" / "logs"
    session_dir = tmp_path / "worker"
    for path in (input_dir, output_root, logs_dir, session_dir / "jobs", session_dir / "results", session_dir / "job_logs"):
        path.mkdir(parents=True, exist_ok=True)
    (session_dir / "ready.json").write_text(json.dumps({"status": "ready"}), encoding="utf-8")
    input_video = input_dir / "selected_segment.mp4"
    input_video.write_bytes(b"video")
    observed_job: dict[str, object] = {}

    @contextlib.contextmanager
    def fake_lock(*, enabled: bool):
        yield 0.25 if enabled else 0.0

    def fake_worker() -> None:
        deadline = time.perf_counter() + 5.0
        while time.perf_counter() < deadline:
            job_paths = list((session_dir / "jobs").glob("*.json"))
            if not job_paths:
                time.sleep(0.05)
                continue
            job_path = job_paths[0]
            payload = json.loads(job_path.read_text(encoding="utf-8"))
            observed_job.update(payload)
            job_id = str(payload["jobId"])
            results_pkl = output_root / input_video.stem / "wham_output.pkl"
            results_pkl.parent.mkdir(parents=True, exist_ok=True)
            results_pkl.write_bytes(b"pkl")
            (session_dir / "job_logs" / f"{job_id}.stdout.log").write_text("worker stdout", encoding="utf-8")
            (session_dir / "job_logs" / f"{job_id}.stderr.log").write_text("worker stderr", encoding="utf-8")
            (session_dir / "results" / f"{job_id}.json").write_text(
                json.dumps(
                    {
                        "jobId": job_id,
                        "status": "completed",
                        "outputDir": "/workspace/candidate/raw/wham/selected_segment",
                        "resultsPkl": "/workspace/candidate/raw/wham/selected_segment/wham_output.pkl",
                        "elapsedSeconds": 0.1,
                    }
                ),
                encoding="utf-8",
            ...
            return
        raise AssertionError("warm worker test did not receive a job")

    monkeypatch.setattr(wham_runner_module, "wham_docker_run_lock", fake_lock)
    worker = threading.Thread(target=fake_worker)
    worker.start()

    result = wham_runner_module.run_wham_with_warm_worker(
        ...
        warm_worker_mount_root=mount_root,
        timeout_seconds=5.0,
    )
    worker.join(timeout=5.0)

    assert not worker.is_alive()
    assert observed_job["video"] == "/workspace/candidate/input/selected_segment.mp4"
    assert observed_job["outputRoot"] == "/workspace/candidate/raw/wham"
    assert observed_job["estimateLocalOnly"] is True
    assert observed_job["runSmplify"] is False
    assert result.results_pkl == output_root / input_video.stem / "wham_output.pkl"
    assert result.warm_worker is True
    assert result.docker_lock_wait_seconds == pytest.approx(0.25)
    timing = result.timing_payload()
    assert timing["warmWorker"] is True
    assert timing["warmWorkerSessionDir"] == str(session_dir.resolve())
    assert (logs_dir / "wham.stdout.log").read_text(encoding="utf-8") == "worker stdout"
    assert (logs_dir / "wham.stderr.log").read_text(encoding="utf-8") == "worker stderr"
...
def test_contact_sheet_sequence_labels_extend_order_badge_below_timestamp_area(tmp_path: Path) -> None:
    ...
    labeled_path = tmp_path / "labeled.jpg"
    assert build_frame_contact_sheet(
    ...
    plain = cv2.imread(str(plain_path))
    labeled = cv2.imread(str(labeled_path))
    assert plain is not None
    assert labeled is not None
    assert plain[50, 40].mean() > 220
    assert labeled[50, 40].mean() < 40
...
def test_trim_video_uses_resolved_ffmpeg_binary(
    tmp_path: Path,
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    source_path = tmp_path / "source.mp4"
    ...
    def fake_trim_video_with_ffmpeg(
        *,
        source_path: Path,
        output_path: Path,
        start_seconds: float,
    ...
    monkeypatch.setattr("exercise_motion_pkg.video_utils.resolve_ffmpeg_path", lambda: "packaged-ffmpeg")
    monkeypatch.setattr("exercise_motion_pkg.video_utils._trim_video_with_ffmpeg", fake_trim_video_with_ffmpeg)
...
def write_quality_test_video(path: Path, frames: list[np.ndarray], *, fps: float = 10.0) -> Path:
    cv2 = pytest.importorskip("cv2")
...
def test_review_video_encode_fidelity_accepts_video_written_from_preview_frames(tmp_path: Path) -> None:
    pytest.importorskip("cv2")
    frames = [
        np.full((48, 64, 3), (40 + index * 20, 80, 140), dtype=np.uint8)
        for index in range(4)
    ]
    data_urls = [frame_data_url(frame) for frame in frames]
    video_path = tmp_path / "preview.mp4"

    bake_and_rank_module.write_review_video_from_data_urls(data_urls, video_path, fps=12.0)
    metrics = bake_and_rank_module.review_video_encode_fidelity_metrics(video_path, data_urls)

    assert metrics["passed"] is True
    assert metrics["decodedFrameCount"] == len(data_urls)
    assert metrics["meanAbsolutePixelError"] <= metrics["maxMeanAbsolutePixelError"]
...
def test_review_video_render_quality_rejects_leading_blank_export(tmp_path: Path) -> None:
    ...
    visible = np.full((48, 64, 3), 80, dtype=np.uint8)
    video_path = write_quality_test_video(
        tmp_path / "leading_blank.mp4",
        [blank, blank, blank, visible, visible],
        fps=10.0,
    )

    metrics = bake_and_rank_module.review_video_render_quality_metrics(video_path)

    assert metrics["passed"] is False
    assert metrics["leadingBlankFrames"] == 3
    assert "review_video_leading_blank_frames" in metrics["rejectionReasons"]


def test_selected_preview_html_link_metrics_rejects_broken_local_links(tmp_path: Path) -> None:
    html_path = tmp_path / "selected_preview.html"
    (tmp_path / "preview.webm").write_bytes(b"preview")
    html_path.write_text(
        '<video src="preview.webm"></video><a href="missing.mp4">source</a>',
        encoding="utf-8",
    )

    broken = bake_and_rank_module.selected_preview_html_link_metrics(html_path)
    assert broken["passed"] is False
    assert broken["missingLinkCount"] == 1
    assert "preview_html_broken_links" in broken["rejectionReasons"]

    (tmp_path / "missing.mp4").write_bytes(b"source")
    fixed = bake_and_rank_module.selected_preview_html_link_metrics(html_path)
...
def test_llama_cpp_ranker_allows_existing_server_with_matching_reasoning_mode(
    ...
    readiness_posts: list[dict[str, object]] = []
    ...
    monkeypatch.setattr(youtube_module.httpx, "get", fake_get)
    monkeypatch.setattr(youtube_module.httpx, "post", fake_post)
    monkeypatch.setattr(
        youtube_module,
        "llama_cpp_server_command_lines_for_base_url",
        lambda _base_url: ["llama-server.exe --reasoning off --reasoning-format none"],
    ...
    try:
        assert readiness_posts
        assert readiness_posts[0]["max_tokens"] == 1
        assert readiness_posts[0]["reasoning_format"] == "none"
    finally:
        ranker.close()
...
def test_llama_cpp_ranker_starts_server_with_safe_vision_profile(
    ...
    ranker = youtube_module.LlamaCppVisionRanker(
        YouTubeRankingSettings(
            ...
            llama_cpp_image_min_tokens=256,
            llama_cpp_image_max_tokens=768,
            llama_cpp_mtmd_batch_max_tokens=1536,
    ...
    try:
        ...
        assert args[args.index("--image-min-tokens") + 1] == "256"
        assert "--image-max-tokens" in args
        assert args[args.index("--image-max-tokens") + 1] == "768"
        assert "--mtmd-batch-max-tokens" in args
        assert args[args.index("--mtmd-batch-max-tokens") + 1] == "1536"
...
def test_resolve_exercise_name_rewrite_applies_high_confidence_llm_result() -> None:
    ...
    ranker.client = SimpleNamespace(
        caption_images=lambda frame_paths, prompt, max_tokens=None: json.dumps(
            {
...
def test_candidate_vision_prompt_rejects_shaky_step_breakdown_videos() -> None:
    ...
    assert "load/equipment must be visibly used" in prompt
    ...
    assert '"blocking_issues": ["none|wrong_exercise|wrong_variant|partial_movement|slow_instruction|setup_or_talking|unclear"]' in prompt
...
def test_candidate_preparation_only_applies_duration_rejections() -> None:
...
def test_youtube_duration_defaults_allow_concise_demo_sources() -> None:
...
def test_review_candidate_pool_keeps_duration_eligible_candidates_in_search_order() -> None:
    noisy_first = YouTubeCandidate(
        ...
        video_id="noisy",
        title="Bench Press front view shorts compilation",
        channel="Creator",
        duration_seconds=45,
        view_count=9_000_000,
        upload_date=None,
        description_snippet="Bench press clips.",
        thumbnail=None,
    )
    clean_late = YouTubeCandidate(
        url="https://www.youtube.com/watch?v=clean",
        video_id="clean",
        title="Bench Press proper form demonstration full body side view",
        channel="Coach",
        duration_seconds=60,
        view_count=100_000,
        upload_date=None,
        description_snippet="Single person bench press full rep demo.",
        thumbnail=None,
    )
    too_long = YouTubeCandidate(
        url="https://www.youtube.com/watch?v=long",
        video_id="long",
        title="Bench Press full body side view",
        channel="Coach",
        duration_seconds=600,
        view_count=1_000,
        upload_date=None,
        description_snippet="Bench press demo.",
        thumbnail=None,
    )
...
def test_candidate_preparation_does_not_penalize_variants_or_max_attempts() -> None:
...
def test_llama_cpp_vision_client_sends_sampling_parameters(tmp_path: Path) -> None:
    ...
    assert captured_payload["reasoning_format"] == "none"
...
def test_single_worker_vision_review_prepares_candidates_progressively(
    ...
    def fake_prepare_parallel(**kwargs: object) -> dict[str, PreparedVisionReview]:
        requested_candidates = kwargs["candidates"]
        assert isinstance(requested_candidates, list)
        prepared_calls.append([candidate.video_id for candidate in requested_candidates])
...
def test_source_quality_caps_demote_variants_and_max_attempts_after_vision() -> None:
    ...
    assert frontal_score == pytest.approx(0.92)
    assert frontal_reasons == []


def test_vision_ranking_does_not_early_stop_on_source_quality_demoted_sources() -> None:
    candidate = YouTubeCandidate(
...
def test_vision_scoring_caps_wrong_variant_even_with_high_source_score() -> None:
    score, reasons = score_candidate_vision_payload(
        high_quality_vision_payload_for_test(
            blocking_issues=["wrong_variant"],
            reason="No external weight is visible.",
            source_score=1.0,
            target_match=1.0,
            complete_movement=1.0,
            execution_quality=1.0,
    ...
    assert score <= 0.20
    assert "wrong_variant_penalty" in reasons
    assert "vision_reason_contradicts_high_score" in reasons
    assert "valid_motion_scene" not in reasons
...
def test_candidate_vision_prompt_includes_generated_exercise_motion_contract() -> None:
    contract = {
        ...
        "advisoryText": (
            "Source: Barbell supported on upper back in standing stance.\n"
            "Complete: Controlled hip and knee flexion descent, bottom squat, and ascent to standing.\n"
            "Reject: Only unrack or walkout, front squat, or goblet squat.\n"
            "Notes: Judge the visible human motion, not title text."
        ),
    }
    ...
    assert "Exercise-specific movement guidance" in prompt
    assert "visible movement identity, completeness" in prompt
    ...
    assert "commonWrongVariants" not in prompt
...
def test_candidate_vision_prompt_accepts_prepared_exercise_motion_contract() -> None:
    generated_contract = {
        ...
        "exerciseName": "Barbell Back Squat",
        "advisoryText": (
            "Source: Standing under racked bar.\n"
            "Complete: Descent, bottom squat, and ascent to standing finish.\n"
            "Reject: Only unrack or walkout, or front squat.\n"
            "Notes: Use visible movement only."
        ),
    }
    ...
    assert set(prepared_contract) == {"exerciseName", "advisoryText"}
    assert "Exercise-specific movement guidance" in prompt
...
def test_exercise_motion_contract_generation_prompt_is_structured_json_guidance() -> None:
    ...
    assert "Describe one complete visible movement" in prompt
    assert "Return minified JSON only" in prompt
    assert "Use exactly these keys" in prompt
    assert "completionMode" not in prompt
    assert "requiresReturnToStart" not in prompt
    assert "advisoryText" not in prompt
    assert "validStartState" in prompt
    assert "requiredPhases" in prompt
    assert "lockout-to-bottom" in prompt
    assert "body-only skeleton animation" not in prompt


def test_exercise_motion_contract_prompt_derives_completion_policy_in_code() -> None:
    prompt = build_exercise_motion_contract_prompt(
        ExerciseEntry(
            exercise_id="weighted-pull-ups",
    ...
    assert "completionMode" not in prompt
    assert "return_to_start" not in prompt
    assert "stable_hold" not in prompt
    assert "repetition or cycle" in prompt
    assert "body-only skeleton animation" not in prompt
...
def test_final_output_prompt_uses_motion_contract_not_skeleton_contract(tmp_path: Path) -> None:
    item = ReviewItem(
        ...
        candidate_title="Zercher squat demo",
    ...
    ranking = LoopRanking(
        ...
        payload={
            "exerciseMotionContract": {
                ...
                "enabled": True,
                "status": "generated",
                "exerciseName": "Barbell Zercher Squat",
                "advisoryText": "Source: Bar held in elbow crooks.\nComplete: Hips lower and rise.\nReject: Back squat.",
            },
            "exerciseSkeletonContract": {
                "advisoryText": "Skeleton: stale guidance that should not be used.",
            },
        },
    )

    prompt = bake_and_rank_module.build_final_output_validation_prompt(
        item=item,
        ranking=ranking,
...
def test_exercise_motion_contract_generation_uses_bounded_token_budget() -> None:
    captured: dict[str, object] = {}
        ...
        def caption_images(
            ...
            *,
            ...
            return json.dumps(
                {
                    "advisoryText": (
                        "Source: Start hanging from the bar.\n"
                        "Complete: Pull body upward, then lower back to the hanging start.\n"
                        "Boundary: Exclude setup and dismount.\n"
                        "Reject: Chin-ups or setup-only clips.\n"
                        "Notes: Use one full controlled repetition."
                    ),
                    "movementType": "repetition",
                    "requiresReturnToStart": True,
                    "validStartState": "hanging from the bar before the pull",
                    "validEndState": "hanging from the bar after lowering",
                    "requiredPhases": ["hang start", "pull upward", "lower back to hang"],
                    "primaryMovingRegions": ["torso", "upper_limb"],
                    "referenceRegions": ["hands"],
                    "mustBeVisibleRegions": ["torso", "upper_limb"],
                    "excludedSetupOrCleanup": ["setup", "dismount"],
                    "boundaryRule": "Start at the hanging posture and end before dismount.",
                }
            )
    ...
    contract = youtube_module.generate_exercise_motion_contract_with_ranker(
        ...
        settings=YouTubeRankingSettings(llama_cpp_n_predict=2048),
        ranker=FakeRanker(),
    )

    assert contract["status"] == "generated"
    assert contract["advisoryText"].startswith("Source: hanging from the bar before the pull")
    assert contract["movementType"] == "repetition"
    ...
    assert "Return minified JSON only" in str(captured["prompt"])
...
def test_normalize_exercise_motion_contract_uses_plain_advisory_text() -> None:
    contract = youtube_module.normalize_exercise_motion_contract(
        {
            "advisoryText": "Source: Standing with feet grounded.\nComplete: Heels lift and lower.\nReject: Only setup.",
    ...
    assert "targetMotionProfile" not in contract
    assert contract["advisoryText"] == (
        "Source: Standing with feet grounded.\nComplete: Heels lift and lower.\nReject: Only setup."
    )
    assert prepared is not None
    assert prepared["advisoryText"] == contract["advisoryText"]


def test_normalize_exercise_motion_contract_accepts_text_alias() -> None:
    contract = youtube_module.normalize_exercise_motion_contract(
        {
            "text": (
                "Source: Torso hinged forward with arms extended downward.\n"
                "Complete: Barbell pulls toward torso and arms extend to start.\n"
                "Reject: Upright row or only setup."
            ),
        },
    ...
    assert "targetMotionProfile" not in contract
    assert prepared is not None
    assert "Torso hinged forward" in prepared["advisoryText"]
    assert "Upright row or only setup" in prepared["advisoryText"]


def test_normalize_exercise_motion_contract_synthesizes_guidance_from_structured_json() -> None:
    contract = youtube_module.normalize_exercise_motion_contract(
        {
            "requiredPhases": ["torso rises toward hands", "torso lowers back to start"],
            "rejectIf": ["only setup"],
            "observableMotionSpec": {"primaryMovingRegions": ["body", "hip"]},
            "youtubeQueryAliases": ["Strict Pull Up"],
        },
    ...
    assert "Complete: torso rises toward hands; torso lowers back to start" in contract["advisoryText"]
    assert "Reject: only setup" in contract["advisoryText"]
    assert contract["completionMode"] == "representative_cycle"
    assert contract["observableMotionSpec"]["primaryMovingRegions"] == ["torso", "hips"]
    assert prepared is not None
    assert "Structured movement contract" in youtube_module.exercise_motion_contract_prompt_body(prepared)


def test_normalize_exercise_motion_contract_unpacks_nested_advisory_json() -> None:
    contract = youtube_module.normalize_exercise_motion_contract(
        {
            "advisoryText": json.dumps(
                {
                    "movementType": "repetition",
                    "requiresReturnToStart": True,
                    "validStartState": "standing before descent",
                    "validEndState": "standing again after ascent",
                    "requiredPhases": ["descend", "ascend"],
                    "boundaryRule": "Exclude rerack after the final standing posture.",
                }
            )
        },
        exercise=ExerciseEntry(exercise_id="squat", name="Back Squat", slug="back-squat"),
        source="test",
    )
    ...
    assert contract["requiresReturnToStart"] is True
    assert contract["completionMode"] == "return_to_start"
    assert contract["advisoryText"].startswith("Source: standing before descent")
    assert "{" not in contract["advisoryText"]
    assert "rerack" in contract["advisoryText"]


def test_normalize_exercise_motion_contract_derives_repetition_return_and_matching_end_state() -> None:
    contract = youtube_module.normalize_exercise_motion_contract(
        {
            "movementType": "repetition",
            "validStartState": "bar locked out above the chest",
            "validEndState": "bar resting on the chest",
            "requiredPhases": ["lower bar to chest", "press bar upward"],
            "primaryMovingRegions": ["upper_limb"],
            "mustBeVisibleRegions": ["upper_limb", "torso"],
            "excludedSetupOrCleanup": ["unrack", "rerack"],
        },
...
def test_prepared_vision_review_payload_records_exercise_motion_contract(tmp_path: Path) -> None:
    ...
    contract = {
        "exerciseName": "Barbell Back Squat",
        "advisoryText": (
            "Source: Barbell on upper back.\n"
            "Complete: Descent, bottom squat, and ascent to standing finish.\n"
            "Reject: Only walkout or setup, or front squat."
        ),
    }
    prompt_contract = exercise_motion_contract_for_prompt(
        {
            "status": "generated",
            **contract,
        }
    ...
    assert payload is not None
    assert "Descent, bottom squat" in payload["exerciseMotionContract"]["advisoryText"]
    assert "Only walkout or setup" in payload["exerciseMotionContract"]["advisoryText"]
...
def test_full_timeline_vision_review_stops_after_strong_source_interval(
    tmp_path: Path,
) -> None:
    temp_dir = tempfile.TemporaryDirectory(dir=tmp_path)
    frame_path = tmp_path / "frame.jpg"
    ...
    try:
        score, reasons, payload = score_prepared_vision_review(
            prepared=prepared,
            settings=YouTubeRankingSettings(
                vision_max_chunks_per_candidate=0,
                vision_initial_chunks_per_candidate=2,
                vision_expand_chunks_per_candidate=5,
            ),
            caption_images=fake_caption_images,
        )
    finally:
    ...
    assert payload["adaptiveReviewPolicy"]["fullTimelineReview"] is True
    assert payload["adaptiveReviewPolicy"]["earlyStopReason"] == "strong_source_interval"


def test_full_timeline_vision_review_stops_after_single_strong_source_interval(
    tmp_path: Path,
) -> None:
    temp_dir = tempfile.TemporaryDirectory(dir=tmp_path)
    frame_path = tmp_path / "frame.jpg"
...
def test_discover_and_rank_youtube_candidates_writes_manifest_with_mocked_search_and_vision(
    ...
    assert saved["ranking"] | {"timing": None} == {
        ...
        "discoveryProgressJsonlPath": str(out_path.with_name("youtube_discovery_progress.jsonl")),
    ...
    assert out_path.with_name("youtube_discovery_progress.jsonl").exists()
    progress_events = [
        json.loads(line)
        for line in out_path.with_name("youtube_discovery_progress.jsonl").read_text(encoding="utf-8").splitlines()
    ]
    assert [event["event"] for event in progress_events][0] == "discovery_started"
    assert any(event["event"] == "candidate_review_batch_completed" for event in progress_events)
    assert progress_events[-1]["event"] == "manifest_written"
    assert saved["ranking"]["timing"]["totalElapsedSeconds"] >= 0.0
    assert saved["ranking"]["timing"]["searchElapsedSeconds"] >= 0.0
    assert saved["ranking"]["timing"]["reviewPoolPreparationElapsedSeconds"] >= 0.0
...
def test_discover_and_rank_youtube_candidates_logs_contract_error(
    tmp_path: Path,
) -> None:
    plan_path = tmp_path / "plan.json"
    out_path = tmp_path / "youtube_candidates.json"
    ...
    def failing_contract_provider(
        exercise: ExerciseEntry,
        settings: YouTubeRankingSettings,
    ) -> dict[str, object]:
        raise RuntimeError("contract timeout after 30s")
    ...
    discover_and_rank_youtube_candidates(
        ...
        search_fn=fake_search,
        vision_ranker=fake_vision,
        exercise_motion_contract_provider=failing_contract_provider,
    )
    ...
    contract_event = next(
        event for event in progress_events if event["event"] == "exercise_motion_contract_completed"
    )
    assert contract_event["status"] == "failed"
    assert contract_event["source"] == "custom"
...
def test_discover_youtube_candidates_excludes_previous_processed_manifest(tmp_path: Path) -> None:
                        ...
                        "debugCandidates": [
                            {
                                ...
                                "title": "Unreviewed debug candidate from prior attempt",
                                "status": "rejected",
                            }
                        ],
                    }
    ...
    excluded_candidates = [
        ...
        for candidate in attempt.get("excludedCandidates", [])
    ]
    assert any(
        candidate["videoId"] == "old-video"
        and "video:old-video" in candidate["matchedExclusionKeys"]
        and candidate["skipReason"] == "previously_processed_candidate"
        for candidate in excluded_candidates
    )


def test_youtube_candidate_exclusions_skip_retryable_download_failures(tmp_path: Path) -> None:
    previous_candidates_path = tmp_path / "previous_youtube_candidates.json"
    previous_candidates_path.write_text(
        json.dumps(
            {
                "exercises": [
                    {
                        "exerciseName": "Weighted Nordic Curl",
                        "candidates": [
                            {
                                "videoId": "retryable-video",
                                "url": "https://www.youtube.com/watch?v=retryable-video",
                                "status": "rejected",
                                "scoreReasons": [
                                ...
                                "visionPayload": {
                                    "posePrefilter": {
                                        "error": "Preview download failed: Sign in to confirm you are not a bot. Use --cookies."
                                    }
                                },
                            },
                            {
                                "videoId": "semantic-bad-video",
                                "url": "https://www.youtube.com/watch?v=semantic-bad-video",
                                "status": "rejected",
                                "scoreReasons": ["semantic_wrong_exercise"],
                            },
                        ],
                        "searchAttempts": [
                            {
                                "excludedCandidates": [
                                    {
                                        "videoId": "already-excluded-debug",
                                        "url": "https://www.youtube.com/watch?v=already-excluded-debug",
    ...
    assert "video:semantic-bad-video" in excluded_keys
    assert "video:retryable-video" not in excluded_keys
    assert "video:already-excluded-debug" not in excluded_keys


def test_discover_and_rank_youtube_candidates_reviews_all_duration_eligible_candidates(
    tmp_path: Path,
) -> None:
    plan_path = tmp_path / "plan.json"
    out_path = tmp_path / "youtube_candidates.json"
    ...
    def fake_search(query: str, results_per_query: int) -> list[YouTubeCandidate]:
        ...
        if search_calls > 1:
            return []
        return [*early_candidates, *later_candidates, visual_winner]
    ...
    def fake_vision(
        ...
        if candidate.video_id == visual_winner.video_id:
    ...
    assert candidates[0]["videoId"] == visual_winner.video_id
    assert candidates[0]["visionScore"] == pytest.approx(0.96)
    assert len(candidates) == 3


def test_discover_and_rank_youtube_candidates_reviews_low_text_priority_candidate(
    tmp_path: Path,
) -> None:
    plan_path = tmp_path / "plan.json"
    out_path = tmp_path / "youtube_candidates.json"
...
def test_youtube_empty_retry_does_not_retry_hard_search_errors() -> None:
    calls = 0

    def fake_search(query: str, results_per_query: int) -> list[YouTubeCandidate]:
        nonlocal calls
        del results_per_query
        calls += 1
        raise youtube_module.YouTubeSearchError(query, "socket denied")

    with pytest.raises(youtube_module.YouTubeSearchError, match="socket denied"):
        youtube_module.search_youtube_with_empty_retries(
            "Bench Press",
            settings=YouTubeRankingSettings(youtube_search_empty_retries=5),
            search_fn=fake_search,
        )
...
def test_search_youtube_raises_hard_error_for_yt_dlp_network_failure(monkeypatch: pytest.MonkeyPatch) -> None:
...
def test_discover_and_rank_youtube_candidates_fails_fast_on_hard_search_error(
    tmp_path: Path,
) -> None:
    plan_path = tmp_path / "plan.json"
    out_path = tmp_path / "youtube_candidates.json"
    ...
    with pytest.raises(youtube_module.YouTubeSearchError, match="socket denied"):
        discover_and_rank_youtube_candidates(
            ...
            settings=YouTubeRankingSettings(
                results_per_query=3,
                max_candidates=1,
                youtube_search_empty_retries=5,
            ),
            search_fn=fake_search,
        )
...
def test_pose_prefilter_keeps_front_or_back_view_as_debug_metric_only() -> None:
    metadata = BasicVideoMetadata(fps=2.0, frame_count=12, width=640, height=480)
    samples: list[PoseSample] = []
    for index in range(12):
        motion = math.sin(index / 11.0 * math.pi) * 42.0
...
def test_yolo_pose_model_is_reused_within_worker_thread(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    constructed: list[str] = []

    def fake_construct_yolo_pose_model(model_path: str) -> object:
        constructed.append(model_path)
        return object()

    monkeypatch.setattr(pose_prefilter_module, "_YOLO_MODEL_THREAD_LOCAL", threading.local())
    monkeypatch.setattr(pose_prefilter_module, "construct_yolo_pose_model", fake_construct_yolo_pose_model)

    first = pose_prefilter_module.load_thread_local_yolo_pose_model("model-a.pt")
    second = pose_prefilter_module.load_thread_local_yolo_pose_model("model-a.pt")
    third = pose_prefilter_module.load_thread_local_yolo_pose_model("model-b.pt")

    assert first is second
    assert third is not first
    assert constructed == ["model-a.pt", "model-b.pt"]


def test_yolo_pose_prefilter_checks_device_before_model_load(
    tmp_path: Path,
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    def fail_construct_yolo_pose_model(model_path: str) -> object:
        raise AssertionError("YOLO model should not load before device validation")

    def fail_resolve_yolo_device(device: str | None) -> str:
        raise RuntimeError("no cuda")

    monkeypatch.setitem(sys.modules, "cv2", SimpleNamespace())
    monkeypatch.setattr(
        pose_prefilter_module,
        "read_basic_video_metadata",
        lambda video_path: BasicVideoMetadata(fps=30.0, frame_count=30, width=640, height=480),
    )
    monkeypatch.setattr(pose_prefilter_module, "resolve_pose_model_path", lambda model: "model-a.pt")
    monkeypatch.setattr(pose_prefilter_module, "resolve_yolo_device", fail_resolve_yolo_device)
    monkeypatch.setattr(pose_prefilter_module, "construct_yolo_pose_model", fail_construct_yolo_pose_model)

    with pytest.raises(RuntimeError, match="no cuda"):
        pose_prefilter_module.run_yolo_pose_prefilter(
            video_path=tmp_path / "video.mp4",
            settings=PosePrefilterSettings(device="cuda"),
        )


def test_yolo_pose_prefilter_requires_cuda_and_rejects_cpu(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    monkeypatch.setattr(pose_prefilter_module, "torch_cuda_available", lambda: False)

    with pytest.raises(RuntimeError, match="requires a CUDA-capable Torch environment"):
        pose_prefilter_module.resolve_yolo_device("cuda")

    with pytest.raises(ValueError, match="must use CUDA"):
        pose_prefilter_module.resolve_yolo_device("cpu")

    with pytest.raises(ValueError, match="must use CUDA"):
        PosePrefilterSettings(device="cpu")
    ...
    assert pose_prefilter_module.resolve_yolo_device("gpu") == "0"
    assert pose_prefilter_module.resolve_yolo_device("0") == "0"
    assert pose_prefilter_module.resolve_yolo_device("cuda:0") == "cuda:0"
    assert PosePrefilterSettings(device=None).device == "cuda"
    assert PosePrefilterSettings(device="gpu").device == "cuda"
    assert (
        YouTubeRankingSettings(pose_prefilter_enabled=True, pose_prefilter_device="").pose_prefilter_device
        == "cuda"
    )
    with pytest.raises(ValueError, match="must use CUDA"):
        YouTubeRankingSettings(pose_prefilter_enabled=True, pose_prefilter_device="cpu")
...
def test_pose_prefilter_default_sample_plan_is_bounded_spread_scan() -> None:
    metadata = BasicVideoMetadata(fps=30.0, frame_count=30 * 180, width=1920, height=1080)
...
def test_youtube_pose_prefilter_defaults_are_bounded_for_discovery() -> None:
    settings = youtube_module.YouTubeRankingSettings(pose_prefilter_enabled=True)

    assert settings.pose_prefilter_sample_fps == pytest.approx(2.0)
    assert settings.pose_prefilter_max_seconds == pytest.approx(32.0)
    assert settings.pose_prefilter_scan_strategy == "spread"
    assert settings.pose_prefilter_workers == 1
...
def test_pose_prefilter_prefetches_review_batch_into_shared_cache(
    tmp_path: Path,
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    exercise = ExerciseEntry(exercise_id="bench", name="Bench Press", slug="bench-press")
    candidates = [
        YouTubeCandidate(
            url=f"https://www.youtube.com/watch?v=bench-{index}",
            video_id=f"bench-{index}",
            title=f"Bench Press demo {index}",
            channel="Coach",
            duration_seconds=45,
            view_count=10_000,
            upload_date=None,
    ...
    ranked_with_yolo: list[str] = []

    def fake_prefetch(
        candidates_to_prefetch: list[YouTubeCandidate],
        settings: YouTubeRankingSettings,
    ) -> None:
        assert settings.youtube_preview_cache_dir == tmp_path / "cache"
        prefetched.append([candidate.video_id or "" for candidate in candidates_to_prefetch])

    def fake_yolo(
        exercise: ExerciseEntry,
        candidate: YouTubeCandidate,
        settings: YouTubeRankingSettings,
        exercise_motion_contract: dict[str, object] | None = None,
    ) -> tuple[float, list[str], dict[str, object]]:
        del exercise, settings, exercise_motion_contract
        ranked_with_yolo.append(candidate.video_id or "")
        return (
            0.91,
            ["pose_good_motion_source"],
            {"enabled": True, "passed": True, "score": 0.91},
        )

    monkeypatch.setattr(youtube_module, "prefetch_youtube_previews_parallel", fake_prefetch)
    monkeypatch.setattr(youtube_module, "rank_candidate_with_yolo_pose", fake_yolo)
    ...
    youtube_module.rank_candidates_with_pose_prefilter(
        ...
        settings=YouTubeRankingSettings(
            ...
            pose_prefilter_workers=1,
            youtube_preview_cache_dir=tmp_path / "cache",
        ),
    )

    assert prefetched == [["bench-0", "bench-1"]]
    assert ranked_with_yolo == ["bench-0", "bench-1"]
...
def test_pose_prefilter_front_or_back_view_does_not_reject_candidate_before_vision() -> None:
    candidate = YouTubeCandidate(
    ...
    assert scored.final_score == pytest.approx(0.95)
    assert "pose_frontal_or_back_view" not in scored.score_reasons
    assert "poor_reconstruction_view_source_cap" not in scored.score_reasons
    assert scored.vision_payload is not None
    assert "frontal_or_back_view" not in scored.vision_payload["posePrefilter"].get("qualityIssues", [])
    assert youtube_module.candidate_is_eligible_for_vision_review(scored, settings) is True
    ...
    assert reviewed.final_score == pytest.approx(1.0)
    assert candidate_passes_vision_hard_gates(reviewed, settings) is True
...
def test_semantic_gate_uses_bounded_pool_before_pose_and_vision(
    ...
    assert set(semantic_reviewed) == {f"pull-{index}" for index in range(5)}
    assert set(pose_reviewed) == {f"pull-{index}" for index in range(5)}
    assert vision_reviewed == ["pull-4"]
    assert result_candidates[0]["videoId"] == "pull-4"
    assert result_candidates[0]["visionPayload"]["posePrefilter"]["passed"] is True


def test_semantic_gate_scores_configured_max_before_pose_pool(
    tmp_path: Path,
) -> None:
    plan_path = tmp_path / "plan.json"
    out_path = tmp_path / "youtube_candidates.json"
    ...
    assert set(pose_reviewed) == {"pull-0", "pull-3", "pull-5", "pull-7"}
    ...
    assert exercise_payload["candidateExpansion"]["initialSemanticGateTargetPassCount"] == 1


def test_semantic_gate_unresolved_plausible_candidate_continues_to_pose(
    tmp_path: Path,
) -> None:
    plan_path = tmp_path / "plan.json"
    out_path = tmp_path / "youtube_candidates.json"
    plan_path.write_text(json.dumps({"exercises": [{"name": "Weighted Pull-Ups"}]}), encoding="utf-8")
    plausible = YouTubeCandidate(
        url="https://www.youtube.com/watch?v=pull",
        video_id="pull",
    ...
    wrong_variant = YouTubeCandidate(
        ...
        view_count=9_000,
        upload_date=None,
        description_snippet="Weighted chin-up demonstration.",
        thumbnail=None,
    )
    semantic_reviewed: list[str] = []
    pose_reviewed: list[str] = []
    ...
    def fake_search(query: str, results_per_query: int) -> list[YouTubeCandidate]:
        return [plausible, wrong_variant] if "Weighted Pull" in query else []
    ...
    manifest = discover_and_rank_youtube_candidates(
        ...
        settings=YouTubeRankingSettings(
            ...
            pose_prefilter_enabled=True,
            pose_prefilter_candidates_per_exercise=2,
            exercise_name_rewrite_enabled=False,
            exercise_motion_contract_enabled=False,
            vision_llm_workers=1,
        ),
        search_fn=fake_search,
        semantic_gate=fake_semantic_gate,
    ...
    assert set(semantic_reviewed) == {"pull", "chin"}
    assert pose_reviewed == ["pull", "chin"]
    assert result_candidates[0]["videoId"] == "pull"
    semantic_payload = result_candidates[0]["visionPayload"]["semanticGate"]
    assert semantic_payload["passed"] is False
    assert semantic_payload["unresolved"] is True
    assert "semantic_gate_unresolved_visual_fallback" in result_candidates[0]["scoreReasons"]
...
def test_discover_and_rank_youtube_candidates_uses_semantic_order_before_expansion(
    tmp_path: Path,
) -> None:
    plan_path = tmp_path / "plan.json"
    out_path = tmp_path / "youtube_candidates.json"
    ...
    def fake_semantic_gate(
        ...
        semantic_reviewed.append(candidate.video_id or "")
        score = 0.99 if candidate.video_id == "usable-side" else 0.94
        return (
            score,
            ["semantic_text_match"],
            {
    ...
    def fake_pose(
        ...
        return (
            ...
            ["pose_good_motion_source"] if passed else ["pose_weak_body_joint_motion"],
            {
                "enabled": True,
                "passed": passed,
    ...
    assert len(set(semantic_reviewed)) == len(candidates)
    assert len(set(pose_reviewed)) == 2
    assert expansion["triggered"] is False
    assert expansion["initialSuitableCandidateCount"] == 1
    assert expansion["availableReviewCandidates"] == len(candidates)
    assert len(exercise_payload["debugCandidates"]) == len(candidates)
    decisions = [
        json.loads(line)
    ...
    assert any(
        ...
        and row["candidateExpansionTriggered"] is False
        and row["survivedCandidateList"] is True
        and row["semanticReviewed"] is True
        and row["posePassed"] is True
        for row in decisions
    )
    assert any(
        row["videoId"] == "front-0"
        and row["semanticReviewed"] is True
        and row["posePassed"] is False
        and "pose_weak_body_joint_motion" in row["scoreReasons"]
        for row in decisions
    )
...
def test_semantic_gate_zero_survivors_expands_and_soft_fallbacks_to_pose(
    ...
    assert set(semantic_reviewed) == {"hard-wrong", "maybe-crunch"}
...
def test_semantic_gate_trusts_model_pass_without_title_conflict_override() -> None:
    exercise = ExerciseEntry(
    ...
    assert semantic_payload["score"] == pytest.approx(1.0)
    assert "textConflictReasons" not in semantic_payload
    assert scored.status == "recommended"
    assert "semantic_gate_passed" in scored.score_reasons


def test_semantic_gate_rejects_model_reported_named_variant_terms() -> None:
    exercise = ExerciseEntry(exercise_id="pull-up", name="Pull Up", slug="pull-up")
    ...
    scored = apply_semantic_gate_score(
        ...
        semantic_payload={
            ...
            "wrongExercise": False,
            "wrongEquipment": False,
            "unrequestedVariantTerms": ["chest to bar"],
            "matchedExercise": "Pull Up",
            "reason": "The model reports a named variant.",
        },
        settings=YouTubeRankingSettings(),
    )
...
def test_llama_cpp_semantic_gate_disables_reasoning_for_parallel_text_gate() -> None:
            ...
            return json.dumps(
                {
                    ...
                    "matchedExercise": "Weighted Pull-Ups",
                    "reason": "exact match",
                }
            )

    gate = LlamaCppSemanticGate(
        YouTubeRankingSettings(llama_cpp_n_predict=512),
        shared_ranker=SimpleNamespace(client=FakeClient()),
    )

    score, reasons, payload = gate(
        ExerciseEntry(
            exercise_id="weighted-pull-ups",
            name="Weighted Pull-Ups",
            slug="weighted-pull-ups",
    ...
    assert score == pytest.approx(1.0)
    assert "semantic_text_match" in reasons
...
def test_semantic_gate_prompt_scores_related_variants_low() -> None:
    ...
    assert "Return minified JSON only" in prompt
    assert "For generic weighted/loaded targets" in prompt
    assert "Use score 0.0 to 1.0 for exact-target confidence" in prompt
...
def test_load_ranked_candidates_appends_youtube_fallback_candidates_for_bake(tmp_path: Path) -> None:
    candidates_path = tmp_path / "youtube_candidates.json"
    candidates_path.write_text(
        json.dumps(
            {
                ...
                "exercises": [
                    {
                        ...
                        "candidates": [
                            ...
                            },
                            {
                                "videoId": "fallback",
                                "url": "https://www.youtube.com/watch?v=fallback",
                                "title": "Lower ranked source",
                                "status": "rejected",
                                "visionPayload": {"source_score": 0.82},
                            },
                            {
                                "videoId": "disabled",
    ...
    assert [candidate.video_id for candidate in candidates] == ["primary", "fallback"]
    fallback = candidates[1]
    assert fallback.candidate["bakeFallbackCandidate"] is True
    assert fallback.candidate["sourceDiscoveryStatus"] == "rejected"
    assert fallback.candidate["status"] == "rejected"
    source_gate = bake_and_rank_module.evaluate_source_candidate_gate(fallback)
    assert source_gate["passed"] is True
    assert source_gate["bakeFallbackCandidate"] is True
    assert "candidate_status_rejected" not in source_gate["reasons"]
...
def test_source_window_expansion_skips_too_short_full_cycle_vlm_chunk_for_pose_chunk(
    tmp_path: Path,
) -> None:
    request = BakeAndRankRequest(
        candidates_json=tmp_path / "candidates.json",
    ...
    candidate = RankedCandidate(
        ...
        candidate={
            ...
            "exerciseMotionContract": {
                ...
                "requiredEquipment": ["barbell"],
                "requiredStartPosture": "barbell above chest",
                "requiredEndPosture": "barbell above chest again",
                "requiredPhases": [
                    "barbell lowers to the chest",
                    "barbell presses back to full extension",
                ],
                "primaryMotionRegions": ["upper_limb", "elbows"],
                "mustBeVisible": ["elbows", "barbell travel"],
                "rejectIf": ["only lowering phase visible", "only pressing phase visible"],
                "commonWrongVariants": [],
                "reviewNotes": ["one-way fragments are not complete repetitions"],
                "observableMotionSpec": {
                    "schemaVersion": 1,
                    "primaryMovingRegions": ["upper_limb", "elbows"],
                    "referenceRegions": ["shoulders", "torso"],
                    "primaryAxis": "vertical",
                    "motionPattern": "joint_flex_extend",
                    "requiresReturnToStart": True,
                    "oneWayPartialIsInvalid": True,
                    "mustShowFullCycle": True,
                    "mustBeVisibleRegions": ["upper_limb", "elbows"],
                },
            },
            "visionPayload": {
                "bestChunkSource": "chunked_source_video_review",
                "bestChunkStartSeconds": 44.48,
                "bestChunkEndSeconds": 46.48,
                "bestChunkScore": 1.0,
                "reviewedChunks": [
                    {
                        "startSeconds": 44.48,
                ...
                "posePrefilter": {
                    ...
                    "score": 0.96,
                    "validChunks": [
                        {"startSeconds": 40.48, "endSeconds": 46.48, "score": 0.96},
                        {"startSeconds": 44.48, "endSeconds": 46.48, "score": 0.92},
                    ],
                },
            },
        },
    ...
    hint = expanded[0].source_chunk_hint
    assert hint is not None
    assert hint.start_seconds == pytest.approx(40.48)
    assert hint.end_seconds == pytest.approx(46.48)
    assert expanded[0].candidate["sourceWindowAttemptSource"] == "pose_prefilter"
    assert expanded[0].candidate["visionPayload"]["bestChunkStartSeconds"] == pytest.approx(40.48)
    assert expanded[0].candidate["visionPayload"]["bestChunkEndSeconds"] == pytest.approx(46.48)


def test_source_window_expansion_uses_parent_when_full_cycle_best_chunk_is_too_short(
    tmp_path: Path,
) -> None:
    request = BakeAndRankRequest(
        candidates_json=tmp_path / "candidates.json",
    ...
    candidate = RankedCandidate(
        exercise_index=0,
        candidate_rank=0,
        exercise_id="weighted-nordic-curl",
        exercise_name="Weighted Nordic Curl",
        exercise_slug="weighted-nordic-curl",
        candidate={
            "videoId": "nordic",
            "title": "Weighted Nordic Curl",
            "exerciseMotionContract": {
                "exerciseName": "Weighted Nordic Curl",
                "advisoryText": (
                    "Complete: Visible movement from a kneeling position where the torso is lowered "
                    "toward the floor, followed by a return to the starting position."
                ),
            },
            "visionPayload": {
                "bestChunkSource": "chunked_source_video_review",
                "bestChunkStartSeconds": 4.0,
                "bestChunkEndSeconds": 5.867,
                "bestChunkScore": 0.95,
                "reviewedChunks": [
                    {
                        "startSeconds": 4.0,
                ...
                "posePrefilter": {
                    ...
                    "score": 0.92,
                    "sampledWindows": [
                        {"startSeconds": 0.0, "endSeconds": 5.933},
                    ],
                    "validChunks": [
                        {"startSeconds": 4.0, "endSeconds": 5.867, "score": 0.92},
                    ],
                },
            },
        },
    ...
    assert hint is not None
    assert hint.start_seconds == pytest.approx(0.0)
    assert hint.end_seconds == pytest.approx(5.933)
    assert expanded[0].candidate["sourceWindowAttemptSource"] == "source_window_parent_hint"
...
def test_bake_and_rank_pipeline_keeps_multiple_selected_results_for_manual_review(
    ...
    assert manifest["selected"]["previewHtmlPath"] == manifest["selected"]["selectedPreviewHtmlPath"]
    assert manifest["selectedResults"][0]["previewHtmlPath"] == manifest["selectedResults"][0]["selectedPreviewHtmlPath"]
    assert manifest["selectedResults"][1]["previewHtmlPath"] == manifest["selectedResults"][1]["selectedPreviewHtmlPath"]
    assert Path(manifest["selectedResults"][0]["sourcePreviewHtmlPath"]).name == "motion_preview.html"
    assert Path(manifest["selectedResults"][1]["sourcePreviewHtmlPath"]).name == "motion_preview.html"
...
def test_bake_and_rank_parallel_pipeline_keeps_fallback_candidates_for_final_review(
    ...
    assert manifest["timings"]["candidateSelectionMode"] == "parallel_ready_candidate_requests_then_selection"
...
def test_bake_and_rank_queue_skips_previous_terminal_candidate_result(
    ...
    assert candidate_results[0]["previousTerminalResult"]["finalSelectionStatus"] == "rejected_after_materialized_review"
    assert candidate_results[1]["finalSelectionStatus"] == "selected"
    ...
    (workspace / "selection_manifest.json").write_text(
        json.dumps({"candidateResults": [candidate_results[0]]}),
        encoding="utf-8",
    )
...
def test_bake_and_rank_retries_parent_source_window_after_partial_child_rejection(
    tmp_path: Path,
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    request = BakeAndRankRequest(
    ...
    processed_modes: list[object] = []

    def make_review_item(active_candidate: RankedCandidate) -> ReviewItem:
        candidate_workspace = request.workspace / active_candidate.workspace_slug
        skeleton_path = candidate_workspace / "wear" / "skeleton.json"
        review_video_path = candidate_workspace / "review" / "preview.webm"
        skeleton_path.parent.mkdir(parents=True, exist_ok=True)
        review_video_path.parent.mkdir(parents=True, exist_ok=True)
        ...
        if bake_and_rank_module.parse_optional_bool(active_candidate.candidate.get("sourceWindowParentFallback")) is True:
            segment_payload = {
                "sourcePrepReason": "parent_source_window_fallback",
                "selectedSpan": {"startSeconds": 0.0, "endSeconds": 8.0},
            }
        else:
            segment_payload = {
                "sourcePrepReason": "pre_wham_source_window_choice",
                "candidateSourceWindow": {"startSeconds": 0.0, "endSeconds": 8.0},
                "selectedSpan": {"startSeconds": 5.5, "endSeconds": 7.5},
            }
        ...
        return ReviewItem(
            exercise_index=active_candidate.exercise_index,
            candidate_rank=active_candidate.candidate_rank,
            loop_index=-1,
            exercise_name=active_candidate.exercise_name,
            candidate_title=active_candidate.title,
            candidate_workspace=candidate_workspace,
            preview_html_path=candidate_workspace / "preview" / "motion_preview.html",
            skeleton_path=skeleton_path,
            review_video_path=review_video_path,
    ...
    def fake_process_ranked_candidate(
        active_candidate: RankedCandidate,
        *,
        request: BakeAndRankRequest,
        preview_baker,
        review_items: list[ReviewItem],
        ...
        **_kwargs: object,
        ...
        processed_modes.append(active_candidate.candidate.get("sourceWindowAttemptMode"))
        item = make_review_item(active_candidate)
        review_items.append(item)
        review_item_entries.append(bake_and_rank_module.review_item_to_manifest(item))
        return {
            "status": "ready_for_selection",
            "candidate": active_candidate.candidate,
            "candidateWorkspace": str(request.workspace / active_candidate.workspace_slug),
            **bake_and_rank_module.source_window_attempt_manifest(active_candidate),
        }
    ...
    def fake_choose_best_materialized_review_item(
        ...
        *,
        ...
        selected = (items[0], rankings[0])
        if bake_and_rank_module.parse_optional_bool(items[0].candidate.get("sourceWindowParentFallback")) is True:
            return selected, None
        rejected_ranking = LoopRanking(
            score=0.0,
            reasons=["materialized_output_rejected", "partial_movement"],
            payload={
                "finalOutputValidation": {
                    "passed": False,
                    "needsRetry": True,
                    "rejectionReasons": ["partial_movement"],
                }
            },
        )
        return None, (items[0], rejected_ranking)
    ...
    assert processed_modes == [None, bake_and_rank_module.PARENT_SOURCE_WINDOW_FALLBACK_ATTEMPT_MODE]
    assert selected is not None
    assert bake_and_rank_module.parse_optional_bool(selected[0].candidate.get("sourceWindowParentFallback")) is True
    assert [item.candidate.get("sourceWindowParentFallback") for item, _ranking in selected_results] == [True]
    assert candidate_results[0]["finalSelectionStatus"] == "rejected_after_materialized_review"
    assert candidate_results[0]["parentSourceWindowFallbackQueued"] is True
    assert candidate_results[0]["parentSourceWindowFallbackReason"] == "partial_movement"
    assert candidate_results[1]["finalSelectionStatus"] == "selected"
    assert selection_attempts[0]["parentSourceWindowFallbackQueued"] is True
    assert timings["parentSourceWindowFallbackQueuedCount"] == 1
    assert timings["newCandidateAttemptCount"] == 2
...
def test_previous_terminal_results_recover_candidate_bake_manifests(tmp_path: Path) -> None:
    candidate = RankedCandidate(
        exercise_index=0,
        candidate_rank=0,
        exercise_id="bench-press",
        exercise_name="Bench Press",
        exercise_slug="bench-press",
        candidate={"videoId": "old-bad", "title": "Old bad source"},
    )
    workspace = tmp_path / "build"
    candidate_workspace = workspace / candidate.workspace_slug
    bake_manifest_path = candidate_workspace / "review" / "bake_manifest.json"
    bake_manifest_path.parent.mkdir(parents=True, exist_ok=True)
    bake_manifest_path.write_text(
        json.dumps(
            {
                "exerciseIndex": candidate.exercise_index,
                "candidateRank": candidate.candidate_rank,
    ...
    key = bake_and_rank_module.ranked_candidate_attempt_key(candidate)
    assert previous_terminal[key]["finalSelectionStatus"] == "rejected_after_materialized_review"


def test_previous_terminal_results_keep_legacy_skipped_rows(tmp_path: Path) -> None:
    candidate = RankedCandidate(
        exercise_index=0,
        candidate_rank=0,
        exercise_id="bench-press",
        exercise_name="Bench Press",
        exercise_slug="bench-press",
        candidate={"videoId": "legacy-bad", "title": "Legacy bad source"},
    )
    workspace = tmp_path / "build"
    workspace.mkdir()
    (workspace / "selection_manifest.json").write_text(
                ...
                "candidateResults": [
                    {
                        ...
                        "exerciseName": candidate.exercise_name,
                        "candidate": candidate.candidate,
                        "candidateWorkspace": str(workspace / candidate.workspace_slug),
                        ...
                        "previousFinalSelectionStatus": "rejected_after_materialized_review",
                        **bake_and_rank_module.source_window_attempt_manifest(candidate),
                    }
                ]
            }
    ...
    assert bake_and_rank_module.ranked_candidate_attempt_key(candidate) in previous_terminal


def test_failed_download_candidate_is_not_terminal_retry_decision() -> None:
    result = {
        "status": "failed",
        "finalSelectionStatus": "not_reviewable",
        "failures": [
            {
    ...
    assert bake_and_rank_module.candidate_result_has_terminal_quality_decision(result) is False


def test_source_gate_too_short_full_cycle_window_can_queue_parent_fallback(tmp_path: Path) -> None:
    request = BakeAndRankRequest(
        candidates_json=tmp_path / "candidates.json",
        workspace=tmp_path / "build",
        wham_repo_path=None,
    ...
    source_gate = bake_and_rank_module.evaluate_source_candidate_gate(candidate, request=request)

    assert source_gate["passed"] is False
    assert "source_window_too_short_for_full_cycle" in source_gate["reasons"]
    should_queue, reason = (
    ...
    assert reason == "source_window_too_short_for_full_cycle"


def test_segment_detection_no_span_failure_can_queue_parent_fallback(tmp_path: Path) -> None:
    request = BakeAndRankRequest(
        candidates_json=tmp_path / "candidates.json",
        workspace=tmp_path / "build",
        wham_repo_path=None,
                ...
                "failures": [
                    {
                        ...
                        "message": (
                            "Source segment detection did not find a usable "
                            "Weighted Nordic Curl span."
                        ),
                    }
                ],
            },
...
def test_selected_section_wear_skeleton_cache_requires_selected_preview_settings() -> None:
    ...
    export_payload = {
        ...
        "selectedPreviewSettings": {
            **dict(options),
            "manualModelRotationDegrees": manual_rotation,
        },
...
def test_lazy_llama_cpp_session_reports_expired_candidate_budget_as_candidate_timeout(
    tmp_path: Path,
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    class FakeClient:
        def caption_images(self, **_kwargs: object) -> str:
            raise AssertionError("an expired candidate budget must not start another VLM request")
    ...
    monkeypatch.setattr(bake_and_rank_module, "LlamaCppVisionRanker", FakeLlamaCppVisionRanker)
    monkeypatch.setattr(bake_and_rank_module.time, "perf_counter", lambda: 2.0)
    session = bake_and_rank_module.LazyLlamaCppVisionSession(
        BakeAndRankRequest(
            candidates_json=tmp_path / "candidates.json",
            workspace=tmp_path / "build",
            wham_repo_path=None,
    ...
    session.set_candidate_deadline(1.0)

    with pytest.raises(
        bake_and_rank_module.CandidateWallTimeBudgetExpired,
...
def test_classify_support_dominance_from_review_loop_uses_classification_callback(
    ...
    def fake_extract_window_frames(
        *,
        ...
        max_frame_width: int,
        contact_sheet_sequence_labels: bool,
        output_dir: Path,
    ) -> list[Path]:
        assert frames_per_window == 7
        assert contact_sheet_sequence_labels is True
...
def source_scorecard_candidate(
    candidate_id: str,
    *,
    exercise_match: float = 0.96,
    full_movement: float = 0.92,
    start_visible: float = 0.90,
    finish_visible: float = 0.90,
    mechanics_consistency: float = 0.92,
    start_state_ready: float = 0.92,
    start_boundary_clean: float = 0.92,
    finish_boundary_clean: float = 0.92,
    setup_or_filler: float = 0.0,
    source_quality: float = 0.92,
    confidence: float = 0.90,
    reject: list[str] | None = None,
    note: str = "Candidate contains the full target movement.",
) -> dict[str, object]:
    return {
        "id": candidate_id,
...
def test_prepare_candidate_input_video_validates_padded_ranked_chunk_before_wham(
    ...
    def fake_caption_images(**kwargs: object) -> str:
        ...
        return json.dumps(
            {
                "candidates": [
                    source_scorecard_candidate(candidate_id, note="Candidate contains one complete clean movement.")
    ...
    pre_wham_contract = {
        ...
        "exerciseName": "Bench Press",
        "advisoryText": (
            "Source: Barbell bench press setup.\n"
            "Complete: Bar lowers to chest and presses back to extended arms.\n"
            "Reject: Only setup or unrack, or incline press.\n"
            "Notes: Use visible motion only."
        ),
    }
    ...
    selected = bake_and_rank_module.prepare_candidate_input_video(
        ...
        exercise_motion_contract_resolver=lambda _candidate: pre_wham_contract,
    )
    ...
    assert len(trim_calls) == 1
    assert trim_calls[0]["start_seconds"] == pytest.approx(6.0)
    assert trim_calls[0]["end_seconds"] == pytest.approx(10.7)
    prompts = captured["prompts"]
    assert isinstance(prompts, list)
    assert '"candidates"' in prompts[0]
    assert '"approved"' in prompts[0]
    assert '"confidence"' in prompts[0]
    assert "setup_or_filler" in prompts[0]
    assert "code owns timing" in prompts[0]
    assert "Approve only when" in prompts[0]
    assert "selected candidate IDs" in prompts[0]
    assert "Exercise-specific source identity guidance" in prompts[0]
    ...
    assert selection["selectedSpan"]["endSeconds"] == pytest.approx(10.7)
    assert selection["selectedSpanInOriginalSource"]["startSeconds"] == pytest.approx(6.0)
    assert selection["selectedSpanInOriginalSource"]["endSeconds"] == pytest.approx(10.7)


def test_exercise_motion_contract_resolver_generates_on_demand_and_caches_per_exercise(
    tmp_path: Path,
) -> None:
    prompts: list[str] = []
    ...
    def fake_caption_images(**kwargs: object) -> str:
        ...
        disable_reasoning_values.append(kwargs.get("disable_reasoning"))
        temperatures.append(kwargs.get("temperature"))
        top_ps.append(kwargs.get("top_p"))
        top_ks.append(kwargs.get("top_k"))
        return json.dumps(
            {
                "movementType": "repetition",
                "requiresReturnToStart": True,
                "validStartState": "standing with the barbell on the upper back",
                "validEndState": "standing after the ascent",
                "requiredPhases": ["squat descent", "bottom position", "ascent back to standing"],
                "commonWrongVariants": ["front-rack squats", "setup-only clips"],
                "boundaryRule": "Exclude setup and rerack.",
            }
        )

    resolver = bake_and_rank_module.build_exercise_motion_contract_resolver(
        request=BakeAndRankRequest(
            candidates_json=tmp_path / "candidates.json",
                workspace=tmp_path / "build",
                wham_repo_path=None,
    ...
    assert first["exerciseMotionContractStatus"] == "generated_on_demand"
    assert "squat descent" in first["advisoryText"]
    assert "front-rack squats" in first["advisoryText"]
    assert len(prompts) == 1
    assert disable_reasoning_values == [False]
    assert temperatures == [0.0]
    assert top_ps == [1.0]
    assert top_ks == [0]
    assert "Write a compact exercise-specific movement contract" in prompts[0]
...
def test_exercise_motion_contract_resolver_reuses_candidate_contract_without_llm(
            ...
            candidate={
                ...
                "visionPayload": {
                    "exerciseMotionContract": {
                        ...
                        "exerciseName": "Barbell Back Squat",
                        "advisoryText": (
                            "Source: Barbell over the back.\n"
                            "Complete: Descent and ascent.\n"
                            "Reject: Front rack squat.\n"
                            "Notes: Use visible motion only."
                        ),
                    }
                },
    ...
    assert contract["exerciseMotionContractStatus"] == "reused_candidate_contract"
    assert "Front rack squat" in contract["advisoryText"]
...
def test_prepare_candidate_input_video_rejects_partial_ranked_chunk_before_wham(
        ...
        return json.dumps(
            {
                "candidates": [
                    source_scorecard_candidate(
                        ...
                        finish_visible=0.10,
                        confidence=0.80,
                        reject=["partial_movement"],
                        note="Every candidate is a partial one-way fragment.",
                    )
                ],
            }
        )
    ...
    rows = selection["sourceCutRanking"]["payload"]["sourceCutScorecardCandidates"]
    assert rows[0]["reject"] == ["partial_movement"]


def test_generate_candidate_motion_rejects_qaxj9nbssn4_before_wham(
    tmp_path: Path,
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    source_video = tmp_path / "qaXj9nBSSN4.mp4"
    source_video.write_bytes(b"video")

    class FakeChunkEstimate:
        rep_duration_min_sec = 3.0
        rep_duration_max_sec = 6.0
        movement_complexity = "compound"
        chunk_seconds = 6.0
        chunk_overlap_seconds = 1.0
        source = "test"
        ...
        return json.dumps(
            {
                "candidates": [
                    source_scorecard_candidate(
                        ...
                        start_visible=0.20,
                        finish_visible=0.20,
                        setup_or_filler=0.80,
                        source_quality=0.40,
                        confidence=0.86,
                        reject=["wrong_exercise", "partial_movement", "setup_or_filler"],
                        note="This is not a clean weighted pull-up source window.",
                    )
                    for candidate_id in candidate_ids
                ]
            }
    ...
    monkeypatch.setattr(
        ...
        lambda _request: pytest.fail("WHAM generation should not run for a rejected source scorecard"),
    )
...
def test_generate_candidate_motion_treats_detection_timeout_as_critical(
    tmp_path: Path,
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    source_video = tmp_path / "source.mp4"
    ...
    with pytest.raises(CriticalVlmInteractionError) as exc_info:
        bake_and_rank_module.generate_candidate_motion(
            RankedCandidate(
                exercise_index=0,
                candidate_rank=0,
    ...
    assert exc_info.value.details["stage"] == "source_segment_detection"
    ...
    report_path = (
        ...
        / "bulgarian-split-squat-001-bulgarian-split-squat"
        / "segment_detection"
        / "vlm_critical_error.json"
    )
    assert report_path.exists()
    report = json.loads(report_path.read_text(encoding="utf-8"))
    assert report["error"]["critical"] is True
    assert report["context"]["stage"] == "source_segment_detection"
    assert not (report_path.parent / "segment_selection.json").exists()
...
def test_artifact_retention_debug_prunes_heavy_intermediates_and_keeps_debug_outputs(tmp_path: Path) -> None:
    ...
    final_source_contact_sheet = (
        candidate
        / "review"
        / "full-clip.default-final-output-validation"
        / "source"
    ...
    for path, content in (
        ...
        (frame_dump, b"frame"),
        (final_source_contact_sheet, b"final-source-sheet"),
        ...
        "reviewItems": [
            {
                ...
                "ranking": {
                    "payload": {
                        "finalOutputValidation": {
                            "sourceContactSheetPaths": [str(final_source_contact_sheet)],
                        },
                    },
                },
            }
    ...
    assert final_source_contact_sheet.exists()
...
def make_scene_orientation_payload(*, inverted: bool, frame_count: int = 6) -> dict[str, object]:
    head_y = -1.5 if inverted else 1.5
    neck_y = -1.2 if inverted else 1.2
    pelvis_y = 0.0
...
def make_moving_feet_scene_orientation_payload(*, inverted: bool, frame_count: int = 6) -> dict[str, object]:
    payload = make_scene_orientation_payload(inverted=inverted, frame_count=frame_count)
    frames = payload["frames"]
    assert isinstance(frames, list)
    divisor = max(1, frame_count - 1)
    for index, frame in enumerate(frames):
        assert isinstance(frame, dict)
        joints = frame["joints"]
        assert isinstance(joints, dict)
        left_offset = -0.45 * (index / divisor)
        right_offset = 0.45 * (index / divisor)
        for joint_name in ("left_ankle", "left_foot"):
            point = joints[joint_name]
            assert isinstance(point, list)
            point[0] += left_offset
        for joint_name in ("right_ankle", "right_foot"):
            point = joints[joint_name]
            assert isinstance(point, list)
            point[0] += right_offset
    return payload


def make_moving_support_scene_orientation_payload(*, inverted: bool, frame_count: int = 6) -> dict[str, object]:
    payload = make_moving_feet_scene_orientation_payload(
        inverted=inverted,
    ...
    for index, frame in enumerate(frames):
        ...
        joints = frame["joints"]
        assert isinstance(joints, dict)
        left_offset = -0.55 * (index / divisor)
        right_offset = 0.55 * (index / divisor)
        hand_y = -0.2 if inverted else 0.2
        joints["left_wrist"] = [-0.30 + left_offset, hand_y, 0.08]
        joints["left_hand"] = [-0.34 + left_offset, hand_y, 0.12]
        joints["right_wrist"] = [0.30 + right_offset, hand_y, 0.08]
        joints["right_hand"] = [0.34 + right_offset, hand_y, 0.12]
    return payload
...
def test_deterministic_post_bake_scene_orientation_fallback_preserves_auto_alignment() -> None:
    options = {
        **bake_and_rank_module.build_preview_bake_base_options(motion_tuning_enabled=True),
        "autoWorldAlignment": True,
        "sceneInverted": False,
...
def test_deterministic_preview_settings_hints_keep_auto_alignment_for_horizontal_body() -> None:
    ...
    assert hint["horizontalBodyAutoAlignmentRisk"] is True
    assert hint["sceneInversionTrialRecommended"] is True
    assert hint["forceAutoWorldAlignmentFalse"] is False
    assert corrected["options"]["autoWorldAlignment"] is True
...
def test_adaptive_preview_planner_applies_deterministic_scene_orientation_correction(
    ...
    monkeypatch.setattr(
        ...
        lambda data_urls, output_path, *, timestamps=None, sequence_labels=False: (
            captured.__setitem__("contact_sheet_frame_count", len(data_urls)),
            captured.__setitem__("timestamps", timestamps),
            captured.__setitem__("sequence_labels", sequence_labels),
    ...
    assert captured["sequence_labels"] is True
...
def test_adaptive_preview_planner_keeps_default_auto_alignment_when_orientation_clean(
    tmp_path: Path,
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    base_options = bake_and_rank_module.build_preview_bake_base_options(motion_tuning_enabled=True)
    ...
    assert len(variants) == 1
    assert captured["sequence_labels"] is True
    effective_options = {**base_options, **variants[0]["options"]}
    assert effective_options["autoWorldAlignment"] is True
    assert effective_options["sceneInverted"] is False
    plan_payload = json.loads((tmp_path / "full-input.adaptive-settings-planning" / "adaptive_settings_plan.json").read_text())
    ...
    class FakePage:
        def evaluate(self, script: str, payload: dict[str, object]) -> object:
            if "bakeLoop" in script:
                options = payload["options"]
                assert isinstance(options, dict)
                result = make_scene_orientation_payload(inverted=True, frame_count=24)
                result["selectedPreviewSettings"]["sceneInverted"] = bool(options.get("sceneInverted"))
                return result
            if "renderFrame" in script:
                return "data:image/png;base64,"
            raise AssertionError(script)
...
def test_adaptive_preview_planner_adds_feet_and_hands_lock_trials_after_orientation(
    tmp_path: Path,
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    base_options = bake_and_rank_module.build_preview_bake_base_options(motion_tuning_enabled=True)
    ...
    class FakePage:
        def evaluate(self, script: str, payload: dict[str, object]) -> object:
            if "bakeLoop" in script:
                options = payload["options"]
                assert isinstance(options, dict)
                result = make_moving_support_scene_orientation_payload(inverted=True, frame_count=24)
                result["selectedPreviewSettings"]["sceneInverted"] = bool(options.get("sceneInverted"))
                return result
            if "renderFrame" in script:
                return "data:image/png;base64,"
    ...
    assert "final hands-lock trial after orientation planning" in hands_lock_trial["adaptivePreviewSettings"]["reason"]
    plan_payload = json.loads((tmp_path / "full-input.adaptive-settings-planning" / "adaptive_settings_plan.json").read_text())
    assert len(plan_payload["plannedVariants"]) == 3
    assert plan_payload["plannedVariants"][1]["id"] == "adaptive-feet-lock-trial"
    assert plan_payload["plannedVariants"][2]["id"] == "adaptive-hands-lock-trial"
    assert len(plan_payload["materializedVariants"]) == 3


def test_adaptive_preview_planner_caps_materialized_variants(
    tmp_path: Path,
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    base_options = bake_and_rank_module.build_preview_bake_base_options(motion_tuning_enabled=True)
    ...
    assert len(variants) == 1
    assert variants[0]["id"] == "adaptive-baseline"
    skipped_trials = variants[0]["adaptivePreviewSettings"].get("skippedVariantTrials", [])
    assert [trial["reason"] for trial in skipped_trials[-2:]] == [
        "max_adaptive_preview_settings_cap",
        "max_adaptive_preview_settings_cap",
    ]
    plan_payload = json.loads((tmp_path / "full-input.adaptive-settings-planning" / "adaptive_settings_plan.json").read_text())
    assert len(plan_payload["plannedVariants"]) == 3
    assert len(plan_payload["materializedVariants"]) == 1
    assert plan_payload["materializedVariants"][0]["id"] == "adaptive-baseline"


def test_support_lock_trials_are_suppressed_when_support_motion_is_too_large() -> None:
    base_options = bake_and_rank_module.build_preview_bake_base_options(motion_tuning_enabled=True)
    variants = [
        {
    ...
    preview_settings_hint = {
        ...
        "leftFootMotionRatio": 0.52,
        "rightFootMotionRatio": 0.48,
        "leftHandMotionRatio": 0.44,
        "rightHandMotionRatio": 0.46,
    ...
    by_id = {str(variant["id"]): variant for variant in planned}
    assert list(by_id) == ["adaptive-baseline"]
    skipped_trials = by_id["adaptive-baseline"]["adaptivePreviewSettings"].get("skippedVariantTrials", [])
...
def test_adaptive_variant_cap_keeps_deterministically_recommended_feet_lock() -> None:
    preview_settings_hint = {
        ...
        "handSupportLockTrialRecommended": False,
        "leftFootMotionRatio": 0.22,
        "rightFootMotionRatio": 0.23,
        "leftHandMotionRatio": 0.52,
        "rightHandMotionRatio": 0.50,
        "supportLockTrialThreshold": 0.35,
    }
    adaptive_settings = {
        "source": "deterministic_support_lock_trial",
        "deterministicPreviewSettings": preview_settings_hint,
    }
    variants = [
        {
            "id": "adaptive-baseline",
            "options": {"lockPlantedFeet": False, "lockPlantedHands": False},
            "adaptivePreviewSettings": adaptive_settings,
        },
        {
            "id": "adaptive-feet-lock-trial",
            "options": {"lockPlantedFeet": True, "lockPlantedHands": False},
            "adaptivePreviewSettings": adaptive_settings,
        },
    ]

    limited = bake_and_rank_module.limit_adaptive_preview_settings_variants(
        variants,
        max_variants=1,
    )
...
def test_preview_bake_base_options_default_auto_world_alignment_on() -> None:
    ...
    assert options["autoWorldAlignment"] is True
    ...
    assert catalog_entry["default"] is True
    catalog_entry = next(
        item for item in bake_and_rank_module.preview_tuning_option_catalog(motion_tuning_enabled=True)
...
def test_loop_adjustment_penalizes_paired_hands_distortion(tmp_path: Path) -> None:
    ...
    assert adjusted.score == pytest.approx(0.49)
    assert "paired_hands_distortion_penalty" in adjusted.reasons
    assert "deterministic_review_validation_skipped" not in adjusted.reasons
    assert adjusted.payload is not None
...
def test_materialized_output_gate_rejects_paired_hands_distortion_even_when_vlm_approves(
    ...
    skeleton = tmp_path / "candidate" / "wear" / "paired-distorted.json"
    review = tmp_path / "candidate" / "review" / "paired-distorted.webm"
    write_paired_hands_skeleton(skeleton, distorted=True)
    ...
    monkeypatch.setattr(
        bake_and_rank_module,
        "review_video_render_quality_metrics",
        lambda _path: {"passed": True, "rejectionReasons": []},
    )
    monkeypatch.setattr(
        bake_and_rank_module,
        "validate_final_output_with_caption_images",
    ...
    selected = bake_and_rank_module.apply_materialized_output_acceptance_gate(
        ...
        final_output_caption_images=lambda **_kwargs: "{}",
    )

    _selected_item, selected_ranking = selected
    assert bake_and_rank_module.selected_artifact_score(selected) < 0.55
    assert selected_ranking.payload is not None
    assert selected_ranking.payload["materializedOutputRejected"] is True
    gate = selected_ranking.payload["materializedOutputAcceptanceGate"]
    assert gate["passed"] is False
    assert "materialized_paired_hands_distortion" in gate["rejectionReasons"]
...
def test_materialized_selection_blocks_failed_movement_cut_before_materialization(
    tmp_path: Path,
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    candidate_workspace = tmp_path / "candidate"
    preview_html = candidate_workspace / "preview" / "motion_preview.html"
    skeleton = candidate_workspace / "wear" / "skeleton.baked.full-input.json"
    review_video = candidate_workspace / "review" / "full-input.webm"
    preview_html.parent.mkdir(parents=True)
    ...
    item = ReviewItem(
        ...
        loop_start_seconds=0.0,
        loop_end_seconds=3.0,
        candidate={"videoId": "zercher"},
        settings_options={"autoWorldAlignment": True, "sceneInverted": True},
    )
    ranking = LoopRanking(
        score=0.68,
        reasons=["movement_cut_scorecard_no_passing_candidate"],
        payload={
            "score": 0.0,
            "modelScore": 0.0,
            "movementCutManualTrimRequired": True,
            "movementCutManualTrimReason": "movement_cut_scorecard_no_passing_candidate",
    ...
    monkeypatch.setattr(
        bake_and_rank_module,
        "materialize_llm_selected_time_range",
        lambda *_args, **_kwargs: pytest.fail("failed movement cut should not be materialized"),
    )
    monkeypatch.setattr(
        bake_and_rank_module,
        "apply_materialized_output_acceptance_gate",
        lambda *_args, **_kwargs: pytest.fail("failed movement cut should not reach final validation"),
    )

    accepted, rejected = bake_and_rank_module.materialize_selection_candidate(
        (item, ranking),
        request=BakeAndRankRequest(
            candidates_json=tmp_path / "candidates.json",
            workspace=tmp_path / "build",
        ...
        materialized_quality_rescore_enabled=False,
    )

    assert accepted is None
    assert rejected is not None
    rejected_item, rejected_ranking = rejected
    assert rejected_item is item
    assert rejected_ranking is not None
    assert rejected_ranking.score == 0.0
    assert rejected_ranking.model_score == 0.0
    assert "movement_cut_selection_blocked" in rejected_ranking.reasons
    assert "movement_cut_scorecard_no_passing_candidate" in rejected_ranking.reasons
    assert rejected_ranking.payload is not None
    assert rejected_ranking.payload["selectionBlocked"] is True
    assert rejected_ranking.payload["selectionBlockedReason"] == "movement_cut_scorecard_no_passing_candidate"
    assert bake_and_rank_module.selected_artifact_score(rejected) == pytest.approx(0.0)


def test_materialized_output_gate_ignores_loop_bridge_when_loop_not_required(
    tmp_path: Path,
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    candidate_workspace = tmp_path / "candidate"
    skeleton = candidate_workspace / "wear" / "loop-mismatch.json"
    review_video = candidate_workspace / "review" / "loop-mismatch.webm"
    write_loop_bridge_mismatch_skeleton(skeleton)
    review_video.parent.mkdir(parents=True)
    review_video.write_bytes(b"video")
    item = ReviewItem(
        ...
        candidate_title="Zercher squat",
        candidate_workspace=candidate_workspace,
        preview_html_path=candidate_workspace / "preview.html",
        skeleton_path=skeleton,
        review_video_path=review_video,
        duration_sec=3.0,
        loop_start_seconds=0.0,
    ...
    not_required = bake_and_rank_module.materialized_output_acceptance_metrics(
        item,
        LoopRanking(score=0.9, reasons=[], payload={"loopContinuityRequired": False}),
    )
    required = bake_and_rank_module.materialized_output_acceptance_metrics(
        item,
        LoopRanking(score=0.9, reasons=[], payload={"loopContinuityRequired": True}),
    )

    assert not_required["passed"] is True
    assert "materialized_loop_bridge_mismatch_ignored_loop_not_required" in not_required["skippedReasons"]
    assert "materialized_loop_bridge_pose_mismatch" not in not_required["rejectionReasons"]
...
def test_materialized_selection_rejects_kinematic_failure_without_subinterval_retry(
    tmp_path: Path,
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    candidate_workspace = tmp_path / "candidate"
    ...
    assert selected is None
    assert rejected_best is not None
    rejected_item, rejected_ranking = rejected_best
    assert rejected_ranking is not None
    assert rejected_item.settings_variant_id.startswith("llm-selected-section")
    assert rejected_ranking.score < 0.55
    assert "kinematic_clean_subinterval_fallback" not in rejected_ranking.reasons
    assert "support_lock_safe_rebake" not in rejected_ranking.reasons
    assert "support_lock_loop_bridge_rebake" not in rejected_ranking.reasons
    assert rejected_ranking.payload is not None
    assert rejected_ranking.payload["materializedOutputRejected"] is True
    assert rejected_ranking.payload["materializedValidationStatus"] == "rejected"
    assert "materialized_output_rejected" in rejected_ranking.reasons
    assert "limb_velocity_spike_penalty" in rejected_ranking.reasons
    assert "joint_angle_spike_penalty" in rejected_ranking.reasons
    assert "bone_length_instability_penalty" in rejected_ranking.reasons
    assert bake_calls[0][0] == "llm-selected-section"
    assert [call[0] for call in bake_calls] == ["llm-selected-section"]
...
def test_rank_review_item_does_not_run_post_wham_movement_cut(
    tmp_path: Path,
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    candidate_workspace = tmp_path / "candidate"
    ...
    skeleton_path.write_text(json.dumps({"frames": []}), encoding="utf-8")
    review_video_path.write_bytes(b"video")
    item = ReviewItem(
        exercise_index=0,
        candidate_rank=0,
        ...
        candidate={
            ...
            "visionPayload": {
                "exerciseMotionContract": {
                    ...
                    "advisoryText": (
                        "Source: Rear foot stays elevated while the front knee bends.\n"
                        "Complete: Front leg drives the body back upward after the descent.\n"
                        "Reject: Standard lunge with both feet on the floor, forward lunge, or step-up.\n"
                        "Notes: Choose the window with the full down-and-up repetition."
                    ),
                }
            },
        },
    ...
    monkeypatch.setattr(
        ...
        lambda **_kwargs: called.append("source_contact_sheet"),
    )
    monkeypatch.setattr(
        bake_and_rank_module,
        "render_review_window_contact_sheet",
        lambda **_kwargs: called.append("preview_contact_sheet"),
    )

    def fake_caption_images(**_kwargs: object) -> str:
        called.append("caption_images")
        raise AssertionError("post-WHAM movement cut should not call the VLM")
    ...
    assert ranking.payload["exerciseMotionContractEnabled"] is True
    assert ranking.payload["exerciseMotionContract"]["exerciseName"] == "Bulgarian Split Squat"
    assert "Rear foot stays elevated" in ranking.payload["exerciseMotionContract"]["advisoryText"]
    assert "Front leg drives" in ranking.payload["exerciseMotionContract"]["advisoryText"]
...
def test_movement_cut_prompt_can_omit_exercise_contract_for_ab(tmp_path: Path) -> None:
    ...
    contract = {
        ...
        "advisoryText": (
            ...
            "Complete: Descent and ascent.\n"
            "Reject: Front rack squat.\n"
            "Notes: Use a full down-and-up rep."
        ),
    }
    ...
    assert "Movement-cut exercise guidance." in enabled_prompt
    assert "Front rack squat" in enabled_prompt
    assert "Movement-cut exercise guidance." not in disabled_prompt
...
def test_movement_cut_contract_can_come_from_advisory_source_review_payload() -> None:
    ...
    assert "Descent and ascent" in contract["advisoryText"]
    assert "Front rack squat" in contract["advisoryText"]
...
def test_movement_cut_selects_single_motion_coverage_valid_candidate_without_vlm(
        ...
        return json.dumps(
            {
                "candidates": [
                    source_scorecard_candidate(
                        "A",
                        confidence=0.90,
                        note="Candidate A keeps the full movement.",
                    )
                ]
            }
        )
    ...
    assert [candidate["candidateId"] for candidate in vlm_candidates] == ["A"]
    assert ranking.payload["movementCutSelectionPolicy"] == "shortest_scorecard_complete_movement"
...
def test_movement_cut_visual_integrity_falls_back_when_all_candidates_fail(
        ...
        if "Candidate A:" in prompt:
            candidate_rows.append(
                source_scorecard_candidate(
                    "A",
                    confidence=0.83,
                    note="Candidate A is complete but longer.",
                )
            )
        if "Candidate B:" in prompt:
            candidate_rows.append(
                source_scorecard_candidate(
                    "B",
                    confidence=0.84,
                    note="Candidate B keeps the complete clean motion.",
                )
            )
    ...
    monkeypatch.setattr(
        bake_and_rank_module,
        "selected_window_is_materially_shorter",
...
def test_final_output_source_contact_sheets_use_exact_selected_source_window(
    tmp_path: Path,
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    candidate_workspace = tmp_path / "candidate"
    selected_segment = candidate_workspace / "input" / "selected_segment.mp4"
    selected_segment.parent.mkdir(parents=True)
    selected_segment.write_bytes(b"video")
    item = ReviewItem(
        exercise_index=0,
        candidate_rank=0,
        loop_index=-1,
    ...
    result = bake_and_rank_module.final_output_source_contact_sheets(item, output_dir=tmp_path / "source")

    assert result == [sheet_path]
    assert captured["video_path"] == selected_segment
    window = captured["window"]
    assert isinstance(window, DetectionWindow)
    assert window.start_seconds == pytest.approx(1.25)
    assert window.end_seconds == pytest.approx(3.75)
...
def test_source_cut_candidate_windows_can_respect_full_rep_minimum_for_pre_wham() -> None:
    parent = DetectionWindow(index=0, start_seconds=0.0, end_seconds=8.0)
    chunk_estimate = SimpleNamespace(rep_duration_min_sec=3.0, rep_duration_max_sec=8.0)

    source_windows = bake_and_rank_module.build_source_cut_candidate_windows(
        window=parent,
        chunk_estimate=chunk_estimate,
        min_estimated_duration_ratio=bake_and_rank_module.SOURCE_CUT_MIN_ESTIMATED_DURATION_RATIO,
    )
    movement_windows = bake_and_rank_module.build_source_cut_candidate_windows(
        window=parent,
        chunk_estimate=chunk_estimate,
        min_estimated_duration_ratio=bake_and_rank_module.MOVEMENT_CUT_MIN_ESTIMATED_DURATION_RATIO,
    )

    assert min(window.end_seconds - window.start_seconds for window in source_windows) == pytest.approx(3.0)
    assert min(window.end_seconds - window.start_seconds for window in movement_windows) == pytest.approx(1.8)
...
def test_movement_cut_vlm_shortlist_keeps_one_candidate_requests_with_multiple_images(
    tmp_path: Path,
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    candidate_workspace = tmp_path / "candidate"
    ...
    windows = [
        DetectionWindow(index=index, start_seconds=0.0, end_seconds=float(index + 1))
        for index in range(8)
    ]
    ...
    def fake_build_movement_cut_review_candidate(**kwargs: object) -> bake_and_rank_module.SourceCutCandidate:
        candidate_id = str(kwargs["candidate_id"])
        output_dir = kwargs["output_dir"]
        assert isinstance(output_dir, Path)
        frame_paths = [
            output_dir / "start_boundary.jpg",
            output_dir / "finish_boundary.jpg",
            output_dir / "full_sheet.jpg",
        ]
        ...
        return bake_and_rank_module.SourceCutCandidate(
            ...
            boundary_audit_frame_paths=frame_paths[:2],
            motion_coverage={
                "passed": True,
                "sourceMotionCoverageRatio": 1.0,
                ...
                "targetMotionGate": {
                    "targetMotionRangeRatio": 1.0,
                    "minTargetMotionRangeRatio": 0.1,
                },
            },
        )
    ...
    def fake_caption_images(*, frame_paths: list[Path], prompt: str, **_kwargs: object) -> str:
        calls.append((frame_paths, prompt))
        candidate_ids = re.findall(r"Candidate ([A-Z]+):", prompt)
        ...
        return json.dumps(
            {
                "candidates": [
                    source_scorecard_candidate(
                        candidate_id,
                        confidence=0.92,
                        reject=[] if candidate_id == "C" else ["partial_movement"],
                        note="Complete." if candidate_id == "C" else "Partial.",
                    )
                ]
            }
        )
    ...
    monkeypatch.setattr(
        bake_and_rank_module,
        "build_movement_cut_review_candidate",
        fake_build_movement_cut_review_candidate,
    )
    monkeypatch.setattr(
        bake_and_rank_module,
        "selected_window_is_materially_shorter",
    ...
    result = bake_and_rank_module.rank_movement_cut_candidates_with_caption_images(
        item=item,
        timeline_window=DetectionWindow(index=0, start_seconds=0.0, end_seconds=8.0),
        chunk_estimate=SimpleNamespace(movement_complexity="compound"),
        output_dir=tmp_path / "movement_cut",
        frame_count=32,
        caption_images=fake_caption_images,
    ...
    assert result is not None
    ranking, _render_seconds, _vlm_seconds = result
    assert len(calls) == bake_and_rank_module.MOVEMENT_CUT_MAX_INITIAL_VLM_CANDIDATES
    assert ranking.payload is not None
    assert ranking.payload["movementCutVlmRequestPolicy"] == "one_candidate_per_request"
    assert ranking.payload["movementCutVlmEligibleCandidateCount"] == 8
    assert ranking.payload["movementCutVlmShortlistedCandidateCount"] == 6
    assert ranking.payload["movementCutVlmShortlistFilteredCount"] == 2
    assert ranking.payload["movementCutVlmReviewedRequestCount"] == 6
    assert ranking.payload["movementCutTargetedBoundaryRefinementAllowed"] is False
    assert ranking.payload["movementCutTargetedBoundaryRefinementTriggered"] is False
    assert ranking.payload["selectedCandidateId"] == "C"


def test_movement_cut_duration_floor_blocks_short_vlm_candidates(
    tmp_path: Path,
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    candidate_workspace = tmp_path / "candidate"
    item = ReviewItem(
        ...
        duration_sec=2.8,
        loop_start_seconds=0.0,
        loop_end_seconds=2.8,
        candidate={"videoId": "squat", "exerciseMotionContract": lower_body_motion_contract()},
    )
    windows = [
        DetectionWindow(index=0, start_seconds=1.0, end_seconds=2.8),
        DetectionWindow(index=1, start_seconds=0.0, end_seconds=2.8),
    ]
    ...
    def fake_build_movement_cut_review_candidate(**kwargs: object) -> bake_and_rank_module.SourceCutCandidate:
        ...
        output_dir = kwargs["output_dir"]
        assert isinstance(output_dir, Path)
        frame_paths = [output_dir / "contact_sheet_01.jpg"]
        for path in frame_paths:
            path.parent.mkdir(parents=True, exist_ok=True)
            path.write_bytes(b"sheet")
        return bake_and_rank_module.SourceCutCandidate(
        ...
        return json.dumps(
            {
                "candidates": [
                    source_scorecard_candidate(
                        "B",
                        confidence=0.96,
                        note="Candidate B keeps the full down-and-up repetition.",
                    )
                ]
            }
        )
    ...
    result = bake_and_rank_module.rank_movement_cut_candidates_with_caption_images(
        item=item,
        timeline_window=DetectionWindow(index=0, start_seconds=0.0, end_seconds=2.8),
        chunk_estimate=SimpleNamespace(rep_duration_min_sec=4.0),
        output_dir=tmp_path / "movement_cut",
        frame_count=32,
        caption_images=fake_caption_images,
    )
    ...
    assert result is not None
    ranking, _render_seconds, _vlm_seconds = result
    assert reviewed_candidate_ids == ["B"]
    assert ranking.payload is not None
    assert ranking.payload["selectedCandidateId"] == "B"
    assert ranking.payload["selected_section_start_seconds"] == pytest.approx(0.0)
    assert ranking.payload["selected_section_end_seconds"] == pytest.approx(2.8)
    assert ranking.payload["movementCutMinSelectedDurationSeconds"] == pytest.approx(2.8)
    assert ranking.payload["movementCutDurationFloorFilteredCount"] == 1
    assert [candidate["candidateId"] for candidate in ranking.payload["movementCutVlmInputCandidates"]] == ["B"]
...
def test_movement_cut_motion_coverage_rejects_one_way_candidate_even_when_parent_phase_fails(tmp_path: Path) -> None:
    candidate_workspace = tmp_path / "candidate"
    skeleton_path = candidate_workspace / "wear" / "skeleton.baked.full-input.json"
    review_video_path = candidate_workspace / "review" / "full-input.webm"
    write_motion_strength_skeleton_with_source_times(
    ...
    metrics = bake_and_rank_module.movement_cut_candidate_motion_coverage_metrics(
        item=item,
        parent_window=DetectionWindow(index=0, start_seconds=0.0, end_seconds=3.0),
        candidate_window=DetectionWindow(index=1, start_seconds=0.0, end_seconds=2.5),
        chunk_estimate=SimpleNamespace(movement_complexity="compound"),
        exercise_motion_contract=lower_body_motion_contract(),
    )

    assert metrics["parentFullRepetitionPhaseCompletenessMetrics"]["passed"] is False
    assert metrics["candidateFullRepetitionPhaseCompletenessMetrics"]["passed"] is False
    assert metrics["passed"] is False
    assert "movement_cut_incomplete_repetition_phase" in metrics["rejectionReasons"]
...
def test_movement_cut_uses_plain_text_contract_for_lower_body_target() -> None:
    contract = {
        "schemaVersion": 1,
        "enabled": True,
        "status": "generated",
        ...
        "exerciseName": "Barbell Zercher Squat",
        "advisoryText": (
            "Source: Person holds a barbell in the crooks of the elbows, standing upright with hips back. "
            "Complete: Movement starts from standing, descends until hips are below parallel, and returns to standing. "
            "Reject: barbell held on the shoulders."
        ),
    }

    metrics = bake_and_rank_module.movement_cut_target_motion_gate_metrics(
        exercise_name="Barbell Zercher Squat",
        candidate_metrics={
            "rootVerticalRangeRatio": 0.18,
            "lowerBodyProximalRootRelativeRangeRatio": 0.16,
            "lowerBodyDistalRootRelativeRangeRatio": 0.50,
            "upperBodyRootRelativeRangeRatio": 0.20,
        },
        candidate_phase_metrics={"dominantJoint": "left_ankle"},
        chunk_estimate=SimpleNamespace(movement_complexity="compound"),
        exercise_motion_contract=contract,
    )

    assert metrics["targetMotionRegion"] == "lower_body"
    assert metrics["targetMotionRangeRatio"] >= metrics["minTargetMotionRangeRatio"]
    assert metrics["distalLowerBodyDominant"] is True
    assert metrics["distalDominanceGateRequired"] is False
    assert metrics["passed"] is True
    assert "movement_cut_distal_setup_motion_dominates" not in metrics["rejectionReasons"]
        ...
        json.dumps(
            {
                "candidates": [
                    source_scorecard_candidate(
                        ...
                        finish_visible=0.10,
                        reject=["partial_movement"],
                        note="All candidates are partial lowering-only fragments.",
                    )
                ],
            }
        ),
...
def test_source_cut_scorecard_choice_prefers_shortest_passing_candidate() -> None:
    candidates = [
        bake_and_rank_module.SourceCutCandidate(
            candidate_id="A",
            window=DetectionWindow(index=0, start_seconds=0.0, end_seconds=5.0),
            frame_paths=[Path("a.jpg")],
        ),
        bake_and_rank_module.SourceCutCandidate(
            candidate_id="B",
            window=DetectionWindow(index=1, start_seconds=1.0, end_seconds=3.0),
            frame_paths=[Path("b.jpg")],
        ),
    ]
        ...
        json.dumps(
            {
                "candidates": [
                    source_scorecard_candidate(
                        ...
                        source_quality=0.96,
                        confidence=0.95,
                        note="Longer complete movement.",
                    ),
                    source_scorecard_candidate("B", confidence=0.76, note="Shortest complete movement."),
                ]
            }
        ),
        candidates,
    ...
    assert ranking.payload is not None
    assert ranking.payload["selectedCandidateId"] == "B"
    assert ranking.payload["sourceCutSelectionPolicy"] == "progressive_multiscale_stable_level_then_score"
...
def test_source_cut_scorecard_choice_prefers_later_equal_duration_candidate() -> None:
    candidates = [
        bake_and_rank_module.SourceCutCandidate(
            candidate_id="A",
            window=DetectionWindow(index=0, start_seconds=40.48, end_seconds=44.48),
            frame_paths=[Path("setup-biased.jpg")],
        ),
        bake_and_rank_module.SourceCutCandidate(
            candidate_id="B",
            window=DetectionWindow(index=1, start_seconds=42.48, end_seconds=46.48),
            frame_paths=[Path("movement-complete.jpg")],
        ),
    ]
    ...
    ranking = bake_and_rank_module.parse_source_cut_candidate_choice(
        json.dumps(
            {
                "candidates": [
                    source_scorecard_candidate("A", note="Earlier same-duration window."),
                    source_scorecard_candidate("B", note="Same duration and confidence, later in the source chunk."),
                ]
            }
        ),
        candidates,
    ...
    assert ranking.payload is not None
    assert ranking.payload["selectedCandidateId"] == "B"
    assert ranking.payload["selected_section_start_seconds"] == pytest.approx(42.48)
    assert ranking.payload["selected_section_end_seconds"] == pytest.approx(46.48)


def test_source_cut_classifier_schema_requires_boundary_fields() -> None:
    candidates = [
        bake_and_rank_module.SourceCutCandidate(
            candidate_id="A",
            window=DetectionWindow(index=0, start_seconds=0.0, end_seconds=4.0),
    ...
    assert "completeMovement" in row["missingFields"]
    assert "startBoundaryClean" in row["missingFields"]
    assert "finishBoundaryClean" in row["missingFields"]
    assert "setupOrFiller" in row["missingFields"]


def test_source_cut_classifier_schema_rejects_dirty_boundaries() -> None:
    candidates = [
        bake_and_rank_module.SourceCutCandidate(
            candidate_id="A",
            window=DetectionWindow(index=0, start_seconds=38.74, end_seconds=43.24),
            frame_paths=[Path("rerack.jpg")],
        )
    ]
                ...
                "candidates": [
                    {
                        ...
                        "startBoundaryClean": False,
                        "finishBoundaryClean": False,
                        "setupOrFiller": True,
                        "reject": ["bad_boundary", "setup_or_filler"],
                        "note": "Starts low and reracks.",
                    }
                ],
            }
        ),
    ...
    row = ranking.payload["sourceCutScorecardCandidates"][0]
    assert row["passed"] is False
    assert "source_cut_start_boundary_bad" in row["rejectionReasons"]
    assert "source_cut_finish_boundary_bad" in row["rejectionReasons"]
    assert "source_cut_setup_or_filler" in row["rejectionReasons"]


def test_source_cut_classifier_schema_accepts_clean_complete_window() -> None:
    candidates = [
        bake_and_rank_module.SourceCutCandidate(
            candidate_id="A",
            window=DetectionWindow(index=0, start_seconds=36.0, end_seconds=42.0),
            frame_paths=[Path("clean.jpg")],
        )
    ]
    ...
    assert ranking is not None
    assert ranking.score == pytest.approx(0.93)
    assert ranking.payload["selectedCandidateId"] == "A"
    row = ranking.payload["sourceCutScorecardCandidates"][0]
    assert row["passed"] is True
    assert row["startBoundaryClean"] is True
    assert row["finishBoundaryClean"] is True
    assert row["setupOrFiller"] is False


def test_source_cut_scorecard_parser_rejects_missing_fields_and_invalid_json() -> None:
    candidates = [
        bake_and_rank_module.SourceCutCandidate(
            candidate_id="A",
            window=DetectionWindow(index=0, start_seconds=0.0, end_seconds=3.0),
...
def test_source_cut_scorecard_without_contract_uses_stricter_thresholds() -> None:
    candidates = [
        bake_and_rank_module.SourceCutCandidate(
            candidate_id="A",
            window=DetectionWindow(index=0, start_seconds=0.0, end_seconds=3.0),
    ...
    with_contract = bake_and_rank_module.parse_source_cut_candidate_choice(
        payload,
        candidates,
        has_contract=True,
    )
    without_contract = bake_and_rank_module.parse_source_cut_candidate_choice(
        payload,
        candidates,
        has_contract=False,
    )

    assert with_contract is not None
    assert with_contract.payload["selectedCandidateId"] == "A"
    assert without_contract is not None
    assert without_contract.score == pytest.approx(0.0)
    row = without_contract.payload["sourceCutScorecardCandidates"][0]
    assert "source_cut_wrong_exercise" in row["rejectionReasons"]
    assert "source_cut_partial_movement" in row["rejectionReasons"]


def test_weighted_pullup_source_scorecard_rejects_h_and_selects_p() -> None:
    candidates = [
        bake_and_rank_module.SourceCutCandidate(
            candidate_id="H",
            window=DetectionWindow(index=7, start_seconds=40.0, end_seconds=42.0),
            frame_paths=[Path("candidate-h.jpg")],
        ),
        bake_and_rank_module.SourceCutCandidate(
            candidate_id="P",
            window=DetectionWindow(index=15, start_seconds=40.0, end_seconds=45.0),
            frame_paths=[Path("candidate-p.jpg")],
        ),
    ]
        ...
        json.dumps(
            {
                "candidates": [
                    source_scorecard_candidate(
                        ...
                        finish_visible=0.30,
                        reject=["partial_movement"],
                        note="Only the top phase is visible.",
                    ),
                    source_scorecard_candidate(
                        "P",
                        exercise_match=0.95,
                        ...
                        finish_visible=0.92,
                        confidence=0.91,
                        note="Full bottom-top-bottom weighted pull-up is visible.",
                    ),
                ]
            }
        ),
    ...
    assert ranking is not None
    assert ranking.payload is not None
    assert ranking.payload["selectedCandidateId"] == "P"
    rows = {row["id"]: row for row in ranking.payload["sourceCutScorecardCandidates"]}
    assert rows["H"]["passed"] is False
    assert "source_cut_scorecard_reject_partial_movement" in rows["H"]["rejectionReasons"]
    assert rows["P"]["passed"] is True


def test_source_cut_scorecard_rejects_mechanics_inconsistency_even_with_high_identity() -> None:
    candidates = [
        bake_and_rank_module.SourceCutCandidate(
            candidate_id="A",
            window=DetectionWindow(index=0, start_seconds=0.0, end_seconds=4.0),
            frame_paths=[Path("mechanics-change.jpg")],
        )
    ]
        ...
        json.dumps(
            {
                "candidates": [
                    source_scorecard_candidate(
                        ...
                        start_visible=0.94,
                        finish_visible=0.94,
                        mechanics_consistency=0.35,
                        source_quality=0.95,
                        confidence=0.90,
                        reject=["mechanics_inconsistent"],
                        note="Defining setup changes mid movement.",
                    )
                ]
            }
        ),
    ...
    row = ranking.payload["sourceCutScorecardCandidates"][0]
    assert row["passed"] is False
    assert row["mechanics_consistency"] == pytest.approx(0.35)
    assert "source_cut_mechanics_inconsistent" in row["rejectionReasons"]
    assert "source_cut_scorecard_reject_mechanics_inconsistent" in row["rejectionReasons"]


def test_source_cut_scorecard_rejects_unclean_boundaries_even_with_high_identity() -> None:
    candidates = [
        bake_and_rank_module.SourceCutCandidate(
            candidate_id="A",
            window=DetectionWindow(index=0, start_seconds=0.0, end_seconds=4.0),
            frame_paths=[Path("setup-boundary.jpg")],
        )
    ]
        ...
        json.dumps(
            {
                "candidates": [
                    source_scorecard_candidate(
                        ...
                        start_visible=0.94,
                        finish_visible=0.94,
                        mechanics_consistency=0.95,
                        start_boundary_clean=0.20,
                        finish_boundary_clean=0.90,
                        source_quality=0.95,
                        confidence=0.90,
                        reject=["start_boundary_bad"],
                        note="Starts with setup before target movement.",
                    )
                ]
            }
        ),
    ...
    row = ranking.payload["sourceCutScorecardCandidates"][0]
    assert row["passed"] is False
    assert row["start_boundary_clean"] == pytest.approx(0.20)
    assert "source_cut_start_boundary_bad" in row["rejectionReasons"]
    assert "source_cut_scorecard_reject_start_boundary_bad" in row["rejectionReasons"]


def test_source_cut_scorecard_rejects_start_state_not_ready_even_with_high_identity() -> None:
    candidates = [
        bake_and_rank_module.SourceCutCandidate(
            candidate_id="A",
            window=DetectionWindow(index=0, start_seconds=0.0, end_seconds=4.0),
            frame_paths=[Path("setup-transition.jpg")],
        )
    ]
        ...
        json.dumps(
            {
                "candidates": [
                    source_scorecard_candidate(
                        ...
                        finish_visible=0.94,
                        mechanics_consistency=0.95,
                        start_state_ready=0.20,
                        source_quality=0.95,
                        confidence=0.90,
                        reject=["start_state_not_ready"],
                        note="Early frames show setup transition.",
                    )
                ]
            }
        ),
    ...
    row = ranking.payload["sourceCutScorecardCandidates"][0]
    assert row["passed"] is False
    assert row["start_state_ready"] == pytest.approx(0.20)
    assert "source_cut_start_state_not_ready" in row["rejectionReasons"]
    assert "source_cut_scorecard_reject_start_state_not_ready" in row["rejectionReasons"]


def test_movement_cut_scorecard_choice_prefers_shortest_complete_candidate() -> None:
    candidates = [
        bake_and_rank_module.SourceCutCandidate(
            candidate_id="A",
            window=DetectionWindow(index=0, start_seconds=0.0, end_seconds=5.0),
        ...
        json.dumps(
            {
                "candidates": [
                    ...
                    source_scorecard_candidate(
                        "B",
                        confidence=0.75,
                        note="Shortest complete movement.",
                    ),
                ]
            }
        ),
    ...
    assert ranking.payload["selectedCandidateId"] == "B"
    assert ranking.payload["movementCutSelectionPolicy"] == "shortest_scorecard_complete_movement"
    assert ranking.payload["finalCutResponsibility"] == "movement_window_selection_only"
    assert ranking.score == pytest.approx(0.75)
    assert "movement_cut_scorecard_passed" in ranking.reasons


def test_movement_cut_scorecard_rejects_invalid_json_and_missing_fields() -> None:
    candidates = [
        bake_and_rank_module.SourceCutCandidate(
            candidate_id="A",
            window=DetectionWindow(index=0, start_seconds=0.0, end_seconds=5.0),
    ...
    assert bake_and_rank_module.parse_movement_cut_candidate_choice("not json", candidates) is None
    ...
    assert ranking is not None
    assert ranking.score == pytest.approx(0.0)
    assert "movement_cut_scorecard_no_passing_candidate" in ranking.reasons
...
def write_source_cut_test_frames(
    ...
    *,
    ...
    for index in range(frame_count):
        image = np.full((180, 320, 3), 255, dtype=np.uint8)
        motion_offset = min(index, frame_count - 1 - index) if frame_count > 1 else 0
        motion_offset = int(motion_offset * 3)
        cv2.rectangle(image, (90, 82 + motion_offset), (245, 94 + motion_offset), (30, 30, 30), -1)
        cv2.rectangle(image, (132, 58 + motion_offset), (220, 110 + motion_offset), (30, 30, 210), -1)
        cv2.circle(image, (80, 88 + motion_offset), 20, (30, 180, 30), -1)
        cv2.circle(image, (260, 88 + motion_offset), 20, (30, 180, 30), -1)
        cv2.rectangle(image, (150, 108 + motion_offset), (178, 152 + motion_offset), (20, 20, 20), -1)
        cv2.rectangle(image, (218, 108 + motion_offset), (246, 152 + motion_offset), (20, 20, 20), -1)
...
def test_source_cut_pose_prefilter_records_windows_without_yolo_motion_overlap(
    tmp_path: Path,
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    video_path = tmp_path / "source.mp4"
    video_path.write_bytes(b"video")
    parent_window = DetectionWindow(index=0, start_seconds=0.0, end_seconds=6.0)
    windows = [
        DetectionWindow(index=0, start_seconds=0.0, end_seconds=2.0),
        DetectionWindow(index=1, start_seconds=4.0, end_seconds=6.0),
    ]
    prompts: list[str] = []

    def fake_windows(**kwargs: object) -> list[DetectionWindow]:
        window = kwargs["window"]
        assert isinstance(window, DetectionWindow)
        if window.start_seconds == pytest.approx(parent_window.start_seconds) and window.end_seconds == pytest.approx(parent_window.end_seconds):
            return windows
        return [window]
    ...
    def fake_caption_images(*, frame_paths: list[Path], prompt: str, **_kwargs: object) -> str:
        prompts.append(prompt)
        candidate_ids = list(dict.fromkeys(path.parent.name.rsplit("_", 1)[-1] for path in frame_paths))
        return json.dumps(
            {
                "candidates": [
                    source_scorecard_candidate(
                        candidate_id,
                        full_movement=0.92 if candidate_id == "B" else 0.20,
                        start_visible=0.92 if candidate_id == "B" else 0.20,
                        finish_visible=0.92 if candidate_id == "B" else 0.20,
                        reject=[] if candidate_id == "B" else ["partial_movement"],
                        note="Candidate has YOLO-backed motion."
                        if candidate_id == "B"
                        else "Candidate lacks reliable visible motion.",
                    )
                    for candidate_id in candidate_ids
                ],
            }
    ...
    result = bake_and_rank_module.rank_source_video_cut_candidates_with_caption_images(
        ...
        exercise_name="Bench Press",
        candidate_title="Bench Press",
        timeline_window=parent_window,
        chunk_estimate=SimpleNamespace(rep_duration_min_sec=2.0, rep_duration_max_sec=2.0),
        output_dir=tmp_path / "source_candidates",
        frame_count=16,
        caption_images=fake_caption_images,
        source_pose_prefilter_payload={
            "validChunks": [
                {"startSeconds": 4.0, "endSeconds": 6.0, "score": 0.92},
            ]
        },
    )
    ...
    assert ranking.payload["selectedCandidateId"] == "B"
    assert all_candidates[0]["posePrefilter"]["passed"] is False
    assert all_candidates[1]["posePrefilter"]["passed"] is True
    assert ranking.payload["sourceCutPosePrefilterFilteredCount"] == 0


def source_pose_phase_sample(time_seconds: float, wrist_y: float) -> dict[str, object]:
    return {
        "timeSeconds": time_seconds,
        "confidence": 0.95,
        "bbox": [0.2, 0.1, 0.8, 0.9],
        "keypoints": {
            "left_shoulder": [0.42, 0.42, 0.95],
            "right_shoulder": [0.58, 0.42, 0.95],
            "left_elbow": [0.40, (0.42 + wrist_y) * 0.5, 0.95],
            "right_elbow": [0.60, (0.42 + wrist_y) * 0.5, 0.95],
            "left_wrist": [0.38, wrist_y, 0.95],
            "right_wrist": [0.62, wrist_y, 0.95],
            "left_hip": [0.44, 0.72, 0.95],
            "right_hip": [0.56, 0.72, 0.95],
            "left_knee": [0.44, 0.86, 0.95],
            "right_knee": [0.56, 0.86, 0.95],
        },
    }


def test_source_cut_motion_gate_rejects_window_without_pose_samples_when_pose_payload_exists() -> None:
    metrics = bake_and_rank_module.source_cut_candidate_motion_coverage_metrics(
        candidate_window=DetectionWindow(index=0, start_seconds=2.0, end_seconds=4.0),
        pose_payload={
            "dominantPoseSamples": [
                source_pose_phase_sample(0.0, 0.72),
                source_pose_phase_sample(0.125, 0.60),
            ],
            "sampleFps": 8.0,
        },
        exercise_name="Bench Press",
    ...
    assert metrics["passed"] is False
    assert metrics["sourcePoseSampleCount"] == 0
    assert metrics["skippedReasons"] == ["source_cut_pose_samples_unavailable"]
    assert metrics["rejectionReasons"] == ["source_cut_insufficient_repetition_phase_evidence"]
    assert metrics["candidateFullRepetitionPhaseCompletenessMetrics"]["reason"] == "source_cut_pose_samples_unavailable"


def test_source_cut_motion_gate_records_one_way_source_pose_fragments(
    tmp_path: Path,
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    video_path = tmp_path / "source.mp4"
        ...
        return json.dumps(
            {
                "candidates": [
                    source_scorecard_candidate(
                        ...
                        finish_visible=0.92 if candidate_id == "B" else 0.20,
                        reject=[] if candidate_id == "B" else ["partial_movement"],
                        note="Candidate has both press directions."
                        if candidate_id == "B"
                        else "Candidate shows only one direction.",
                    )
                    for candidate_id in candidate_ids
                ],
            }
    ...
    result = bake_and_rank_module.rank_source_video_cut_candidates_with_caption_images(
        ...
        candidate_title="Bench Press",
        timeline_window=DetectionWindow(index=0, start_seconds=0.0, end_seconds=6.0),
        chunk_estimate=SimpleNamespace(rep_duration_min_sec=2.0, rep_duration_max_sec=2.0, movement_complexity="compound"),
        output_dir=tmp_path / "source_candidates",
        frame_count=16,
        caption_images=fake_caption_images,
        source_pose_prefilter_payload={
            "dominantPoseSampleCoordinateSpace": "normalized_image_xy",
            "dominantPoseSamples": [
                source_pose_phase_sample(0.0, 0.56),
                source_pose_phase_sample(0.5, 0.46),
                source_pose_phase_sample(1.0, 0.36),
                source_pose_phase_sample(1.5, 0.26),
                source_pose_phase_sample(2.0, 0.16),
                source_pose_phase_sample(4.0, 0.16),
                source_pose_phase_sample(4.5, 0.36),
                source_pose_phase_sample(5.0, 0.56),
                source_pose_phase_sample(5.5, 0.36),
                source_pose_phase_sample(6.0, 0.16),
            ],
        },
    )
    ...
    assert "source_cut_incomplete_repetition_phase" in all_candidates[0]["motionCoverage"]["rejectionReasons"]
    assert ranking.payload["sourceCutMotionCoverageFilteredCount"] == 1
    assert ranking.payload["sourceCutMotionCoverageDiagnosticFailedCount"] == 1


def test_source_cut_motion_gate_records_sparse_pose_false_full_cycle(
    tmp_path: Path,
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    video_path = tmp_path / "source.mp4"
    ...
    windows = [
        DetectionWindow(index=0, start_seconds=0.0, end_seconds=2.0),
        DetectionWindow(index=1, start_seconds=0.0, end_seconds=4.0),
    ]
    prompts: list[str] = []
        ...
        return json.dumps(
            {
                "candidates": [
                    source_scorecard_candidate(
                        ...
                        finish_visible=0.92 if candidate_id == "B" else 0.20,
                        reject=[] if candidate_id == "B" else ["partial_movement"],
                        note="Candidate has enough full-return evidence."
                        if candidate_id == "B"
                        else "Candidate has sparse phase evidence.",
                    )
                    for candidate_id in candidate_ids
                ],
            }
    ...
    result = bake_and_rank_module.rank_source_video_cut_candidates_with_caption_images(
        ...
        candidate_title="Bench Press",
        timeline_window=DetectionWindow(index=0, start_seconds=0.0, end_seconds=4.0),
        chunk_estimate=SimpleNamespace(rep_duration_min_sec=2.0, rep_duration_max_sec=4.0, movement_complexity="compound"),
        output_dir=tmp_path / "source_candidates",
        frame_count=16,
        caption_images=fake_caption_images,
        source_pose_prefilter_payload={
            ...
            "dominantPoseSamples": [
                source_pose_phase_sample(0.0, 0.56),
                source_pose_phase_sample(1.0, 0.16),
                source_pose_phase_sample(2.0, 0.54),
                source_pose_phase_sample(3.0, 0.16),
                source_pose_phase_sample(4.0, 0.56),
            ],
        },
    )
    ...
    assert all_candidates[0]["motionCoverage"]["passed"] is False
    assert all_candidates[1]["motionCoverage"]["passed"] is True
    assert "source_cut_insufficient_repetition_phase_evidence" in all_candidates[0]["motionCoverage"]["rejectionReasons"]
    assert all_candidates[0]["motionCoverage"]["candidateFullRepetitionPhaseCompletenessMetrics"]["reason"] == "insufficient_source_pose_samples"
    assert ranking.payload["sourceCutMotionCoverageFilteredCount"] == 1
    assert ranking.payload["sourceCutMotionCoverageDiagnosticFailedCount"] == 1
...
def test_source_cut_reviews_shortest_windows_without_capping_search(
    ...
    video_path.write_bytes(b"video")
    short_windows = [
        DetectionWindow(index=0, start_seconds=0.0, end_seconds=1.0),
        DetectionWindow(index=1, start_seconds=1.0, end_seconds=2.0),
        DetectionWindow(index=2, start_seconds=2.0, end_seconds=3.0),
        DetectionWindow(index=3, start_seconds=3.0, end_seconds=4.0),
        DetectionWindow(index=4, start_seconds=4.0, end_seconds=5.0),
        DetectionWindow(index=5, start_seconds=5.0, end_seconds=6.0),
        DetectionWindow(index=6, start_seconds=6.0, end_seconds=7.0),
        DetectionWindow(index=7, start_seconds=7.0, end_seconds=8.0),
        DetectionWindow(index=8, start_seconds=7.2, end_seconds=8.2),
        DetectionWindow(index=9, start_seconds=8.0, end_seconds=9.0),
    ]
    long_windows = [
        DetectionWindow(index=10, start_seconds=0.0, end_seconds=2.0),
        DetectionWindow(index=11, start_seconds=2.0, end_seconds=4.0),
    ]

    monkeypatch.setattr(
        bake_and_rank_module,
        "build_source_video_pyramid_candidate_windows",
        lambda **kwargs: [*short_windows, *long_windows],
    )
    monkeypatch.setattr(
        bake_and_rank_module,
        "selected_window_is_materially_shorter",
        ...
        return json.dumps(
                {
                    "candidates": [
                        source_scorecard_candidate(
                            candidate_id,
                            full_movement=0.90 if candidate_id in {"H", "I"} else 0.20,
                            start_visible=0.90 if candidate_id in {"H", "I"} else 0.20,
                            finish_visible=0.90 if candidate_id in {"H", "I"} else 0.20,
                            reject=[] if candidate_id in {"H", "I"} else ["partial_movement"],
                            note="Shortest complete movement." if candidate_id in {"H", "I"} else "Partial movement.",
                        )
                    for candidate_id in candidate_ids
                ]
            }
    ...
    result = bake_and_rank_module.rank_source_video_cut_candidates_with_caption_images(
        ...
        exercise_name="Bench Press",
        candidate_title="Bench Press",
        timeline_window=DetectionWindow(index=0, start_seconds=0.0, end_seconds=10.0),
        chunk_estimate=SimpleNamespace(rep_duration_min_sec=1.0, rep_duration_max_sec=2.0),
        output_dir=tmp_path / "source_candidates",
        frame_count=16,
        caption_images=fake_caption_images,
    )
...
def test_cut_candidate_reviews_one_candidate_per_vlm_request() -> None:
    candidates = [
        bake_and_rank_module.SourceCutCandidate(
            candidate_id=bake_and_rank_module.source_cut_candidate_id_for_index(index),
            window=DetectionWindow(index=index, start_seconds=float(index), end_seconds=float(index) + 1.0),
            frame_paths=[Path(f"candidate_{index}.jpg")],
        )
        for index in range(6)
    ]
    call_sizes: list[int] = []
        ...
        return json.dumps(
            {
                "candidates": [
                    source_scorecard_candidate(
                        candidate_id,
                        full_movement=0.90 if candidate_id == "C" else 0.20,
                        start_visible=0.90 if candidate_id == "C" else 0.20,
                        finish_visible=0.90 if candidate_id == "C" else 0.20,
                        confidence=0.88,
                        reject=[] if candidate_id == "C" else ["partial_movement"],
                        note="Complete." if candidate_id == "C" else "Partial.",
                    )
    ...
    choice = bake_and_rank_module.rank_cut_candidates_shortest_first_with_caption_images(
        candidates=candidates,
        caption_images=fake_caption_images,
        prompt_builder=lambda candidate: bake_and_rank_module.build_source_cut_candidate_choice_prompt(
            exercise_name="Bench Press",
            candidate_title="Bench Press",
            candidate=candidate,
        ),
        parser=lambda raw, candidate: bake_and_rank_module.parse_source_cut_candidate_choice(raw, [candidate]),
    )

    assert call_sizes == [1, 1, 1, 1, 1, 1]
    assert choice.errors == []
    assert choice.reviewed_request_count == 6
    assert choice.ranking is not None
    assert choice.ranking.payload is not None
    assert choice.ranking.payload["selectedCandidateId"] == "C"


def test_source_cut_timeout_request_is_critical_failure(tmp_path: Path) -> None:
    frame_path = tmp_path / "candidate_a.jpg"
    frame_path.write_bytes(b"frame")
    candidates = [
        bake_and_rank_module.SourceCutCandidate(
            candidate_id="A",
    ...
    with pytest.raises(CriticalVlmInteractionError) as exc_info:
        bake_and_rank_module.rank_cut_candidates_shortest_first_with_caption_images(
            candidates=candidates,
            caption_images=lambda **_kwargs: (_ for _ in ()).throw(TimeoutError("timed out")),
            prompt_builder=lambda candidate: bake_and_rank_module.build_source_cut_candidate_choice_prompt(
                exercise_name="Bench Press",
                candidate_title="Bench Press",
                candidate=candidate,
    ...
    assert exc_info.value.recoverable is True
    assert exc_info.value.details["candidateIds"] == ["A"]
    report_path = tmp_path / "vlm_critical_error.json"
    assert report_path.exists()
    report = json.loads(report_path.read_text(encoding="utf-8"))
    assert report["error"]["critical"] is True
    assert report["context"]["candidateIds"] == ["A"]


def test_source_cut_request_choice_prefers_later_equal_duration() -> None:
    candidates = [
        bake_and_rank_module.SourceCutCandidate(
            candidate_id="A",
            window=DetectionWindow(index=0, start_seconds=40.48, end_seconds=44.48),
            frame_paths=[Path("candidate_a.jpg")],
        ),
        bake_and_rank_module.SourceCutCandidate(
            candidate_id="B",
            window=DetectionWindow(index=1, start_seconds=42.48, end_seconds=46.48),
            frame_paths=[Path("candidate_b.jpg")],
        ),
    ]
    ...
    choice = bake_and_rank_module.rank_cut_candidates_shortest_first_with_caption_images(
        ...
        parser=lambda raw, candidate: bake_and_rank_module.parse_source_cut_candidate_choice(raw, [candidate]),
        prefer_later_equal_duration=True,
    )

    assert choice.ranking is not None
    assert choice.ranking.payload is not None
    assert choice.ranking.payload["selectedCandidateId"] == "B"


def test_source_cut_candidate_windows_honor_full_cycle_min_duration_floor() -> None:
    windows = bake_and_rank_module.build_source_cut_candidate_windows(
        window=DetectionWindow(index=0, start_seconds=40.48, end_seconds=46.48),
        chunk_estimate=SimpleNamespace(rep_duration_min_sec=2.0, rep_duration_max_sec=7.0),
        min_estimated_duration_ratio=2.0,
        min_duration_floor_seconds=4.0,
    )

    assert windows
    assert min(window.end_seconds - window.start_seconds for window in windows) >= 4.0 - 1e-6
    assert any(
...
def test_source_cut_full_cycle_rejects_too_short_parent_before_vlm(
    ...
    result = bake_and_rank_module.rank_source_video_cut_candidates_with_caption_images(
        ...
        exercise_name="Bench Press",
        candidate_title="Bench Press",
        timeline_window=DetectionWindow(index=0, start_seconds=44.48, end_seconds=46.48),
        chunk_estimate=SimpleNamespace(rep_duration_min_sec=2.0, rep_duration_max_sec=7.0),
        output_dir=tmp_path / "source_candidates",
        frame_count=16,
        caption_images=fake_caption_images,
...
def test_cut_candidate_requests_can_run_in_parallel() -> None:
    candidates = [
        bake_and_rank_module.SourceCutCandidate(
            candidate_id=bake_and_rank_module.source_cut_candidate_id_for_index(index),
            window=DetectionWindow(index=index, start_seconds=0.0, end_seconds=1.0),
            frame_paths=[Path(f"candidate_{index}.jpg")],
        )
        for index in range(2)
    ]
        ...
        return json.dumps(
            {
                "candidates": [
                    source_scorecard_candidate(
                        candidate_id,
                        full_movement=0.90 if candidate_id == "A" else 0.20,
                        start_visible=0.90 if candidate_id == "A" else 0.20,
                        finish_visible=0.90 if candidate_id == "A" else 0.20,
                        reject=[] if candidate_id == "A" else ["partial_movement"],
                        note="Complete." if candidate_id == "A" else "Partial.",
                    )
                    for candidate_id in candidate_ids
                ]
            }
    ...
    assert choice.ranking is not None
    assert choice.ranking.payload is not None
    assert choice.ranking.payload["selectedCandidateId"] == "A"


def test_source_cut_does_not_recursively_refine_padded_window(
    tmp_path: Path,
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    video_path = tmp_path / "source.mp4"
    ...
    def fake_windows(**kwargs: object) -> list[DetectionWindow]:
        window = kwargs["window"]
        assert isinstance(window, DetectionWindow)
        calls.append((window.start_seconds, window.end_seconds))
        if window.start_seconds == pytest.approx(0.0) and window.end_seconds == pytest.approx(6.0):
            return [
                DetectionWindow(index=0, start_seconds=0.0, end_seconds=6.0),
                DetectionWindow(index=1, start_seconds=1.0, end_seconds=5.0),
            ]
        if window.start_seconds == pytest.approx(1.0) and window.end_seconds == pytest.approx(5.0):
            return [
                DetectionWindow(index=0, start_seconds=1.0, end_seconds=5.0),
                DetectionWindow(index=1, start_seconds=2.0, end_seconds=4.0),
            ]
        return [window]
    ...
    def fake_caption_images(*, frame_paths: list[Path], prompt: str) -> str:
        candidate_ids = re.findall(r"Candidate ([A-Z]+):", prompt)
        selected_id = "B" if "B" in candidate_ids else "A"
        return json.dumps(
            {
                "candidates": [
                    source_scorecard_candidate(
                        candidate_id,
                        full_movement=0.90 if candidate_id == selected_id else 0.20,
                        start_visible=0.90 if candidate_id == selected_id else 0.20,
                        finish_visible=0.90 if candidate_id == selected_id else 0.20,
                        confidence=0.88,
                        reject=[] if candidate_id == selected_id else ["partial_movement"],
                        note="Shortest complete movement." if candidate_id == selected_id else "Partial movement.",
                    )
                    for candidate_id in candidate_ids
                ]
            }
    ...
    result = bake_and_rank_module.rank_source_video_cut_candidates_with_caption_images(
        ...
        candidate_title="Bench Press",
        timeline_window=DetectionWindow(index=0, start_seconds=0.0, end_seconds=6.0),
        chunk_estimate=SimpleNamespace(rep_duration_min_sec=2.0, rep_duration_max_sec=6.0),
        output_dir=tmp_path / "source_candidates",
        frame_count=16,
        caption_images=fake_caption_images,
    )
    ...
    assert ranking.payload is not None
    assert ranking.payload["selected_section_start_seconds"] == pytest.approx(1.0)
    assert ranking.payload["selected_section_end_seconds"] == pytest.approx(5.0)
    assert "sourceCutRefinementStages" not in ranking.payload
    assert "cutRefinementApplied" not in ranking.payload
...
def test_source_cut_candidate_choice_rejects_low_subject_realism() -> None:
        ...
        json.dumps(
            {
                "candidates": [
                    source_scorecard_candidate(
                        "A",
                        source_quality=0.70,
                        reject=["synthetic_subject"],
                        note="The movement is complete but rendered.",
                    )
                ],
            }
        ),
    ...
    assert ranking.score == pytest.approx(0.0)
    row = ranking.payload["sourceCutScorecardCandidates"][0]
    assert "source_cut_scorecard_reject_synthetic_subject" in row["rejectionReasons"]
    assert "source_cut_low_source_quality" in row["rejectionReasons"]
        ...
        json.dumps(
            {
                "candidates": [
                    source_scorecard_candidate(
                        ...
                        confidence=0.88,
                        reject=[],
                        note="The movement is real footage with animated text overlays.",
                    )
                ],
            }
        ),
        ...
        json.dumps(
            {
                "candidates": [
                    source_scorecard_candidate(
                        ...
                        exercise_match=0.0,
                        source_quality=0.0,
                        reject=["wrong_exercise", "synthetic_subject"],
                        note="The subject is not a real target exercise movement.",
                    )
                ]
            }
        ),
...
def test_source_cut_candidate_choice_rejects_unknown_reject_tags() -> None:
    candidates = [
        bake_and_rank_module.SourceCutCandidate(
            candidate_id="C",
        ...
        json.dumps(
            {
                "candidates": [
                    source_scorecard_candidate(
                        "C",
                        reject=["camera_cut"],
                        note="The movement is complete but the response used a non-fixed reject tag.",
                    )
                ],
            }
        ),
    ...
    assert ranking.score == pytest.approx(0.0)
    row = ranking.payload["sourceCutScorecardCandidates"][0]
    assert row["reject"] == ["invalid_reject_tag"]
    assert row["invalidRejectTags"] == ["camera_cut"]
    assert "source_cut_scorecard_invalid_reject_tag" in row["rejectionReasons"]
...
def test_source_cut_candidate_choice_rejects_missing_target_identity_gate() -> None:
                ...
                "candidates": [
                    {
                        ...
                        "confidence": 0.92,
                        "reject": [],
                        "note": "The model omitted target identity score.",
                    }
                ],
            }
        ),
    ...
    assert ranking is not None
    assert ranking.score == pytest.approx(0.0)
    assert "exercise_match" in ranking.payload["sourceCutScorecardCandidates"][0]["missingFields"]
        ...
        json.dumps(
            {
                "candidates": [
                    source_scorecard_candidate(
                        "C",
                        exercise_match=0.45,
                        reject=["wrong_exercise"],
                        note="The visible movement uses related equipment but is not the target exercise.",
                    )
                ],
            }
        ),
...
def test_review_window_contact_sheet_renders_with_selected_postprocessing_options(
    ...
    monkeypatch.setattr(
        bake_and_rank_module,
        "write_review_contact_sheet_from_data_urls",
        lambda _data_urls, output_path, *, timestamps=None, sequence_labels=False: (
            captured.__setitem__("timestamps", timestamps),
            captured.__setitem__("sequence_labels", sequence_labels),
            Path(output_path).write_bytes(b"sheet"),
        ),
    ...
    captured["bake_options"] = None
    captured["render_options"] = []
    captured["timestamps"] = None
    vlm_result = bake_and_rank_module.render_review_window_contact_sheet(
        item=item,
        window=DetectionWindow(index=0, start_seconds=0.0, end_seconds=3.0),
        output_dir=tmp_path / "vlm-frames",
        frame_count=3,
        vlm_review_style=True,
    )

    assert vlm_result == [tmp_path / "vlm-frames" / "contact_sheet.jpg"]
    expected_vlm_options = bake_and_rank_module.vlm_review_render_options(item.settings_options)
    assert captured["bake_options"] == expected_vlm_options
    render_options = captured["render_options"]
    assert isinstance(render_options, list)
    assert render_options
    assert all(options == expected_vlm_options for options in render_options)
    assert captured["timestamps"] == [0.0, 1.5, 3.0]


def test_final_output_preview_contact_sheets_prefers_vlm_review_render(
    tmp_path: Path,
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    preview_html_path = tmp_path / "candidate" / "preview" / "motion_preview.html"
    ...
    paths = bake_and_rank_module.final_output_preview_contact_sheets(
        item,
    ...
    assert window.end_seconds == pytest.approx(3.5)
...
def test_review_item_to_manifest_materializes_exact_selected_source_clip(
    tmp_path: Path,
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    candidate_workspace = tmp_path / "candidate"
    selected_segment = candidate_workspace / "input" / "selected_segment.mp4"
    selected_segment.parent.mkdir(parents=True)
    selected_segment.write_bytes(b"parent video")
    item = ReviewItem(
        exercise_index=0,
        candidate_rank=0,
        loop_index=-1,
    ...
    selected_input_path = Path(manifest["selectedInputVideoPath"])
    assert selected_input_path.name == "selected_source_section_001250_003750.mp4"
    assert selected_input_path.read_bytes() == b"trimmed video"
    assert manifest["selectedInputSourceVideoPath"] == str(selected_segment)
    assert manifest["selectedInputSourceWindow"]["sourceVideoPath"] == str(selected_segment)
    assert manifest["selectedInputSourceWindow"]["startSeconds"] == pytest.approx(1.25)
    assert manifest["selectedInputSourceWindow"]["endSeconds"] == pytest.approx(3.75)
    assert manifest["selectedInputSourceWindow"]["exactClipRequired"] is True
    assert captured["source_path"] == selected_segment
    assert captured["start_seconds"] == pytest.approx(1.25)
    assert captured["end_seconds"] == pytest.approx(3.75)


def test_review_item_to_manifest_maps_materialized_section_to_source_timeline(
    tmp_path: Path,
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    candidate_workspace = tmp_path / "candidate"
    selected_segment = candidate_workspace / "input" / "selected_segment.mp4"
    source_review_video = candidate_workspace / "review" / "full-input.webm"
    selected_segment.parent.mkdir(parents=True)
    source_review_video.parent.mkdir(parents=True)
    selected_segment.write_bytes(b"parent video")
    source_review_video.write_bytes(b"review video")
    item = ReviewItem(
        exercise_index=0,
        candidate_rank=0,
        loop_index=-1,
        ...
        skeleton_path=candidate_workspace / "wear" / "selected.json",
        review_video_path=candidate_workspace / "review" / "selected.webm",
        duration_sec=1.4666666666666666,
        loop_start_seconds=1.1666666666666667,
        loop_end_seconds=2.6333333333333333,
        candidate={"videoId": "squat"},
        llm_time_range_cut_applied=True,
        source_review_video_path=source_review_video,
        source_review_start_seconds=0.0,
        source_review_end_seconds=2.7666666666666666,
    )
    captured: dict[str, object] = {}

    def fake_source_video_duration_seconds(path: Path) -> float:
        if path == selected_segment:
            return 4.7964631297964635
        if path == source_review_video:
            return 2.7666666666666666
        return 0.0

    monkeypatch.setattr(
        bake_and_rank_module,
        "source_video_duration_seconds",
        fake_source_video_duration_seconds,
    )
    ...
    scale = 4.7964631297964635 / 2.7666666666666666
    expected_start = 1.1666666666666667 * scale
    expected_end = 2.6333333333333333 * scale
    expected_name = (
        f"selected_source_section_{int(round(expected_start * 1000)):06d}_"
        f"{int(round(expected_end * 1000)):06d}.mp4"
    )
    selected_input_path = Path(manifest["selectedInputVideoPath"])
    assert selected_input_path.name == expected_name
    assert manifest["selectedInputSourceWindow"]["startSeconds"] == pytest.approx(expected_start)
    assert manifest["selectedInputSourceWindow"]["endSeconds"] == pytest.approx(expected_end)
    assert manifest["selectedInputSourceWindow"]["selectedReviewStartSeconds"] == pytest.approx(1.1666666666666667)
    assert manifest["selectedInputSourceWindow"]["selectedReviewEndSeconds"] == pytest.approx(2.6333333333333333)
    assert manifest["selectedInputSourceWindow"]["sourceReviewStartSeconds"] == pytest.approx(0.0)
    assert manifest["selectedInputSourceWindow"]["sourceReviewEndSeconds"] == pytest.approx(2.7666666666666666)
    assert manifest["selectedInputSourceWindow"]["sourceWindowMappedFromReviewTimeline"] is True
    assert captured["source_path"] == selected_segment
    assert captured["start_seconds"] == pytest.approx(expected_start)
    assert captured["end_seconds"] == pytest.approx(expected_end)


def test_review_item_from_manifest_prefers_source_preview_path_for_rehydration() -> None:
    item = bake_and_rank_module.review_item_from_manifest(
        {
            "exerciseIndex": 0,
            "candidateRank": 0,
            "loopIndex": -1,
            "exerciseName": "Bench Press",
            "candidateTitle": "Demo",
            "candidateWorkspace": "workspace/candidate",
            "previewHtmlPath": "workspace/selected_section_preview.html",
            "sourcePreviewHtmlPath": "workspace/candidate/preview/motion_preview.html",
            "skeletonPath": "workspace/candidate/wear/skeleton.json",
            "reviewVideoPath": "workspace/candidate/review/preview.webm",
            "durationSec": 2.0,
    ...
    assert item.preview_html_path == Path("workspace/candidate/preview/motion_preview.html")
...
def test_final_output_validator_rejects_materialized_selection(
    tmp_path: Path,
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    skeleton_path = tmp_path / "candidate" / "wear" / "selected.json"
    ...
    def fake_validate_final_output_with_caption_images(
        item: ReviewItem,
        ranking: LoopRanking,
        *,
        request: BakeAndRankRequest,
        ...
        return {
            ...
            "passed": False,
            "score": 0.2,
            "rejectionReasons": ["unreadable_preview"],
        }

    monkeypatch.setattr(
        bake_and_rank_module,
        "validate_final_output_with_caption_images",
        fake_validate_final_output_with_caption_images,
    )
    monkeypatch.setattr(
        bake_and_rank_module,
...
def test_final_output_validator_keeps_soft_boundary_concerns_as_manual_review_warning(
    ...
    _selected_item, selected_ranking = selected
    assert bake_and_rank_module.selected_artifact_score(selected) == pytest.approx(0.88)
    assert selected_ranking.payload is not None
    assert selected_ranking.payload["materializedOutputRejected"] is False
    assert selected_ranking.payload["materializedValidationStatus"] == "selected_with_warnings"
    assert selected_ranking.payload["manualBoundaryReviewRequired"] is True
    assert selected_ranking.payload["materializedWarnings"] == ["bad_boundary"]
    assert "materialized_output_rejected" not in selected_ranking.reasons


def test_materialized_acceptance_rejects_hard_deterministic_target_motion_failure(
    tmp_path: Path,
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    item = ReviewItem(
    ...
    assert validation["passed"] is True
    assert validation["backend"] == "stub"
    assert validation["deterministicGatePassed"] is False
    assert validation["deterministicGateBlocking"] is True
    assert validation["deterministicBlockingReasons"] == ["materialized_low_target_motion_observability"]
    assert "materialized_low_target_motion_observability" in validation["deterministicWarnings"]
    assert "final_output_low_deterministic_readability" in validation["deterministicWarnings"]
    assert "materialized_output_rejected" in selected_ranking.reasons


def test_materialized_acceptance_rejects_deterministic_phase_failure_despite_final_vlm_approval(
    tmp_path: Path,
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    item = ReviewItem(
        ...
        candidate_title="Standing Calf Raise demo",
        candidate_workspace=tmp_path / "candidate",
        preview_html_path=tmp_path / "candidate" / "preview" / "motion_preview.html",
        skeleton_path=tmp_path / "candidate" / "wear" / "selected.json",
        review_video_path=tmp_path / "candidate" / "review" / "selected.webm",
        ...
        loop_start_seconds=0.0,
        loop_end_seconds=2.0,
        candidate={"videoId": "partial"},
    )
    ranking = LoopRanking(
        score=0.92,
        reasons=["model_liked_preview"],
        payload={"modelScore": 0.92},
        model_score=0.92,
    )
    ...
    def fake_validate_final_output_with_caption_images(
        ...
        *,
        ...
        return {
            ...
            "reject": [],
            "rejectionReasons": [],
            "note": "The generated movement is acceptable.",
        }
    ...
    assert "materialized_incomplete_repetition_phase" in validation["deterministicWarnings"]
    assert "materialized_output_rejected" in selected_ranking.reasons


def test_final_output_validator_rejects_self_contradictory_pass() -> None:
    parsed = bake_and_rank_module.parse_final_output_validation_response(
        json.dumps(
            {
                "approved": True,
                "confidence": 0.75,
                "retry": False,
                "reject": [],
                "note": "The preview looks more like a Romanian deadlift rather than the target movement.",
            }
        ),
        min_score=0.70,
    )
    ...
    assert "final_output_note_contradicts_approval" in parsed["warningReasons"]


def test_final_output_validation_passes_source_context_before_preview(
    tmp_path: Path,
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    item = ReviewItem(
        ...
        loop_index=-1,
        exercise_name="Weighted Pull-Up",
        candidate_title="Weighted pull up demo",
        candidate_workspace=tmp_path / "candidate",
        preview_html_path=tmp_path / "candidate" / "preview" / "motion_preview.html",
        skeleton_path=tmp_path / "candidate" / "wear" / "selected.json",
        review_video_path=tmp_path / "candidate" / "review" / "selected.webm",
        ...
        loop_start_seconds=0.0,
        loop_end_seconds=2.0,
        candidate={"videoId": "pullup"},
    )
    source_sheet = tmp_path / "source_contact.jpg"
    preview_sheet = tmp_path / "preview_contact.jpg"
    source_sheet.write_bytes(b"source")
    preview_sheet.write_bytes(b"preview")
    monkeypatch.setattr(
        bake_and_rank_module,
        "final_output_source_contact_sheets",
        lambda _item, *, output_dir: [source_sheet],
    )
    monkeypatch.setattr(
        bake_and_rank_module,
        "final_output_preview_contact_sheets",
        lambda _item, *, output_dir: [preview_sheet],
    )
    captured: dict[str, object] = {}
        ...
        return json.dumps(
            {
                ...
                "retry": False,
                "reject": [],
                "note": "The generated preview matches the source movement.",
            }
        )
    ...
    assert captured["frame_paths"] == [source_sheet, preview_sheet]
    assert "Use the source sheets as context" in str(captured["prompt"])
    assert "Do not pass the result because the source is good" in str(captured["prompt"])
    assert '"approved": boolean' in str(captured["prompt"])
    assert validation["approved"] is True
    assert validation["confidence"] == pytest.approx(0.95)
    assert validation["sourceContextProvidedToValidator"] is True
    assert validation["validatorImageOrder"] == ["source_contact_sheets", "preview_contact_sheets"]
    assert validation["sourceContactSheetPaths"] == [str(source_sheet)]
    assert validation["previewContactSheetPaths"] == [str(preview_sheet)]


def test_final_output_validation_does_not_grade_source_without_preview(
    tmp_path: Path,
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    item = ReviewItem(
...
def test_final_output_validation_runs_vlm_despite_deterministic_rejection(
    tmp_path: Path,
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    item = ReviewItem(
    ...
    def fake_validate_final_output_with_caption_images(*_args: Any, **_kwargs: Any) -> dict[str, Any]:
        called["vlm"] = True
        return {
            "enabled": True,
    ...
    validation = bake_and_rank_module.final_output_validation_metrics(
        item,
        LoopRanking(score=0.95, reasons=[], payload={"modelScore": 0.95}, model_score=0.95),
        request=BakeAndRankRequest(
            candidates_json=tmp_path / "candidates.json",
            workspace=tmp_path,
            wham_repo_path=None,
        ...
        deterministic_metrics={
            "passed": False,
            "rejectionReasons": ["materialized_incomplete_repetition_phase"],
        },
    ...
    assert validation["passed"] is True
    assert validation["backend"] == "llama_cpp_vision"
    assert validation["deterministicGatePassed"] is False
    assert "final_output_deterministic_gate_failed" in validation["deterministicWarnings"]


def test_final_output_validation_warns_on_low_deterministic_readability_without_vlm(tmp_path: Path) -> None:
    item = ReviewItem(
        exercise_index=0,
        candidate_rank=0,
        loop_index=-1,
        ...
        loop_start_seconds=0.0,
        loop_end_seconds=2.0,
        candidate={"videoId": "tilted"},
    )
    ranking = LoopRanking(score=0.95, reasons=[], payload={"modelScore": 0.95}, model_score=0.95)
    metrics = {
        "passed": True,
        "previewReadabilityMetrics": {
            "previewReadabilityScore": 0.43,
        },
    }
    ...
    assert validation["passed"] is True
    assert validation["backend"] == "deterministic"
    assert validation["rejectionReasons"] == []
    assert "final_output_low_deterministic_readability" in validation["warningReasons"]
...
def test_preview_html_exposes_headless_bake_automation_api(tmp_path: Path) -> None:
    ...
    assert "vlmReviewStyle" in html
    assert "cameraYawDegrees" in html
    assert "cameraPitchDegrees" in html
    assert "selectTimeRange(startSeconds, endSeconds)" in html
    assert "bakeTimeRange(startSeconds, endSeconds" in html
    assert "sourceRange: sourceTimeBounds()" in html
    assert "manualModelRotationAxisSemantics" in html
    assert "body_lateral_axis_sagittal_plane_normal" in html
    assert "bodyRelativeManualRotationAxes" in html


def test_preview_manual_yaw_bypasses_auto_orientation_with_and_without_feet_lock(tmp_path: Path) -> None:
    playwright_module = pytest.importorskip("playwright.sync_api")
    html_path = tmp_path / "motion_preview.html"
    write_preview_html(html_path, build_fixture_clip(), title="Squat")

    def joint_horizontal_ranges(payload: dict[str, object], joint_name: str) -> tuple[float, float]:
        frames = payload["frames"]
        assert isinstance(frames, list)
        points = [
            ...
            and joint_name in frame["joints"]
        ]
        assert len(points) >= 2
    ...
    with playwright_module.sync_playwright() as playwright:
        browser = bake_and_rank_module.launch_chromium_browser(playwright)
        page = browser.new_page(viewport={"width": 960, "height": 540})
        page.goto(html_path.resolve().as_uri())
        page.wait_for_function("() => window.exerciseMotionAutomation != null")
        for lock_planted_feet in (False, True):
            base_options = {
                "fixedRoot": True,
                "autoWorldAlignment": False,
                "sceneInverted": False,
                "lockYDrift": False,
                "lockPlantedFeet": lock_planted_feet,
                "lockPlantedHands": False,
                "cameraYawDegrees": 45.0,
                "cameraPitchDegrees": 30.0,
                "playbackSpeed": 1.0,
            }
            unrotated_payload = page.evaluate(
                """(options) => window.exerciseMotionAutomation.bakeTimeRange(0, 0.2, options)""",
                {**base_options, "modelRotationYDegrees": 0.0},
            )
            rotated_payload = page.evaluate(
                """(options) => window.exerciseMotionAutomation.bakeTimeRange(0, 0.2, options)""",
                {**base_options, "modelRotationYDegrees": 90.0},
            )
            ...
            assert (
                rotated_payload["selectedPreviewSettings"]["manualModelRotationAxisSemantics"]["y"]
                == "world_y_axis"
            )
            assert (
                rotated_payload["bakedPreviewConfiguration"]["previewSagittalPlaneAlignment"]["status"]
                == "skipped_manual_model_rotation"
            )
            assert (
                rotated_payload["bakedPreviewConfiguration"]["bakedSagittalPlaneAlignment"]["status"]
                == "skipped_manual_model_rotation"
            )

            pitched_payload = page.evaluate(
                """(options) => window.exerciseMotionAutomation.bakeTimeRange(0, 0.2, options)""",
                {**base_options, "modelRotationXDegrees": 10.0, "modelRotationYDegrees": 0.0},
            )
            assert pitched_payload["selectedPreviewSettings"]["manualModelRotationDegrees"]["x"] == 10.0
            assert (
                pitched_payload["selectedPreviewSettings"]["manualModelRotationAxisSemantics"]["x"]
                == "body_lateral_axis_sagittal_plane_normal"
            )
            assert (
                pitched_payload["bakedPreviewConfiguration"]["manualModelRotationAxisSemantics"]["z"]
                == "body_sagittal_forward_axis"
            )
            assert (
                pitched_payload["bakedPreviewConfiguration"]["previewSagittalPlaneAlignment"]["status"]
                != "skipped_manual_model_rotation"
            )
            assert (
                pitched_payload["bakedPreviewConfiguration"]["bakedSagittalPlaneAlignment"]["status"]
                != "skipped_manual_model_rotation"
            )
        browser.close()
